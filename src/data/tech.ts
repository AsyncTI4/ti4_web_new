// Auto-generated file - Do not edit manually
// Generated from src/main/resources/data/technologies/*.json files

import { Tech } from "./types";

export const techs: Tech[] = [
  {
    alias: "absol_ac2",
    name: "Advanced Carrier II",
    types: ["UNITUPGRADE"],
    requirements: "BB",
    faction: "sol",
    baseUpgrade: "absol_cv2",
    source: "absol",
    text: "Cost 3, Combat 9, Move 2, Capacity 8\nSUSTAIN DAMAGE",
    homebrewReplacesID: "ac2",
    imageURL: "/hover_images/techs/faction/advanced_carrier_2.jpg",
  },
  {
    alias: "absol_ah",
    name: "Aerie Hololattice",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "argent",
    source: "absol",
    text: "Other players cannot move ships through systems that contain your structures.\nEach planet that contains 1 or more of your structures gains the PRODUCTION X ability as if it were a unit, where X is the number of structures on that planet.",
    homebrewReplacesID: "ah",
  },
  {
    alias: "absol_aida",
    name: "AI Development Algorithm",
    types: ["WARFARE"],
    source: "absol",
    text: "When you research a unit upgrade technology, you may exhaust this card to ignore any 1 prerequisite.\nWhen you pass, you may spend 6 influence to research a unit upgrade technology.",
    homebrewReplacesID: "aida",
    imageURL: "/hover_images/techs/absol/generic/AIDevelopmentAlgorithm.png",
  },
  {
    alias: "absol_amd",
    name: "Antimass Deflectors",
    types: ["PROPULSION"],
    source: "absol",
    text: "Your ships can move into and through asteroid fields and retain their printed Move value when moving out of nebulae.\nWhen other players' units use SPACE CANNON against your units, apply -1 to the result of each die roll.",
    homebrewReplacesID: "amd",
    imageURL: "/hover_images/techs/absol/generic/AntimassDeflectors.png",
  },
  {
    alias: "absol_as",
    name: "Aetherstream",
    types: ["PROPULSION"],
    requirements: "BB",
    faction: "empyrean",
    source: "absol",
    text: "After you or one of your neighbors activates a system that is adjacent to an anomaly, you may apply +1 to the move value of all of that player's ships during this tactical action.",
    homebrewReplacesID: "as",
    imageURL: "/hover_images/techs/faction/aetherstream.jpg",
  },
  {
    alias: "absol_asc",
    name: "Assault Cannon",
    types: ["WARFARE"],
    requirements: "RRR",
    source: "absol",
    text: "At the start of space combat, your opponent must destroy 1 of their non-fighter ships for every 3 of your non-fighter, non-Infantry ships in the active system.",
    homebrewReplacesID: "asc",
    imageURL: "/hover_images/techs/absol/generic/AssaultCannon.png",
  },
  {
    alias: "absol_asn",
    name: "Agency Supply Network",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "keleres",
    source: "absol",
    text: "After you resolve the PRODUCTION abilities of 1 or more of your units, you may resolve the PRODUCTION ability of 1 of your other units in any system; the additional use does not trigger this ability.",
    homebrewReplacesID: "asn",
    imageURL: "/hover_images/techs/absol/AgencySupplyNetwork.png",
  },
  {
    alias: "absol_bio",
    name: "Bioplasmosis",
    types: ["BIOTIC"],
    requirements: "GG",
    faction: "arborec",
    source: "absol",
    text: "When you produce units in a system, you may exhaust this card and reduce the combined PRODUCTION value of your units in that system by up to half (rounded down) to reduce the combined cost of the produced units by the same amount.",
    homebrewReplacesID: "bio",
    imageURL: "/hover_images/techs/absol/Bioplasmosis.png",
  },
  {
    alias: "absol_bs",
    name: "Bio-stims",
    types: ["BIOTIC"],
    requirements: "G",
    source: "absol",
    text: "You may exhaust this card at the end of your turn to ready 1 of your other technologies.",
    homebrewReplacesID: "bs",
    imageURL: "/hover_images/techs/absol/generic/Biostims.png",
  },
  {
    alias: "absol_cl2",
    name: "Crimson Legionnaire II",
    types: ["UNITUPGRADE"],
    requirements: "GG",
    faction: "mahact",
    baseUpgrade: "absol_inf2",
    source: "absol",
    text: "Cost: 1(x2), Combat 7\nAfter this unit is destroyed, gain 2 commodities or convert 1 of your commodities to a trade good. Then, place the unit on this card. At the start of your next turn, place each unit that is on this card on a planet that contains 1 of your structures.",
    homebrewReplacesID: "cl2",
    imageURL: "/hover_images/techs/absol/CrimsonLegionnaireII.png",
  },
  {
    alias: "absol_cm",
    name: "Chaos Mapping",
    types: ["PROPULSION"],
    requirements: "B",
    faction: "saar",
    source: "absol",
    text: "At the start of your turn, you may exhaust this card to produce 1 unit at each of your space docks.\nApply +2 to your ship's combat rolls in asteroid fields.",
    homebrewReplacesID: "cm",
    imageURL: "/hover_images/techs/absol/ChaosMapping.png",
  },
  {
    alias: "absol_cr2",
    name: "Cruiser II",
    types: ["UNITUPGRADE"],
    requirements: "GYR",
    source: "absol",
    text: "Cost 2, Combat 6, Move 3, Capacity 1",
    homebrewReplacesID: "cr2",
    imageURL: "/hover_images/techs/generic/cruiser_2.jpg",
  },
  {
    alias: "absol_cv2",
    name: "Carrier II",
    types: ["UNITUPGRADE"],
    requirements: "BB",
    source: "absol",
    text: "Cost 3, Combat 9, Move 2, Capacity 6",
    homebrewReplacesID: "cv2",
    imageURL: "/hover_images/techs/generic/carrier_2.jpg",
  },
  {
    alias: "absol_da",
    name: "Duranium Armor",
    types: ["WARFARE"],
    requirements: "RR",
    source: "absol",
    text: "Your ships can move into and through asteroid fields.\nDuring each combat round, after you assign hits to your units, repair 1 of your damaged units that did not use SUSTAIN DAMAGE during this combat round.",
    homebrewReplacesID: "da",
    imageURL: "/hover_images/techs/absol/generic/DuraniumArmor.png",
  },
  {
    alias: "absol_dd2",
    name: "Destroyer II",
    types: ["UNITUPGRADE"],
    requirements: "RR",
    source: "absol",
    text: "Cost 1, Combat 8, Move 2\nANTI-FIGHTER BARRAGE 6(x3)\nWhen this unit generates a hit on a combat roll, your opponent must destroy 1 fighter in the active system, if able.",
    homebrewReplacesID: "dd2",
    imageURL: "/hover_images/techs/absol/generic/DestroyerII.png",
  },
  {
    alias: "absol_det",
    name: "Dark Energy Tap",
    types: ["PROPULSION"],
    source: "absol",
    text: "After you perform a tactical action in a system that contains a frontier token, if you have 1 or more ships in that system, you may explore that token.\nYour ships can retreat into adjacent systems that do not contain other players' units, even if you do not have units or control planets in that system.",
    homebrewReplacesID: "det",
    imageURL: "/hover_images/techs/absol/generic/DarkEnergyTap.png",
  },
  {
    alias: "absol_dn2",
    name: "Dreadnought II",
    types: ["UNITUPGRADE"],
    requirements: "BBY",
    source: "absol",
    text: 'Cost 4, Combat 5, Move 2, Capacity 1\nSUSTAIN DAMAGE, BOMBARDMENT 5\nThis unit cannot be destroyed by "Direct Hit" action cards.',
    homebrewReplacesID: "dn2",
    imageURL: "/hover_images/techs/generic/dreadnought_2.jpg",
  },
  {
    alias: "absol_ds",
    name: "Dimensional Splicer",
    types: ["WARFARE"],
    requirements: "R",
    faction: "ghost",
    source: "absol",
    text: "During each round of space combat in a system that contains 1 or more wormholes, you produce 1 additional hit.  Your opponent must assign this hit to 1 of their non-fighter ships, if able.",
    homebrewReplacesID: "ds",
    imageURL: "/hover_images/techs/absol/DimensionalSplicer.png",
  },
  {
    alias: "absol_dt2",
    name: "Dimensional Tear II",
    types: ["UNITUPGRADE"],
    requirements: "YY",
    faction: "cabal",
    baseUpgrade: "absol_sd2",
    source: "absol",
    text: "PRODUCTION 7\nThis system is a gravity rift; your ships do not roll for this gravity rift. Place a dimensional tear token beneath this unit as a reminder.\nUp to 10 fighters in this system do not count against your ships' capacity. 1 non-fighter ship in this system does not count against your fleet pool.",
    homebrewReplacesID: "dt2",
    imageURL: "/hover_images/techs/absol/DimensionalTearII.png",
  },
  {
    alias: "absol_dxa",
    name: "Dacxive Animators",
    types: ["BIOTIC"],
    requirements: "G",
    source: "absol",
    text: "After you gain control of a planet, you may place 1 infantry from your reinforcements on that planet.\nIf you have an infantry unit upgrade technology, you may reroll any dice rolled as part of your infantry units' abilities.",
    homebrewReplacesID: "dxa",
    imageURL: "/hover_images/techs/absol/generic/DacxiveAnimators.png",
  },
  {
    alias: "absol_ers",
    name: "E-Res Siphons",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "jolnar",
    source: "absol",
    text: "When another player moves ships into a system that contains 1 or more of your structures, gain trade goods equal to the number of non-fighter ships that moved into that system.",
    homebrewReplacesID: "ers",
    imageURL: "/hover_images/techs/absol/faction/EResSiphons.png",
  },
  {
    alias: "absol_exo2",
    name: "Exotrireme II",
    types: ["UNITUPGRADE"],
    requirements: "BBY",
    faction: "sardakk",
    baseUpgrade: "absol_dn2",
    source: "absol",
    text: 'Cost 4, Combat 5, Move 2, Capacity 1\nSUSTAIN DAMAGE, BOMBARDMENT 4(x2)\nThis unit cannot be destroyed by "Direct Hit" action cards. After a round of space combat, you may destroy this unit to destroy up to 2 ships in this system.',
    homebrewReplacesID: "exo2",
    imageURL: "/hover_images/techs/faction/exotrireme_2.jpg",
  },
  {
    alias: "absol_ff2",
    name: "Fighter II",
    types: ["UNITUPGRADE"],
    requirements: "BG",
    source: "absol",
    text: "Cost: 1(x2), Combat 8, Move 2\nThis unit may move without being transported. Fighters in excess of your ships' capacity count against your fleet pool.",
    homebrewReplacesID: "ff2",
    imageURL: "/hover_images/techs/generic/fighter_2.jpg",
  },
  {
    alias: "absol_ffac2",
    name: "Floating Factory II",
    types: ["UNITUPGRADE"],
    requirements: "YY",
    faction: "saar",
    baseUpgrade: "absol_sd2",
    source: "absol",
    text: "Move 2, Capacity 6\nPRODUCTION 7\nThis unit is placed in the space area instead of on a planet. This unit can move and retreat as if it were a ship. If this unit is blockaded, it is destroyed. 1 non-fighter ship in this system does not count against your fleet pool.",
    homebrewReplacesID: "ffac2",
    imageURL: "/hover_images/techs/absol/FloatingFactoryII.png",
  },
  {
    alias: "absol_fl",
    name: "Fleet Logistics",
    types: ["PROPULSION"],
    requirements: "BB",
    source: "absol",
    text: "Once per turn, after you perform a tactical action, you may take an additional tactical action. You may activate the same system for this second tactical action; in that case, ensure that only 1 of your command tokens total is in the system by returning the second command token to your reinforcements instead of placing it in the active system.",
    homebrewReplacesID: "fl",
    imageURL: "/hover_images/techs/absol/generic/FleetLogistics.png",
  },
  {
    alias: "absol_gd",
    name: "Gravity Drive",
    types: ["PROPULSION"],
    requirements: "B",
    source: "absol",
    text: "During your tactical actions, apply +1 to the move value of each of your ships that moves through 1 or more systems adjacent to a gravity rift or supernova, or that moves through 1 or more wormholes.\nAfter you roll dice for your ships moving through a gravity rift, you may reroll any number of those dice.",
    homebrewReplacesID: "gd",
    imageURL: "/hover_images/techs/absol/generic/GravityDrive.png",
  },
  {
    alias: "absol_gls",
    name: "Graviton Laser System",
    types: ["CYBERNETIC"],
    requirements: "Y",
    source: "absol",
    text: "When a player would resolve a retreat, your units may first use SPACE CANNON against the retreating player’s ships in the active system.\nYou may exhaust this card before 1 or more of your units use SPACE CANNON; hits produced by those units must be assigned to non-fighter ships if able.",
    homebrewReplacesID: "gls",
    imageURL: "/hover_images/techs/absol/generic/GravitonLaserSystems.png",
  },
  {
    alias: "absol_gr",
    name: "Genetic Recombination",
    types: ["BIOTIC"],
    requirements: "G",
    faction: "mahact",
    source: "absol",
    text: "When you would cast votes during the agenda phase, you may exhaust 1 planet controlled by each other player whose command tokens are on the Mahact player’s command sheet. You may cast votes from these planets as if you controlled them, even if you would otherwise be prevented from voting.",
    homebrewReplacesID: "gr",
    imageURL: "/hover_images/techs/absol/GeneticRecombination.png",
  },
  {
    alias: "absol_hcf2",
    name: "Hybrid Crystal Fighter II",
    types: ["UNITUPGRADE"],
    requirements: "BG",
    faction: "naalu",
    baseUpgrade: "absol_ff2",
    source: "absol",
    text: "Cost: 1(x2), Combat 7, Move 2\nThis unit may move without being transported. Fighters in excess of your ships' capacity count as 1/2 of a ship against your fleet pool.",
    homebrewReplacesID: "hcf2",
    imageURL: "/hover_images/techs/faction/hybrid_crystal_fighter_2.jpg",
  },
  {
    alias: "absol_hm",
    name: "Hyper Metabolism",
    types: ["BIOTIC"],
    requirements: "GG",
    source: "absol",
    text: "Once per turn, after you perform an action, you may take a component action. \nYou may exhaust this card at the end of your turn to gain 1 command token.",
    homebrewReplacesID: "hm",
    imageURL: "/hover_images/techs/absol/generic/HyperMetabolism.png",
  },
  {
    alias: "absol_ht2",
    name: "Hel-Titan II",
    types: ["UNITUPGRADE"],
    requirements: "YR",
    faction: "titans",
    baseUpgrade: "absol_pds2",
    source: "absol",
    text: "Combat 6.\nPLANETARY SHIELD, SPACE CANNON 5, SUSTAIN DAMAGE, PRODUCTION 1\nThis unit is treated as both a structure and a ground force. It cannot be transported. You may use this unit's SPACE CANNON against ships that are adjacent to this unit's systems.",
    homebrewReplacesID: "ht2",
    imageURL: "/hover_images/techs/faction/heltitan_2.jpg",
  },
  {
    alias: "absol_htp",
    name: "Hegemonic Trade Policy",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "winnu",
    source: "absol",
    text: "During the Production step of your tactical actions, you may treat the resource value of any number of planets you control as if it were equal to that planet's influence value instead.",
    homebrewReplacesID: "htp",
    imageURL: "/hover_images/techs/absol/faction/HegemonicTradePolicy",
  },
  {
    alias: "absol_ic",
    name: "Impulse Core",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "yin",
    source: "absol",
    text: "ACTION: Exhaust this card, choose a system and destroy any number of your cruisers or destroyers in adjacent systems. Generate 1 hit for each ship you destroyed; you opponent must assign those hits to their ships in the chosen system, and must assign them to non-fighter ships, if able.",
    homebrewReplacesID: "ic",
    imageURL: "/hover_images/techs/absol/ImpulseCore.png",
  },
  {
    alias: "absol_ie",
    name: "Integrated Economy",
    types: ["CYBERNETIC"],
    requirements: "YYY",
    source: "absol",
    text: "You may treat each planet with a planet trait that contains 1 or more of your structures as having any planet trait.\nEach planet you control gains the PRODUCTION 1 ability as if it were a unit.",
    homebrewReplacesID: "ie",
    imageURL: "/hover_images/techs/absol/generic/IntegratedEconomy.png",
  },
  {
    alias: "absol_iihq",
    name: "I.I.H.Q. Modernization",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "keleres",
    source: "absol",
    text: "You are neighbors with all players that have units or control planets in or adjacent to the Mecatol Rex system.\nGain the Custodia Vigilia planet card and its legendary planet ability card. You cannot lose these cards, and this card cannot have an X or Y assimilator token placed on it.\n\n[Custodia Vigilia is a 2/3 legendary planet with the ability:\nWhile you control Mecatol Rex, it gains SPACE CANNON 5 and PRODUCTION 3.\nGain 2 command tokens when another player scores Custodians VP using Imperial Primary.]",
    homebrewReplacesID: "iihq",
    imageURL: "/hover_images/techs/faction/iihq_modernization.jpg",
  },
  {
    alias: "absol_inf2",
    name: "Infantry II",
    types: ["UNITUPGRADE"],
    requirements: "GG",
    source: "absol",
    text: "Cost: 1(x2), Combat 7\nAfter this unit is destroyed, roll 1 die. If the result is 6 or greater, place the unit on this card. At the start of your next turn, place each unit that is on this card on a planet you control in your home system.",
    homebrewReplacesID: "inf2",
    imageURL: "/hover_images/techs/generic/infantry_2.jpg",
  },
  {
    alias: "absol_is",
    name: "Inheritance Systems",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "l1z1x",
    source: "absol",
    text: "When you research a technology, you may exhaust this card to ignore all prerequisites on that technology.\nACTION: You may exhaust this card and spend 1 command token from your strategy pool and 4 resources to research a technology.",
    homebrewReplacesID: "is",
    imageURL: "/hover_images/techs/absol/faction/Inheritancesystems.png",
  },
  {
    alias: "absol_it",
    name: "Instinct Training",
    types: ["BIOTIC"],
    requirements: "GG",
    faction: "xxcha",
    source: "absol",
    text: "You may spend 1 token from your strategy pool when one of your neighbors plays an action card; cancel that action card.",
    homebrewReplacesID: "it",
    imageURL: "/hover_images/techs/absol/faction/InstinctTraining.png",
  },
  {
    alias: "absol_l4",
    name: "L4 Disruptors",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "letnev",
    source: "absol",
    text: "During an invasion, units cannot use SPACE CANNON against your units.\nYou may exhaust this card at the start of a ground combat; your opponent cannot make combat rolls during the first combat round.",
    homebrewReplacesID: "l4",
    imageURL: "/hover_images/techs/absol/faction/L4Disruptors.png",
  },
  {
    alias: "absol_lgf",
    name: "Lazax Gate Folding",
    types: ["PROPULSION"],
    requirements: "BB",
    faction: "winnu",
    source: "absol",
    text: "During your tactical actions, you treat your home system and the Mecatol Rex system as if they each contained a gamma wormhole.",
    homebrewReplacesID: "lgf",
    imageURL: "/hover_images/techs/absol/faction/LazaxGateFolding.png",
  },
  {
    alias: "absol_lw2",
    name: "Letani Warrior II",
    types: ["UNITUPGRADE"],
    requirements: "GG",
    faction: "arborec",
    baseUpgrade: "absol_inf2",
    source: "absol",
    text: "Cost: 1(x2), Combat 7\nPRODUCTION 2\nAfter this unit is destroyed, place the unit on this card.\nAt the start of your next turn, for every 2 units that are on this card, place 1 on a planet you control.",
    homebrewReplacesID: "lw2",
    imageURL: "/hover_images/techs/absol/LetaniWarriorII.png",
  },
  {
    alias: "absol_lwd",
    name: "Light/Wave Deflector",
    types: ["PROPULSION"],
    requirements: "BBB",
    source: "absol",
    text: "Your ships can move through systems that contain other players' ships.",
    homebrewReplacesID: "lwd",
    imageURL: "/hover_images/techs/generic/lightwave_deflector.jpg",
  },
  {
    alias: "absol_m2",
    name: "Memoria II",
    types: ["UNITUPGRADE"],
    requirements: "BGY",
    faction: "nomad",
    source: "absol",
    text: "Cost 8, Combat 5(x3), Move 3, Capacity 6\nSUSTAIN DAMAGE, ANTI-FIGHTER BARRAGE 5(x3)\nYou may treat this unit as if it were adjacent to systems that contain 1 or more of your mechs.",
    homebrewReplacesID: "m2",
    imageURL: "/hover_images/techs/absol/MemoriaII.png",
  },
  {
    alias: "absol_mc",
    name: "Mirror Computing",
    types: ["CYBERNETIC"],
    requirements: "YYY",
    faction: "mentak",
    source: "absol",
    text: "When you spend trade goods, every 2 trade goods spent are worth 3 resources or influence instead of 2.",
    homebrewReplacesID: "mc",
    imageURL: "/hover_images/techs/absol/faction/MirrorComputing.png",
  },
  {
    alias: "absol_md",
    name: "Magen Defense Grid",
    types: ["WARFARE"],
    requirements: "R",
    source: "absol",
    text: "At the start of ground combat on a planet you control that contains 1 or more of your structures, you may produce 1 hit per structure and assign them to your opponent's ground forces.\nAfter you perform a tactical action, you may exhaust this card and spend 4 resources to place a structure on a planet you control in the active system.",
    homebrewReplacesID: "md",
    imageURL: "/hover_images/techs/absol/generic/MagenDefenseGrid.png",
  },
  {
    alias: "absol_mi",
    name: "Mageon Implants",
    types: ["BIOTIC"],
    requirements: "GGG",
    faction: "yssaril",
    source: "absol",
    text: "After 1 of your command tokens is placed in a system that contains another player's units, you may exhaust this card to look at that player's hand of action cards. Choose 1 of those cards and add it to your hand.",
    homebrewReplacesID: "mi",
    imageURL: "/hover_images/techs/absol/faction/MageonImplants.png",
  },
  {
    alias: "absol_mr",
    name: "Magmus Reactor",
    types: ["WARFARE"],
    requirements: "RR",
    faction: "muaat",
    source: "absol",
    text: "Your ships can move into supernovas.\nEach supernova that contains 1 or more of your units gains the PRODUCTION 5 ability as if it were 1 of your units.",
    homebrewReplacesID: "mr",
    imageURL: "/hover_images/techs/faction/magmus_reactor_omega.jpg",
  },
  {
    alias: "absol_nes",
    name: "Non-Euclidean Shielding",
    types: ["WARFARE"],
    requirements: "RR",
    faction: "letnev",
    source: "absol",
    text: "When 1 of your units uses SUSTAIN DAMAGE, cancel 2 hits instead of 1.",
    homebrewReplacesID: "nes",
    imageURL: "/hover_images/techs/faction/noneuclidean_shielding.jpg",
  },
  {
    alias: "absol_nf",
    name: "Nullification Field",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "xxcha",
    source: "absol",
    text: "After another player activates a system that contains 1 or more of your ships, you may exhaust this card and spend 1 token from your strategy pool; immediately end that player's turn.",
    homebrewReplacesID: "nf",
    imageURL: "/hover_images/techs/faction/nullification_field.jpg",
  },
  {
    alias: "absol_ng",
    name: "Neuroglaive",
    types: ["BIOTIC"],
    requirements: "GGG",
    faction: "naalu",
    source: "absol",
    text: "After another player activates a system that contains 1 or more of your ships, you may force that player to remove 1 token from their fleet pool and return it to their reinforcements.\nDuring combat, when your opponent plays an action card, produce 1 hit; your opponent must assign it to 1 of their units.",
    homebrewReplacesID: "ng",
    imageURL: "/hover_images/techs/absol/faction/Neuroglaive.png",
  },
  {
    alias: "absol_nm",
    name: "Neural Motivator",
    types: ["BIOTIC"],
    source: "absol",
    text: "The maximum number of action cards you can have in your hand is increased by 3\nAt the end of your turn, you may exhaust this card to draw 2 action cards.",
    homebrewReplacesID: "nm",
    imageURL: "/hover_images/techs/absol/generic/NeuralMotivator.png",
  },
  {
    alias: "absol_pa",
    name: "Psychoarchaeology",
    types: ["BIOTIC"],
    source: "absol",
    text: "You can use technology specialties on planets you control without exhausting them, even if those planets are exhausted.\nAt the end of your turn, you may discard 2 action cards to explore a readied planet you control.",
    homebrewReplacesID: "pa",
    imageURL: "/hover_images/techs/absol/generic/Psychoarchaeology.png",
  },
  {
    alias: "absol_pds2",
    name: "PDS II",
    types: ["UNITUPGRADE"],
    requirements: "YR",
    source: "absol",
    text: "PLANETARY SHIELD, SPACE CANNON 5\nYou may use this unit's SPACE CANNON against ships that are adjacent to this unit's system.",
    homebrewReplacesID: "pds2",
    imageURL: "/hover_images/techs/generic/pds_2.jpg",
  },
  {
    alias: "absol_pfa",
    name: "Pre-Fab Archologies",
    types: ["BIOTIC"],
    requirements: "GGG",
    faction: "naaz",
    source: "absol",
    text: "After you explore a planet, ready that planet.",
    homebrewReplacesID: "pfa",
    imageURL: "/hover_images/techs/faction/prefab_arcologies.jpg",
  },
  {
    alias: "absol_pi",
    name: "Predictive Intelligence",
    types: ["CYBERNETIC"],
    requirements: "Y",
    source: "absol",
    text: "At the start of your turn, you may exhaust this card to redistribute your command tokens.\nWhen you cast votes during the agenda phase, you may cast 3 additional votes",
    homebrewReplacesID: "pi",
    imageURL: "/hover_images/techs/absol/generic/PredictiveIntelligence.png",
  },
  {
    alias: "absol_pm",
    name: "Production Biomes",
    types: ["BIOTIC"],
    requirements: "GG",
    faction: "hacan",
    source: "absol",
    text: "When you give another player 1 or more commodities, they gain an equal number of trade goods.",
    homebrewReplacesID: "pm",
    imageURL: "/hover_images/techs/absol/faction/ProductionBiomes.png",
  },
  {
    alias: "absol_ps",
    name: "Plasma Scoring",
    types: ["WARFARE"],
    source: "absol",
    text: "When 1 or more of your units use unit abilities, 1 of those units may roll 1 additional die.\nWhen the result of your non-fighter ships' combat roll exceeds that ship's combat value by 3 or more, your opponent cannot assign that hit to a fighter unless they first destroy 1 of their fighters in the active system.",
    homebrewReplacesID: "ps",
    imageURL: "/hover_images/techs/absol/generic/PlasmaScoring.png",
  },
  {
    alias: "absol_pws2",
    name: "Prototype Warsun II",
    types: ["UNITUPGRADE"],
    requirements: "YRRR",
    faction: "muaat",
    source: "absol",
    text: 'Cost 8, Combat 3(x3), Move 3, Capacity 6\nSUSTAIN DAMAGE, BOMBARDMENT 3(x3)\nOther player\'s units in this system lose PLANETARY SHIELD. This unit cannot be destroyed by "Direct Hit" action cards.',
    homebrewReplacesID: "pws2",
    imageURL: "/hover_images/techs/absol/PrototypeWarSunII.png",
  },
  {
    alias: "absol_qdn",
    name: "Quantum Datahub Node",
    types: ["CYBERNETIC"],
    requirements: "YYY",
    faction: "hacan",
    source: "absol",
    text: "When another player would choose a strategy card during the strategy phase, if you do not have a strategy card, you may exhaust this card, spend a token from your strategy pool, and give that player 1 trade good for each player that does not have a strategy card to choose your strategy card before them.",
    homebrewReplacesID: "qdn",
    imageURL: "/hover_images/techs/absol/faction/QuantumDatahubNode.png",
  },
  {
    alias: "absol_sar",
    name: "Self-Assembly Routines",
    types: ["WARFARE"],
    requirements: "R",
    source: "absol",
    text: "You may produce 1 additional mech for their cost. [Note: think of mechs as now similar to fighters and infantry, buy 2 for the price of 1]\nACTION: Exhaust this card to produce 1 ship in any system that contains 1 of your space docks.",
    homebrewReplacesID: "sar",
    imageURL: "/hover_images/techs/absol/generic/SelfAssemblyRoutines.png",
  },
  {
    alias: "absol_sc",
    name: "Supercharge",
    types: ["WARFARE"],
    requirements: "R",
    faction: "naaz",
    source: "absol",
    text: "At the start of a combat round, you may exhaust a relic fragment in your play area to apply +1 to the results of each of your units’ combat rolls during this combat round.",
    homebrewReplacesID: "sc",
    imageURL: "/hover_images/techs/absol/Supercharge.png",
  },
  {
    alias: "absol_scc",
    name: "Spatial Conduit Cylinder",
    types: ["PROPULSION"],
    requirements: "BB",
    faction: "jolnar",
    source: "absol",
    text: "During your tactical actions, you may treat systems that contain 1 or more of your space docks as adjacent.",
    homebrewReplacesID: "scc",
    imageURL: "/hover_images/techs/absol/faction/SpatialConduitCylinder.png",
  },
  {
    alias: "absol_sd2",
    name: "Space Dock II",
    types: ["UNITUPGRADE"],
    requirements: "YY",
    source: "absol",
    text: "PRODUCTION X\nThis unit's PRODUCTION value is equal to 4 more than the resource value of this planet.\nUp to 5 fighters in this system do not count against your ships' capacity. 1 non-fighter ship in this system does not count against your fleet pool.",
    homebrewReplacesID: "sd2",
    imageURL: "/hover_images/techs/absol/generic/SpaceDockII.png",
  },
  {
    alias: "absol_sdn",
    name: "Scanlink Drone Network",
    types: ["CYBERNETIC"],
    source: "absol",
    text: "When you activate a system as part of a tactical action, you may explore 1 planet in that system that contains 1 or more of your units.\nWhenever you would resolve an exploration card, you may instead discard that card and gain 1 trade good.",
    homebrewReplacesID: "sdn",
    imageURL: "/hover_images/techs/absol/generic/ScanlinkDroneNetwork.png",
  },
  {
    alias: "absol_sdn2",
    name: "Super-Dreadnought II",
    types: ["UNITUPGRADE"],
    requirements: "BBY",
    faction: "l1z1x",
    baseUpgrade: "absol_dn2",
    source: "absol",
    text: 'Cost 4, Combat 4, Move 2, Capacity 2\nSUSTAIN DAMAGE, BOMBARDMENT 4\nThis unit cannot be destroyed by "Direct Hit" action cards.',
    homebrewReplacesID: "sdn2",
    imageURL: "/hover_images/techs/faction/superdreadnought_2.jpg",
  },
  {
    alias: "absol_se2",
    name: "Saturn Engine II",
    types: ["UNITUPGRADE"],
    requirements: "GYR",
    faction: "titans",
    baseUpgrade: "absol_cr2",
    source: "absol",
    text: "Cost 2, Combat 6, Move 3, Capacity 2\nSUSTAIN DAMAGE",
    homebrewReplacesID: "se2",
    imageURL: "/hover_images/techs/faction/saturn_engine_2.jpg",
  },
  {
    alias: "absol_so",
    name: "Salvage Operations",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "mentak",
    source: "absol",
    text: "At the end of each space combat in systems that contain or are adjacent to your ships, gain 1 commodity.\nIf you win a space combat, you may produce 1 ship of any type that was destroyed during that combat, and may spend commodities as resources.",
    homebrewReplacesID: "so",
    imageURL: "/hover_images/techs/absol/faction/SalvageOperations.png",
  },
  {
    alias: "absol_so2",
    name: "Spec Ops II",
    types: ["UNITUPGRADE"],
    requirements: "GG",
    faction: "sol",
    baseUpgrade: "absol_inf2",
    source: "absol",
    text: "Cost: 1(x2), Combat 6\nAfter this unit is destroyed, roll 1 die. If the result is 5 or greater, place the unit on this card. At the start of your next turn, place each unit that is on this card on a planet that contains 1 of your space docks.",
    homebrewReplacesID: "so2",
    imageURL: "/hover_images/techs/absol/SpecOpsII.png",
  },
  {
    alias: "absol_sr",
    name: "Sling Relay",
    types: ["PROPULSION"],
    requirements: "B",
    source: "absol",
    text: "During your tactical actions, your non-blockaded space docks in adjacent systems may use the PRODUCTION 1 ability in the active system if it does not contain other players' ships. If you have a space dock unit upgrade technology, they instead may use PRODUCTION 2. All units produced are placed in the space area.",
    homebrewReplacesID: "sr",
    imageURL: "/hover_images/techs/absol/generic/SlingRelay.png",
  },
  {
    alias: "absol_st",
    name: "Sarween Tools",
    types: ["CYBERNETIC"],
    source: "absol",
    text: "'When you produce units, reduce the combined cost of the produced units by 1, plus an additional 1 for every 10 of the combined cost.\nDuring the production step of your tactical actions, you may spend 1 resource to repair 1 of your damaged units in the active system, or all of your damaged units in the active system if that system contains 1 or more of your space docks.",
    homebrewReplacesID: "st",
    imageURL: "/hover_images/techs/absol/generic/SarweenTools.png",
  },
  {
    alias: "absol_swa2",
    name: "Strike Wing Alpha II",
    types: ["UNITUPGRADE"],
    requirements: "RR",
    faction: "argent",
    baseUpgrade: "absol_dd2",
    source: "absol",
    text: "Cost 1, Combat 7, Move 2, Capacity 1\nANTI-FIGHTER BARRAGE 6(x3)\nWhen this unit generates a hit on a combat roll, your opponent must destroy 1 fighter in the active system, if able. When this unit uses ANTI-FIGHTER BARRAGE, each result of 9 or 10 also destroys 1 of your opponents infantry in the space area of the active system.",
    homebrewReplacesID: "swa2",
    imageURL: "/hover_images/techs/absol/StrikeWingAlphaII.png",
  },
  {
    alias: "absol_tcs",
    name: "Temporal Command Suite",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "nomad",
    source: "absol",
    text: "At any time during the action phase, you may exhaust this card to perform a transaction with another player. As part of that transaction, you may ready that player's agent.",
    homebrewReplacesID: "tcs",
    imageURL: "/hover_images/techs/absol/TemporalCommandSuite.png",
  },
  {
    alias: "absol_td",
    name: "Transit Diodes",
    types: ["CYBERNETIC"],
    requirements: "YY",
    source: "absol",
    text: "You may exhaust this card at the start of your turn during the action phase; remove up to 4 of your ground forces from the game board and place them on 1 or more planets you control.\nYou may exhaust this card at the start of a round of ground combat on a planet you control. If you do, at the end of that combat round, you may remove all of your ground forces from that planet and place them on another planet you control.",
    homebrewReplacesID: "td",
    imageURL: "/hover_images/techs/absol/generic/TransitDiodes.png",
  },
  {
    alias: "absol_techquantumcore",
    name: "Quantumcore",
    types: ["CYBERNETIC", "WARFARE"],
    source: "absol",
    text: "During the movement step of your tactical actions, you may move 1 of your structures from a system that does not contain 1 of your command tokens to a planet you control in the active system.\nWhen an opponent commits ground forces to a planet that contains 1 or more of your structures, you may destroy 1 structure on that planet to return the committed units to the space area.  If you do, purge this card",
    imageURL: "/hover_images/techs/absol/generic/QuantumCore.png",
  },
  {
    alias: "absol_tp",
    name: "Transparasteel Plating",
    types: ["BIOTIC"],
    requirements: "G",
    faction: "yssaril",
    source: "absol",
    text: "During your turn of the action phase, players that have passed cannot play action cards.\nAt the start of a round of ground combat, you may discard 1 action card to apply -1 to the results of your opponent's die rolls during that combat round.",
    homebrewReplacesID: "tp",
    imageURL: "/hover_images/techs/absol/faction/TransparasteelPlating.png",
  },
  {
    alias: "absol_vpw",
    name: "Valkyrie Particle Weave",
    types: ["WARFARE"],
    requirements: "RR",
    faction: "sardakk",
    source: "absol",
    text: "When you commit ground forces to a planet from 1 or more other planets, you may commit 2 ground forces from each planet.\nAfter making combat rolls during a round of ground combat, if your opponent produced 1 or more hits, you produce 1 additional hit.",
    homebrewReplacesID: "vpw",
    imageURL: "/hover_images/techs/absol/faction/ValkyrieParticleWeave.png",
  },
  {
    alias: "absol_vtx",
    name: "Vortex",
    types: ["WARFARE"],
    requirements: "R",
    faction: "cabal",
    source: "absol",
    text: "ACTION: Exhaust this card to choose another player's non-structure unit in a system that is adjacent to 1 or more of your space docks. Capture 1 unit of that type from that player's reinforcements.",
    homebrewReplacesID: "vtx",
    imageURL: "/hover_images/techs/faction/vortex.jpg",
  },
  {
    alias: "absol_vw",
    name: "Voidwatch",
    types: ["BIOTIC"],
    requirements: "G",
    faction: "empyrean",
    source: "absol",
    text: "At any time, you may look at the top card of the frontier exploration deck. After you do, you may exhaust this card to shuffle the frontier exploration deck.\nAfter a player moves ships into a system that contains 1 or more of your units, they must give you 1 promissory note from their hand, if able.",
    homebrewReplacesID: "vw",
    imageURL: "/hover_images/techs/absol/Voidwatch.png",
  },
  {
    alias: "absol_wg",
    name: "Wormhole Generator",
    types: ["PROPULSION"],
    requirements: "BB",
    faction: "ghost",
    source: "absol",
    text: "ACTION: Exhaust this card to place or move up to 2 Creuss wormhole tokens into systems that contain a planet you control and/or non-home systems that do not contain another player's ships.",
    homebrewReplacesID: "wg",
    imageURL: "/hover_images/techs/absol/WormholeGenerator.png",
  },
  {
    alias: "absol_ws",
    name: "War Sun",
    types: ["UNITUPGRADE"],
    requirements: "YRRR",
    source: "absol",
    text: "Cost 10, Combat 3(x3), Move 2, Capacity 6\nSUSTAIN DAMAGE, BOMBARDMENT 3(x3)\nOther player's units in this system lose PLANETARY SHIELD.",
    homebrewReplacesID: "ws",
    imageURL: "/hover_images/techs/absol/generic/WarSun.png",
  },
  {
    alias: "absol_x89",
    name: "X-89 Bacterial Weapon",
    types: ["BIOTIC"],
    requirements: "GGG",
    source: "absol",
    text: "When your units use BOMBARDMENT, the first hit may be used to produce and assign 1 hit against each ground force on the planet.\nAt the start of a round of ground combat on a planet you control, you may exhaust this card to destroy all ground forces on the planet.",
    homebrewReplacesID: "x89",
    imageURL: "/hover_images/techs/absol/generic/X89BacterialWeapons.png",
  },
  {
    alias: "absol_yso",
    name: "Yin Spinner",
    types: ["BIOTIC"],
    requirements: "GG",
    faction: "yin",
    source: "absol",
    text: "After you produce units, place up to 2 infantry from your reinforcements on any planet you control or in any space area that contains 1 or more of your ships.",
    homebrewReplacesID: "yso",
    imageURL: "/hover_images/techs/faction/yin_spinner_omega.jpg",
  },
  {
    alias: "ac2",
    name: "Advanced Carrier II",
    types: ["UNITUPGRADE"],
    requirements: "BB",
    faction: "sol",
    baseUpgrade: "cv2",
    source: "base",
    text: "Cost 3, Combat 9, Move 2, Capacity 8\nSUSTAIN DAMAGE",
    imageURL: "/hover_images/techs/faction/advanced_carrier_2.jpg",
  },
  {
    alias: "adminssd",
    name: "Tunnel Network II",
    types: ["UNITUPGRADE"],
    requirements: "YY",
    faction: "admins",
    baseUpgrade: "sd2",
    source: "admins",
    text: "PRODUCTION 7, SPACE CANNON 7\nYou may use this unit's SPACE CANNON against ships that are adjacent to this unit's system.\nUp to 4 fighters in this system do not count towards your ships' capacity.",
  },
  {
    alias: "adminsy",
    name: "Molecular Transference",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "admins",
    source: "admins",
    text: "Systems that contain a planet with one of your Space Docks on it are considered adjacent to each other for all game effects.",
  },
  {
    alias: "ah",
    name: "Aerie Hololattice",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "argent",
    source: "pok",
    text: "Other players cannot move ships through systems that contain your structures.\nEach planet that contains 1 or more of your structures gains the PRODUCTION 1 ability as if it were a unit.",
    shortName: "Aerie Holo\n-lattice",
    imageURL: "/hover_images/techs/faction/aerie_hololattice.jpg",
  },
  {
    alias: "aida",
    name: "AI Development Algorithm",
    types: ["WARFARE"],
    source: "pok",
    text: "When you research a unit upgrade technology, you may exhaust this card to ignore any 1 prerequisite.\nWhen 1 or more of your units use PRODUCTION, you may exhaust this card to reduce the combined cost of the produced units by the number of unit upgrade technologies that you own.",
    imageURL: "/hover_images/techs/generic/ai_development_algorithm.png",
    initials: "AI",
    shortName: "AI Develop. Algorithm",
  },
  {
    alias: "amd",
    name: "Antimass Deflectors",
    types: ["PROPULSION"],
    source: "base",
    text: "Your ships can move into and through asteroid fields.\nWhen other players' units use SPACE CANNON against your units, apply -1 to the result of each die roll.",
    imageURL: "/hover_images/techs/generic/antimass_deflectors.png",
    initials: "AMD",
    shortName: "Antimass Deflect'rs",
  },
  {
    alias: "antimatter",
    name: "Antimatter",
    types: ["NONE"],
    source: "twilights_fall",
    text: "After you perform a tactical action in a system that contains a frontier token, if you have 1 or more ships in that system, explore that token.\nYour ships can retreat into adjacent systems that do not contain other players' units, even if you do not have units or control planets in that system.",
  },
  {
    alias: "araracg",
    name: "Void Weave Implants",
    types: ["BIOTIC"],
    requirements: "GGG",
    faction: "arachian",
    source: "other",
    text: "ACTION: Exhaust this card to perform a tactical action in a non-home system that contains your units and one of your Command Tokens. ",
  },
  {
    alias: "araracy",
    name: "Weave Spinner",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "arachian",
    source: "other",
    text: "At the end of your turn, you may exhaust this card to place a Command Token from your reinforcement in up to 2 systems that are adjacent to a system that contains one of your command tokens. ",
  },
  {
    alias: "as",
    name: "Aetherstream",
    types: ["PROPULSION"],
    requirements: "BB",
    faction: "empyrean",
    source: "pok",
    text: "After you or one of your neighbors activates a system that is adjacent to an anomaly, you may apply +1 to the move value of all of that player's ships during this tactical action.",
    shortName: "Aether-\nstream",
    imageURL: "/hover_images/techs/faction/aetherstream.jpg",
  },
  {
    alias: "asc",
    name: "Assault Cannon",
    types: ["WARFARE"],
    requirements: "RRR",
    source: "base",
    text: "At the start of a space combat in a system that contains 3 or more of your non-fighter ships, your opponent must destroy 1 of their non-fighter ships.",
    imageURL: "/hover_images/techs/generic/assault_cannon.png",
    initials: "AC",
  },
  {
    alias: "asn",
    name: "Agency Supply Network",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "keleres",
    source: "pok",
    text: "Whenever you resolve one of your PRODUCTION abilities, you may resolve an additional one of your PRODUCTION abilities in any system; the additional use does not trigger this ability.",
    shortName: "Agency\nSupply Net.",
    shrinkName: false,
    imageURL: "/hover_images/techs/faction/agency_supply_network.jpg",
  },
  {
    alias: "aug",
    name: "Augmentation",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "terminator",
    source: "pbd2000",
    text: "Secret",
  },
  {
    alias: "baarvag",
    name: "Unyielding Will",
    types: ["BIOTIC"],
    requirements: "GG",
    faction: "arvaxi",
    source: "balacasi",
    text: "Your action cards cannot be canceled. ACTION: Exhaust this card and discard 1 action card to draw 1 action card.",
  },
  {
    alias: "baarvay",
    name: "Wartime Innovation",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "arvaxi",
    source: "balacasi",
    text: "At the start of a combat, you may spend 4 influence to research a unit upgrade technology of the same type as 1 of your units that is participating in this combat.",
  },
  {
    alias: "baconcg",
    name: "Strategic Inversion",
    types: ["BIOTIC"],
    requirements: "G",
    faction: "onyxxa",
    source: "balacasi",
    text: "When you perform a strategic action, you may gain 1 command token instead of resolving any ability. While you do not have any control tokens on your commander, apply +1 to your units' combat rolls.",
  },
  {
    alias: "baconcy",
    name: "Sacrificial Command",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "onyxxa",
    source: "balacasi",
    text: "Once per action, when you would spend a token from your strategy pool to resolve the primary or secondary ability of a strategy card, you may destroy 1 of your non-fighter ships in or adjacent to a system that contains the active player's units. If you do, spend that token from your reinforcements instead.",
  },
  {
    alias: "bakalodn",
    name: "Stinger II",
    types: ["UNITUPGRADE"],
    requirements: "BBY",
    faction: "kalora",
    baseUpgrade: "dn2",
    source: "balacasi",
    text: 'This unit cannot be destroyed by "Direct Hit" action cards. When this unit makes a BOMBARDMENT roll against another player’s units, if it produces 1 or more hits, you may gain 1 trade good. SUSTAIN DAMAGE BOMBARDMENT 5 Cost 4 Combat 5 Move 2 Capacity 1',
  },
  {
    alias: "bakalor",
    name: "Chitin Shielding",
    types: ["WARFARE"],
    requirements: "RR",
    faction: "kalora",
    source: "balacasi",
    text: "After you win a space combat in a non-home system, you may exhaust this card to place 1 command token from your opponent’s reinforcements into the active system.",
  },
  {
    alias: "baldrick_amd",
    name: "Antimass Deflectors",
    types: ["PROPULSION"],
    source: "ignis_aurora",
    text: "Your ships can move into and through asteroid fields. When other players' units use SPACE CANNON against your units, apply -2 to the result of each die roll.",
    homebrewReplacesID: "amd",
  },
  {
    alias: "baldrick_bio",
    name: "Bioplasmosis",
    types: ["BIOTIC"],
    requirements: "G",
    faction: "arborec",
    source: "ignis_aurora",
    text: "ACTION: Exhaust this card to place 1 infantry on up to 3 planets that you control.\nAt the start of the action phase, you may choose 1 system. Resolve each of your unit’s Production abilities in that system.",
    homebrewReplacesID: "bio",
  },
  {
    alias: "baldrick_bs",
    name: "Bio-Stims",
    types: ["BIOTIC"],
    source: "ignis_aurora",
    text: "You may exhaust this card at the end of your turn to ready 1 of your planets that has a technology specialty or 1 of your other technologies.",
    homebrewReplacesID: "bs",
  },
  {
    alias: "baldrick_cl2",
    name: "Crimson Legionnaire II",
    types: ["UNITUPGRADE"],
    requirements: "G",
    faction: "mahact",
    baseUpgrade: "inf2",
    source: "ignis_aurora",
    text: "Cost 1(x2), Combat 7\nAfter this unit is destroyed, gain 1 commodity or convert 1 of your commodities to a trade good.\nYou may produce 1 additional infantry for their cost. These infantry do not count against your PRODUCTION limit.",
    homebrewReplacesID: "cl2",
  },
  {
    alias: "baldrick_dxa",
    name: "Dacxive Animators",
    types: ["BIOTIC"],
    requirements: "G",
    source: "ignis_aurora",
    text: "At the start of a round of ground combat, you may exhaust this card. If you do, for each infantry destroyed in that combat round place an infantry token on this card. At the end of the combat round, place 1 infantry from your reinforcements on the planet for each token on this card.",
    homebrewReplacesID: "dxa",
  },
  {
    alias: "baldrick_ers",
    name: "E-Res Siphons",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "jolnar",
    source: "ignis_aurora",
    text: "After another player activates a system that contains 1 or more of your ships, gain 3 trade goods.",
    homebrewReplacesID: "ers",
  },
  {
    alias: "baldrick_fl",
    name: "Fleet Logistics",
    types: ["CYBERNETIC"],
    requirements: "YY",
    source: "ignis_aurora",
    text: "During each of your turns of the action phase, you may perform 2 actions instead of 1.",
    homebrewReplacesID: "fl",
  },
  {
    alias: "baldrick_gd",
    name: "Gravity Drive",
    types: ["PROPULSION"],
    requirements: "B",
    source: "ignis_aurora",
    text: "After you activate a system, you may exhaust this card to apply +1 to the move value of your ships during this tactical action.",
    homebrewReplacesID: "gd",
  },
  {
    alias: "baldrick_hm",
    name: "Hyper Metabolism",
    types: ["BIOTIC"],
    requirements: "G",
    source: "ignis_aurora",
    text: "You may exhaust this card at the end of your turn to gain one command token.",
    homebrewReplacesID: "hm",
  },
  {
    alias: "baldrick_htp",
    name: "Hegemonic Trade Policy",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "winnu",
    source: "ignis_aurora",
    text: "Exhaust this card when 1 or more of your units use PRODUCTION; swap the resource and influence values of 1 planet you control until the end of your turn.",
    homebrewReplacesID: "htp",
  },
  {
    alias: "baldrick_ic",
    name: "Impulse Core",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "yin",
    source: "ignis_aurora",
    text: "When you or your opponent resolve a retreat, you may produce a hit and assign it to one of your opponent's ships in the active system.",
    homebrewReplacesID: "ic",
  },
  {
    alias: "baldrick_inf2",
    name: "Infantry II",
    types: ["UNITUPGRADE"],
    requirements: "G",
    source: "ignis_aurora",
    text: "Cost 1(x2), Combat 7\nYou may produce 1 additional infantry for their cost. These infantry do not count against your PRODUCTION limit.",
    homebrewReplacesID: "inf2",
  },
  {
    alias: "baldrick_is",
    name: "Inheritance Systems",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "l1z1x",
    source: "ignis_aurora",
    text: "ACTION: Exhaust this card and spend 6 resources to research a technology.",
    homebrewReplacesID: "is",
  },
  {
    alias: "baldrick_l4",
    name: "L4 Disruptors",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "letnev",
    source: "ignis_aurora",
    text: "At the start of a space combat you may exhaust this card. If you do, the other player cannot retreat or resolve abilities that would move their ships during this combat.",
    homebrewReplacesID: "l4",
  },
  {
    alias: "baldrick_lgf",
    name: "Lazax Gate Folding",
    types: ["PROPULSION"],
    requirements: "B",
    faction: "winnu",
    source: "ignis_aurora",
    text: "During your tactical actions, treat the Mecatol Rex system as if it contains each non-delta wormhole.",
    homebrewReplacesID: "lgf",
  },
  {
    alias: "baldrick_lwd",
    name: "Light/Wave Deflector",
    types: ["PROPULSION"],
    requirements: "BBB",
    source: "ignis_aurora",
    text: "After you activate a system, you may exhaust this card to allow your ships to move through systems that contain other players' ship during this tactical action.",
    homebrewReplacesID: "lwd",
  },
  {
    alias: "baldrick_nm",
    name: "Neural Motivator",
    types: ["BIOTIC"],
    source: "ignis_aurora",
    text: "During your turn, you may exhaust this card to draw one action card.",
    homebrewReplacesID: "nm",
  },
  {
    alias: "baldrick_pi",
    name: "Predictive Intelligence",
    types: ["CYBERNETIC"],
    requirements: "Y",
    source: "ignis_aurora",
    text: "At the end of your turn, you may exhaust this card to redistribute your command tokens.\nWhen you cast votes during the agenda phase, you may cast 3 additional votes.",
    homebrewReplacesID: "pi",
  },
  {
    alias: "baldrick_ps",
    name: "Plasma Scoring",
    types: ["WARFARE"],
    source: "ignis_aurora",
    text: "When 1 or more of your units use BOMBARDMENT, ANTI-FIGHTER BARRAGE, or SPACE CANNON, 1 of those units may roll 1 additional die.",
    homebrewReplacesID: "ps",
  },
  {
    alias: "baldrick_pws2",
    name: "Prototype War Sun II",
    types: ["UNITUPGRADE"],
    requirements: "RRRY",
    faction: "muaat",
    baseUpgrade: "ws",
    source: "ignis_aurora",
    text: 'Cost 10, Combat 3(x3), Move 2, Capacity 8\nSUSTAIN DAMAGE, BOMBARDMENT 3(x3)\nOther players\' units in this system lose PLANETARY SHIELD.\nThis unit cannot be destroyed by "Direct Hit" action cards.',
    homebrewReplacesID: "pws2",
  },
  {
    alias: "baldrick_scc",
    name: "Spatial Conduit Cylinders",
    types: ["PROPULSION"],
    requirements: "BB",
    faction: "jolnar",
    source: "ignis_aurora",
    text: "You may treat all systems that contain your structures as adjacent.",
  },
  {
    alias: "baldrick_sd2",
    name: "Space Dock II",
    types: ["UNITUPGRADE"],
    requirements: "YY",
    source: "ignis_aurora",
    text: "PRODUCTION X\nThis unit's PRODUCTION value is equal to 5 more than the resource or influence value of this planet.\nUp to 6 fighters in this system do not count against your ships' capacity.",
    homebrewReplacesID: "sd2",
  },
  {
    alias: "baldrick_so2",
    name: "Spec Ops II",
    types: ["UNITUPGRADE"],
    requirements: "G",
    faction: "sol",
    baseUpgrade: "inf2",
    source: "ignis_aurora",
    text: "You may produce 1 additional infantry for their cost. These infantry do not count against your PRODUCTION limit.",
    homebrewReplacesID: "so2",
  },
  {
    alias: "baldrick_td",
    name: "Transit Diodes",
    types: ["CYBERNETIC"],
    requirements: "Y",
    source: "ignis_aurora",
    text: "You may exhaust this card at the start of your turn during the action phase; remove up to 4 of your ground forces from the game board and place them on 1 or more planets you control.",
    homebrewReplacesID: "td",
  },
  {
    alias: "baldrick_ws",
    name: "War Sun",
    types: ["UNITUPGRADE"],
    requirements: "RRRY",
    source: "ignis_aurora",
    text: "Cost 10, Combat 3(x3), Move 2, Capacity 8\nSUSTAIN DAMAGE, BOMBARDMENT 3(x3)\nOther players' units in this system lose PLANETARY SHIELD.",
    homebrewReplacesID: "ws",
  },
  {
    alias: "baldrick_x89",
    name: "X-89 Bacterial Weapon",
    types: ["BIOTIC"],
    requirements: "GGG",
    source: "ignis_aurora",
    text: "After the space cannon defense step of a planetary invasion, you may exhaust this card to destroy all of your opponent's infantry on that planet. Then, roll 1 die for each of your infantry on that planet. For each result of 6 or greater, destroy 1 of those units.",
    homebrewReplacesID: "x89",
  },
  {
    alias: "balunacv",
    name: "Cryptic Hauler II",
    types: ["UNITUPGRADE"],
    requirements: "BB",
    faction: "lunarium",
    baseUpgrade: "cv2",
    source: "balacasi",
    text: "After you activate this system, you may place 1 fighter in this system. This unit has capacity equal to the number of your scored secret objectives plus 6. Cost 3 Combat 9 Move 2 Capacity X",
  },
  {
    alias: "balunag",
    name: "Subliminal Intuition",
    types: ["BIOTIC"],
    requirements: "GG",
    faction: "lunarium",
    source: "balacasi",
    text: "When another player activates a system that contains 1 or more of your ships, you may draw 1 secret objective or gain 2 command tokens. If you gained command tokens, exhaust this card.",
  },
  {
    alias: "batyriinf",
    name: "Phase Trooper II",
    types: ["UNITUPGRADE"],
    requirements: "GG",
    faction: "tyris",
    baseUpgrade: "inf2",
    source: "balacasi",
    text: "This unit counts as 1/2 capacity if it is being transported or if it is in a space area with 1 or more of your ships that have capacity values. After this unit is destroyed, roll 1 die. If the result is 6 or greater, place the unit on this card. At the start of your next turn, place each unit that is on this card on a planet you control, or in the space area of a system that contains 1 or more of your ships. Combat 7",
  },
  {
    alias: "batyriy",
    name: "Temporal Displacement",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "tyris",
    source: "balacasi",
    text: "You may exhaust this card at the start of your turn during the action phase; remove up to 5 of your fighters from the game board and place them in 1 or more systems that contain your ships.",
  },
  {
    alias: "bavyseb",
    name: "Conduit Matrix",
    types: ["PROPULSION"],
    requirements: "BBB",
    faction: "vyserix",
    source: "balacasi",
    text: "During your turn, you consider all systems that contain 1 or more planets with a technology specialty as adjacent to each other.",
  },
  {
    alias: "bavysepds",
    name: "Harmonic Bastion II",
    types: ["UNITUPGRADE"],
    requirements: "RY",
    faction: "vyserix",
    baseUpgrade: "pds2",
    source: "balacasi",
    text: "SPACE CANNON: 5 ANTI-FIGHTER BARRAGE: 7** PLANETARY SHIELD You may use this unit's SPACE CANNON against ships that are adjacent to this unit's system. This unit cannot lose its PLANETARY SHIELD ability.",
  },
  {
    alias: "baxansd",
    name: "Galactic Citadel II",
    types: ["UNITUPGRADE"],
    requirements: "YY",
    faction: "xan",
    baseUpgrade: "sd2",
    source: "balacasi",
    text: "Up to 3 non-fighter ships in this system do not count against your fleet limit. This unit’s PRODUCTION value is equal to 4 more than the resource value of this planet.Up to 3 fighters in this system do not count against your ships' capacity",
  },
  {
    alias: "baxanws",
    name: "Stellar Architect II ",
    types: ["UNITUPGRADE"],
    requirements: "RRRY",
    faction: "xan",
    baseUpgrade: "ws",
    source: "balacasi",
    text: "During space combat, this unit rolls 1 additional die for each space dock in this system. Other players' units in this system lose PLANETARY SHIELD. SUSTAIN DAMAGE BOMBARDMENT 3 (3) Cost 12 Combat 3*** Move 2 Capacity 8",
  },
  {
    alias: "bazephcr",
    name: "Zephyr Hound II",
    types: ["UNITUPGRADE"],
    requirements: "RYG",
    faction: "zephyrion",
    baseUpgrade: "cr2",
    source: "balacasi",
    text: "SPACE CANNON 5 Cost 2 Combat 6 Move 3 Capacity 1",
  },
  {
    alias: "bazephy",
    name: "Hunting Grounds",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "zephyrion",
    source: "balacasi",
    text: "ACTION: Exhaust this card to place or move any number of bounty tokens from other players' faction sheets to unit spaces of non-fighter ships on other players' faction sheets. During space combat, your opponent cannot announce retreat unless you allow it.",
  },
  {
    alias: "bio",
    name: "Bioplasmosis",
    types: ["BIOTIC"],
    requirements: "GG",
    faction: "arborec",
    source: "base",
    text: "At the end of the status phase, you may remove any number of infantry from planets you control and place them on 1 or more planets you control in the same or adjacent systems.",
    shortName: "Bio-\nplasmosis",
    imageURL: "/hover_images/techs/faction/bioplasmosis.jpg",
  },
  {
    alias: "bs",
    name: "Bio-Stims",
    types: ["BIOTIC"],
    requirements: "G",
    source: "pok",
    text: "You may exhaust this card at the end of your turn to ready 1 of your planets that has a technology specialty or 1 of your other technologies.",
    imageURL: "/hover_images/techs/generic/biostims.png",
    initials: "BS",
  },
  {
    alias: "cantor",
    name: "Syoblade",
    types: ["WARFARE"],
    requirements: "RR",
    faction: "canto",
    source: "eronous",
    text: "During the first round of ground combat, apply +2 to the result of each of your unit's combat rolls.",
  },
  {
    alias: "cantoy",
    name: "Integrated Systems",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "canto",
    source: "eronous",
    text: "After 1 of your opponents units is destroyed, you may exhaust this card to gain the unit upgrade technology of that unit type. During a space combat, after 1 of your opponents units are destroyed. Exhaust this card to gain the unit upgrade technology of that unit type.",
  },
  {
    alias: "ciqineqreceptors",
    name: "Ciqineq Receptors",
    types: ["BIOTIC"],
    requirements: "GG",
    faction: "qulane",
    source: "ignis_aurora",
    text: "ACTION: Exhaust this card to draw 2 event cards. Place 1 into your hand and shuffle the other back into the deck.",
  },
  {
    alias: "cl2",
    name: "Crimson Legionnaire II",
    types: ["UNITUPGRADE"],
    requirements: "GG",
    faction: "mahact",
    baseUpgrade: "inf2",
    source: "pok",
    text: "Cost 1(x2), Combat 7\nAfter this unit is destroyed, gain 1 commodity or convert 1 of your commodities to a trade good. Then, place the unit on this card. At the start of your next turn, place each unit that is on this card on a planet you control in your home system.",
    imageURL: "/hover_images/techs/faction/crimson_legionnaire_2.jpg",
  },
  {
    alias: "clmu",
    name: "Cluster Munitions",
    types: ["WARFARE"],
    requirements: "RRR",
    faction: "terminator",
    source: "pbd2000",
    text: "Secret",
  },
  {
    alias: "cm",
    name: "Chaos Mapping",
    types: ["PROPULSION"],
    requirements: "B",
    faction: "saar",
    source: "base",
    text: "Other players cannot activate asteroid fields that contain 1 or more of your ships.\nAt the start of your turn during the action phase, you may produce 1 unit in a system that contains at least 1 of your units that has PRODUCTION.",
    imageURL: "/hover_images/techs/faction/chaos_mapping.jpg",
  },
  {
    alias: "cop",
    name: "Coproduction",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "syndicate",
    source: "pbd2000",
    text: "Secret",
  },
  {
    alias: "cr2",
    name: "Cruiser II",
    types: ["UNITUPGRADE"],
    requirements: "GYR",
    source: "base",
    text: "Cost 2, Combat 6, Move 3, Capacity 1",
    imageURL: "/hover_images/techs/generic/cruiser_2.jpg",
  },
  {
    alias: "cryypter_pi",
    name: "Predictive Intelligence",
    types: ["CYBERNETIC"],
    requirements: "Y",
    source: "voices_of_the_council",
    text: "During the action phase, you may exhaust this card to redistribute your command tokens.\nWhen you predict or cast votes during the agenda phase, you may exhaust this card to cast 4 additional votes for that outcome (even if a game effect would prevent you from voting); if that outcome is resolved, ready this card.",
    homebrewReplacesID: "pi",
  },
  {
    alias: "cv2",
    name: "Carrier II",
    types: ["UNITUPGRADE"],
    requirements: "BB",
    source: "base",
    text: "Cost 3, Combat 9, Move 2, Capacity 6",
    imageURL: "/hover_images/techs/generic/carrier_2.jpg",
  },
  {
    alias: "da",
    name: "Duranium Armor",
    types: ["WARFARE"],
    requirements: "RR",
    source: "base",
    text: "During each combat round, after you assign hits to your units, repair 1 of your damaged units that did not use SUSTAIN DAMAGE during this combat round. [Note: this only occurs after the normal assign hits step in a combat round, which occurs after both sides roll combat dice. It occurs even if the opponent rolled no hits.]",
    imageURL: "/hover_images/techs/generic/duranium_armor.png",
    initials: "DA",
  },
  {
    alias: "dd2",
    name: "Destroyer II",
    types: ["UNITUPGRADE"],
    requirements: "RR",
    source: "base",
    text: "Cost 1, Combat 8, Move 2\nANTI-FIGHTER BARRAGE 6(x3)",
    imageURL: "/hover_images/techs/generic/destroyer_2.jpg",
  },
  {
    alias: "det",
    name: "Dark Energy Tap",
    types: ["PROPULSION"],
    source: "pok",
    text: "After you perform a tactical action in a system that contains a frontier token, if you have 1 or more ships in that system, explore that token.\nYour ships can retreat into adjacent systems that do not contain other players' units, even if you do not have units or control planets in that system.",
    imageURL: "/hover_images/techs/generic/dark_energy_tap.png",
    initials: "DET",
    shortName: "Dark N.R.G. Tap",
  },
  {
    alias: "diverse_strategems",
    name: "Diverse Strategems",
    types: ["WARFARE"],
    requirements: "RRR",
    source: "cpti",
    text: "Before rolling dice during combat, you may exhaust this card to choose 1 of your units of each type. Those units roll an additional die.\n\nIf you have no units of a type left in your reinforcements, apply -1 to the combat value of each of your units of that type.",
  },
  {
    alias: "dn2",
    name: "Dreadnought II",
    types: ["UNITUPGRADE"],
    requirements: "BBY",
    source: "base",
    text: 'Cost 4, Combat 5, Move 2, Capacity 1\nSUSTAIN DAMAGE, BOMBARDMENT 5\nThis unit cannot be destroyed by "Direct Hit" action cards.',
    imageURL: "/hover_images/techs/generic/dreadnought_2.jpg",
  },
  {
    alias: "doba",
    name: "Double Barrels",
    types: ["WARFARE"],
    requirements: "RR",
    faction: "enclave",
    source: "pbd2000",
    text: "Secret",
  },
  {
    alias: "ds",
    name: "Dimensional Splicer",
    types: ["WARFARE"],
    requirements: "R",
    faction: "ghost",
    source: "base",
    text: "At the start of space combat in a system that contains a wormhole and 1 or more of your ships, you may produce 1 hit and assign it to 1 of your opponent's ships.",
    shrinkName: true,
    imageURL: "/hover_images/techs/faction/dimensional_splicer.jpg",
  },
  {
    alias: "dsatokcr",
    name: "Mirrorshard II",
    types: ["UNITUPGRADE"],
    requirements: "GRY",
    faction: "atokera",
    baseUpgrade: "cr2",
    source: "ds",
    text: "Cost 2, Combat 6, Move 3, Capacity 1, Production 1\nDEPLOY: During the agenda phase, after an outcome you voted for or predicted is resolved, place up to 1 cruiser in a system that contains your ships. ",
  },
  {
    alias: "dsatoky",
    name: "Modular Designs",
    types: ["CYBERNETIC"],
    requirements: "YYY",
    faction: "atokera",
    source: "ds",
    text: "Once during each of your actions, choose 1 of your units and 1 unit ability. That unit gains that unit ability with a value of 3, if applicable, until the end of that action. [Note: not currently automated by the bot]",
  },
  {
    alias: "dsaugug",
    name: "Psychographics",
    types: ["BIOTIC"],
    requirements: "GGG",
    faction: "augers",
    source: "ds",
    text: "During the status phase, you may score 1 additional public objective instead of a secret objective.",
    shortName: "Psycho-\ngraphics",
    imageURL: "/hover_images/techs/ds/psychographics.jpg",
  },
  {
    alias: "dsauguy",
    name: "Sentient Datapool",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "augers",
    source: "ds",
    text: "At the start of the status phase, you may spend 3 trade goods to research 1 technology.",
    imageURL: "/hover_images/techs/ds/sentient_datapool.jpg",
  },
  {
    alias: "dsaxisb",
    name: "Rift Engines",
    types: ["PROPULSION"],
    requirements: "B",
    faction: "axis",
    source: "ds",
    text: "After you activate a system, you may exhaust this card to choose 1 ship you control and roll a die. On a result of 1-3, remove that ship from the game board; on a result of 4-10, apply +2 to that ship's move value during this tactical action.",
    imageURL: "/hover_images/techs/ds/rift_engines.png",
  },
  {
    alias: "dsaxisy",
    name: "Emergency Deployment",
    types: ["CYBERNETIC"],
    requirements: "YYY",
    faction: "axis",
    source: "ds",
    text: "ACTION: Exhaust this card to place or move 1 of your space docks onto a planet you control.",
    shrinkName: true,
    imageURL: "/hover_images/techs/ds/emergency_deployment.jpg",
  },
  {
    alias: "dsbelkff",
    name: "Wyvern II",
    types: ["UNITUPGRADE"],
    requirements: "GB",
    faction: "belkosea",
    baseUpgrade: "ff2",
    source: "ds",
    text: "You may commit this unit to a planet as if it were a ground force. When combat ends, return it to the space area. Fighters in excess of your ships’ capacity count against your fleet pool. 1(x2), 8, 2; ANTI-FIGHTER BARRAGE 9(x2)",
  },
  {
    alias: "dsbelky",
    name: "Synchrony Matrix",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "belkosea",
    source: "ds",
    text: "At the end of your turn, you may exhaust this card to ready 1 agent or ability card in your play area.",
  },
  {
    alias: "dsbentg",
    name: "Broker Network",
    types: ["BIOTIC"],
    requirements: "G",
    faction: "bentor",
    source: "ds",
    text: "Each planet you control that has a technology specialty or an attachment gains the PRODUCTION 1 ability as if it were a unit.",
    imageURL: "/hover_images/techs/ds/broker_network.png",
  },
  {
    alias: "dsbenty",
    name: "Merged Replicators",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "bentor",
    source: "ds",
    text: "When 1 or more of your units use PRODUCTION, you may exhaust this card to increase the PRODUCTION value of 1 of those units to match the PRODUCTION value of the unit on the game board with the highest PRODUCTION value, or apply +2 to the total PRODUCTION value of those units.",
    shrinkName: true,
    imageURL: "/hover_images/techs/ds/merged_replicators.png",
  },
  {
    alias: "dsceldr",
    name: "Emergency Mobilization",
    types: ["WARFARE"],
    requirements: "RR",
    faction: "celdauri",
    source: "ds",
    text: "At the end of your turn, you may exhaust this card and spend 1 token from your strategy pool to remove a command token from a system that contains 1 or more of your space docks.",
    shortName: "Emergency Mobilization",
    shrinkName: true,
    imageURL: "/hover_images/techs/ds/emergency_mobilization.png",
  },
  {
    alias: "dsceldsd",
    name: "Trade Port II",
    types: ["UNITUPGRADE"],
    requirements: "YY",
    faction: "celdauri",
    baseUpgrade: "sd2",
    source: "ds",
    text: "PRODUCTION X, ANTI-FIGHTER BARRAGE 6(x2).\nThis unit's PRODUCTION value is equal to 4 more than the resource or influence value of this planet.\nUp to 3 fighters in this system do not count against your ships' capacity.\nThis unit may use its ANTI-FIGHTER BARRAGE during each round of space combat.",
    imageURL: "/hover_images/techs/ds/trade_port_2.jpg",
  },
  {
    alias: "dscheidn",
    name: "Chitin Hulk II",
    types: ["UNITUPGRADE"],
    requirements: "BBY",
    faction: "cheiran",
    baseUpgrade: "dn2",
    source: "ds",
    text: "Cost 4, Combat 5, Move 2, Capacity 1\nSUSTAIN DAMAGE, BOMBARDMENT 5\nThis unit cannot be destroyed by “Direct Hit” action cards.\nWhen this unit is destroyed, you may place 1 fighter or 1 destroyer from your reinforcements in this system's space area.",
    imageURL: "/hover_images/techs/ds/chitin_hulk_2.png",
  },
  {
    alias: "dscheir",
    name: "Brood Pod",
    types: ["WARFARE"],
    requirements: "RR",
    faction: "cheiran",
    source: "ds",
    text: "After another player activates a system that contains 1 of your structures, you may exhaust this card to produce 1 ship in the active system. For each of your structures in the active system, you may remove that unit from the game board to reduce the cost of the produced ship by 3.",
    imageURL: "/hover_images/techs/ds/brood_pod.png",
  },
  {
    alias: "dscymiinf",
    name: "Unholy Abomination II",
    types: ["UNITUPGRADE"],
    requirements: "GG",
    faction: "cymiae",
    baseUpgrade: "inf2",
    source: "ds",
    text: "Cost 1, Combat 4\nAfter this unit is destroyed, roll 1 die. If the result is 6 or greater, place the unit on this card. At the start of your turn, place each unit that is on this card on a planet you control, if able.",
    imageURL: "/hover_images/techs/ds/unholy_abomination_2.jpg",
  },
  {
    alias: "dscymiy",
    name: "Recursive Worm",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "cymiae",
    source: "ds",
    text: "At the start of your turn, you may exhaust this card to choose 1 non-home system other than Mecatol Rex that contains 1 or more of your units; place 1 other player's command token from their reinforcements in that system.",
    imageURL: "/hover_images/techs/ds/recursive_worm.png",
  },
  {
    alias: "dsdihmdn",
    name: "Aegis II",
    types: ["UNITUPGRADE"],
    requirements: "BBY",
    faction: "dihmohn",
    baseUpgrade: "dn2",
    source: "ds",
    text: "Cost 4, Combat 4, Move 2, Capacity 1\nSUSTAIN DAMAGE, BOMBARDMENT 5\nWhen another ship in this system would be destroyed by a game effect, you may have this ship become damaged instead.",
    imageURL: "/hover_images/techs/ds/aegis_2.jpg",
  },
  {
    alias: "dsdihmy",
    name: "Impressment Programs",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "dihmohn",
    source: "ds",
    text: "When you explore a planet, you may produce 1 ship in that planet's system.\nACTION: Exhaust this card and 1 planet you control to explore that planet.\n[Note: this cannot produce ships if enemy ships are present]",
    shortName: "Impress.\nPrograms",
    shrinkName: false,
    imageURL: "/hover_images/techs/ds/impressment_programs.jpg",
  },
  {
    alias: "dsedyng",
    name: "Unity Algorithm",
    types: ["BIOTIC"],
    requirements: "GGG",
    faction: "edyn",
    source: "ds",
    text: "Once per agenda phase, after an agenda is revealed, you may predict aloud an outcome of that agenda. If your prediction is correct, you may score 1 public objective if you fulfill its requirements; each other player who voted for that outcome draws 1 secret objective.",
    imageURL: "/hover_images/techs/ds/unity_algorithm.png",
  },
  {
    alias: "dsedyny",
    name: "Encrypted Trade Hub",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "edyn",
    source: "ds",
    text: "You may exhaust this card to allow a player to exchange 1 of their relics or agendas as part of a transaction.\nYou always vote last during the agenda phase.\nWhen an outcome you voted for or predicted is resolved, each player who voted for that outcome gains 1 commodity.",
    imageURL: "/hover_images/techs/ds/encrypted_trade_hub.png",
  },
  {
    alias: "dsflorff",
    name: "Corsair II",
    types: ["UNITUPGRADE"],
    requirements: "BG",
    faction: "florzen",
    baseUpgrade: "ff2",
    source: "ds",
    text: "Cost 1(x2), Combat 8, Move 3\nANTI-FIGHTER BARRAGE 8\nThis unit may move without being transported.\nFighters in excess of your ships' capacity count against your fleet pool.",
    imageURL: "/hover_images/techs/ds/corsair_2.jpg",
  },
  {
    alias: "dsflorg",
    name: "Blackmail Programs",
    types: ["BIOTIC"],
    requirements: "GG",
    faction: "florzen",
    source: "ds",
    text: "When a player would perform a strategic action, you may exhaust this card and spend 1 token from your strategy pool to end that player's turn; the strategic action is not resolved and the strategy card is not exhausted.",
    imageURL: "/hover_images/techs/ds/blackmail_programs.jpg",
  },
  {
    alias: "dsfreeg",
    name: "Envoy Network",
    types: ["BIOTIC"],
    requirements: "G",
    faction: "freesystems",
    source: "ds",
    text: "At the start of the agenda phase, you may choose and exhaust 1 cultural, 1 hazardous, and 1 industrial planet. When you cast 1 or more votes, if you exhaust at least 1 cultural, 1 hazardous, and 1 industrial planet to cast votes, you may cast 4 additional votes.",
    imageURL: "/hover_images/techs/ds/envoy_network.jpg",
  },
  {
    alias: "dsfreey",
    name: "Covert Strike Teams",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "freesystems",
    source: "ds",
    text: "At the start of a ground combat, you may roll 1 die for each of up to 2 of your ground forces on that planet. For each result equal to or greater than that unit's combat value, produce 1 hit; your opponent must assign it to 1 of their units on that planet.",
    shrinkName: true,
    imageURL: "/hover_images/techs/ds/covert_strike_teams.jpg",
  },
  {
    alias: "dsghemcv",
    name: "Combat Transport II",
    types: ["UNITUPGRADE"],
    requirements: "BB",
    faction: "ghemina",
    baseUpgrade: "cv2",
    source: "ds",
    text: "Cost 3, Combat 9, Move 2, Capacity 6\nYou may reroll 1 of your unit's combat dice during each round of ground combat on a planet in this system that contains 2 or fewer of your infantry.",
    imageURL: "/hover_images/techs/ds/combat_transport_2.jpg",
  },
  {
    alias: "dsghemg",
    name: "War Song Implants",
    types: ["BIOTIC"],
    requirements: "GGG",
    faction: "ghemina",
    source: "ds",
    text: "After you win a space combat, you may ready 1 planet you control.",
    imageURL: "/hover_images/techs/ds/war_song_implants.jpg",
  },
  {
    alias: "dsghotg",
    name: "Networked Command",
    types: ["BIOTIC"],
    requirements: "G",
    faction: "ghoti",
    source: "ds",
    text: "Up to 3 ships in your home system do not count against your fleet pool.\nWhen you cast 1 or more votes, you may exhaust this card to cast an additional number of votes equal to the number of command tokens in your fleet pool.",
    shrinkName: true,
    imageURL: "/hover_images/techs/ds/networked_command.png",
  },
  {
    alias: "dsghoty",
    name: "Parallel Production",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "ghoti",
    source: "ds",
    text: "When you produce 1 or more units, you may produce 1 additional unit in each of 2 systems that contains 1 or more of your ships and 1 of your command tokens.",
    shrinkName: true,
    imageURL: "/hover_images/techs/ds/parallel_production.png",
  },
  {
    alias: "dsgledb",
    name: "Lightning Drives",
    types: ["PROPULSION"],
    requirements: "BBB",
    faction: "gledge",
    source: "ds",
    text: "During movement, you may increase the move value of each of your non-fighter ships not transporting any fighters or infantry by 1.",
    imageURL: "/hover_images/techs/ds/lightning_drives.png",
  },
  {
    alias: "dsgledpds",
    name: "Orion Platform II",
    types: ["UNITUPGRADE"],
    requirements: "RY",
    faction: "gledge",
    baseUpgrade: "pds2",
    source: "ds",
    text: "PLANETARY SHIELD, SPACE CANNON 5\nWhen this unit produces 1 or more hits against another player's units, explore this planet.\nYou may use this unit's SPACE CANNON against ships that are adjacent to this unit's system.",
    imageURL: "/hover_images/techs/ds/orion_platform_2.png",
  },
  {
    alias: "dskhracr",
    name: "Shattered Sky II",
    types: ["UNITUPGRADE"],
    requirements: "GRY",
    faction: "khrask",
    baseUpgrade: "cr2",
    source: "ds",
    text: "Cost 2, Combat 6, Move 3, Capacity 2\nBOMBARDMENT 6",
    imageURL: "/hover_images/techs/ds/shattered_sky_2.jpg",
  },
  {
    alias: "dskhrag",
    name: "Stone's Embrace",
    types: ["BIOTIC"],
    requirements: "GG",
    faction: "khrask",
    source: "ds",
    text: "When you would spend a token from your strategy pool, you may exhaust 1 planet you control to spend 1 token from your reinforcements instead.",
    imageURL: "/hover_images/techs/ds/stones_embrace.jpg",
  },
  {
    alias: "dskjalcv",
    name: "Star Dragon II",
    types: ["UNITUPGRADE"],
    requirements: "BB",
    faction: "kjalengard",
    baseUpgrade: "cv2",
    source: "ds",
    text: "Cost 3, Combat 7, Move 2, Capacity 6\nThis unit may ignore the movement effects of anomalies.",
    imageURL: "/hover_images/techs/ds/star_dragon_2.png",
  },
  {
    alias: "dskjalg",
    name: "Zhrgar Stimulants",
    types: ["BIOTIC"],
    requirements: "G",
    faction: "kjalengard",
    source: "ds",
    text: "Once per action, after a player produces 1 or more hits during a round of combat in a system that contains a Glory token, gain 1 trade good.\nAfter a player rolls combat dice, you may exhaust this card to allow that player to reroll any number of those dice.",
    shortName: "Zhrgar Stimul'nts",
    imageURL: "/hover_images/techs/ds/zhrgar_stimulants.png",
  },
  {
    alias: "dskollb",
    name: "Shrouded Skirmishers",
    types: ["PROPULSION"],
    requirements: "B",
    faction: "kollecc",
    source: "ds",
    text: "At the start of a space combat, you may choose 1 of your ships in the active system that did not begin this action in that system; roll a die. On a result equal to or greater than that ship's combat value, produce 1 hit; your opponent must assign it to 1 of their ships.",
    shortName: "Shrouded Skirmish'rs",
    imageURL: "/hover_images/techs/ds/shrouded_skirmishers.jpg",
  },
  {
    alias: "dskolly",
    name: "Seeker Drones",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "kollecc",
    source: "ds",
    text: "ACTION: Exhaust this card to choose 1 of your neighbors that has 1 or more relic fragments. That player gains 2 trade goods and must give you 1 of those relic fragments of your choice. [Note the neighbor requirement]",
    imageURL: "/hover_images/techs/ds/seeker_drones.jpg",
  },
  {
    alias: "dskolug",
    name: "Applied Biothermics",
    types: ["BIOTIC"],
    requirements: "GG",
    faction: "kolume",
    source: "ds",
    text: "When you pass, you may gain 1 commodity for each other player who has already passed.\nACTION: Exhaust this card.",
    shrinkName: true,
    imageURL: "/hover_images/techs/ds/applied_biothermics.png",
  },
  {
    alias: "dskolur",
    name: "Omniscience Field",
    types: ["WARFARE"],
    requirements: "RRR",
    faction: "kolume",
    source: "ds",
    text: "After a system is activated, you may exhaust this card to choose 1 player. Apply +1 or -1 to all rolls that player makes during this tactical action.",
    shrinkName: true,
    imageURL: "/hover_images/techs/ds/omniscience_field.png",
  },
  {
    alias: "dskortg",
    name: "Tempest Drive",
    types: ["BIOTIC"],
    requirements: "GG",
    faction: "kortali",
    source: "ds",
    text: "After you win a combat in a system, you may exhaust this card to remove 1 command token from that system.",
    imageURL: "/hover_images/techs/ds/tempest_drive.png",
  },
  {
    alias: "dskortr",
    name: "Deliverance Engine",
    types: ["WARFARE"],
    requirements: "RR",
    faction: "kortali",
    source: "ds",
    text: "Once per space combat, after 1 of your non-fighter ships is destroyed, you may produce 1 hit and assign it to 1 of your opponent's non-carrier ships.",
    shrinkName: true,
    imageURL: "/hover_images/techs/ds/deliverance_engine.jpg",
  },
  {
    alias: "dskyrog",
    name: "Indoctrination Teams",
    types: ["BIOTIC"],
    requirements: "GG",
    faction: "kyro",
    source: "ds",
    text: "During the agenda phase, after an outcome you voted for is resolved, place 2 infantry from your reinforcements on a planet you control.",
    shrinkName: true,
    shortName: "Indoctrin\n-ation Teams",
    imageURL: "/hover_images/techs/ds/indoctrination_teams.png",
  },
  {
    alias: "dskyroy",
    name: "Vector Programs",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "kyro",
    source: "ds",
    text: "At the start of the strategy phase, you may place any number of your trade goods on any combination of strategy cards. Then, place 2 trade goods from the supply on 1 strategy card",
    imageURL: "/hover_images/techs/ds/vector_programs.png",
  },
  {
    alias: "dslaneb",
    name: "Spark Thrusters",
    types: ["PROPULSION"],
    requirements: "BB",
    faction: "lanefir",
    source: "ds",
    text: "ACTION: Spend 1 command token from your strategy pool or purge 1 relic fragment to move 1 of your ships to an adjacent system that contains no other player's ships. If you spent a command token, exhaust this card.",
    imageURL: "/hover_images/techs/ds/spark_thrusters.png",
  },
  {
    alias: "dslaner",
    name: "ATS Armaments",
    types: ["WARFARE"],
    requirements: "RR",
    faction: "lanefir",
    source: "ds",
    text: "After you explore, or purge 1 or more relic fragments, place 1 commodity token on this card.\nAt the start of a round of combat, remove any number of tokens from this card to reroll up to that many of your dice during this round of combat.",
    shortName: "ATS Arm\n-aments",
    imageURL: "/hover_images/techs/ds/ats_armaments.png",
  },
  {
    alias: "dslizhb",
    name: "Wraith Engine",
    types: ["PROPULSION"],
    requirements: "BB",
    faction: "lizho",
    source: "ds",
    text: "After you activate a non-home system, you may exhaust this card to choose 1 ship you control; you may treat that ship as adjacent to the active system until the end of this tactical action.",
    imageURL: "/hover_images/techs/ds/wraith_engine.jpg",
  },
  {
    alias: "dslizhff",
    name: "Heavy Bomber II",
    types: ["UNITUPGRADE"],
    requirements: "BG",
    faction: "lizho",
    baseUpgrade: "ff2",
    source: "ds",
    text: "Cost 1(x2), Combat 8, Move 2\nBOMBARDMENT 8\nThis unit may move without being transported. Fighters in excess of your ships' capacity count against your fleet pool.",
    imageURL: "/hover_images/techs/ds/heavy_bomber_2.jpg",
  },
  {
    alias: "dsmirvpds",
    name: "Gauss Cannon II",
    types: ["UNITUPGRADE"],
    requirements: "RY",
    faction: "mirveda",
    baseUpgrade: "pds2",
    source: "ds",
    text: "Move 2, Capacity 1\nSPACE CANNON 4, BOMBARDMENT 4\nThis unit is placed in a space area instead of on a planet.\nThis unit can move and retreat as if it were a ship.\nThis unit can be blockaded, if it is blockaded, it is destroyed.[Note: this unit cannot fire into adjacent systems by itself. Needs Mirveda commander. Also note that if it is alone and an enemy ship moves into its space, it will be blockaded and die before it can shoot PDS.]",
    imageURL: "/hover_images/techs/ds/gauss_cannon_2.jpg",
  },
  {
    alias: "dsmirvr",
    name: "Orbital Defense Grid",
    types: ["WARFARE"],
    requirements: "RR",
    faction: "mirveda",
    source: "ds",
    text: "At the start of invasion, choose up to 1 unit you control in the active system; that unit gains PLANETARY SHIELD.\nWhen a player commits a unit to land on a planet you control, you may exhaust this card to destroy that unit.",
    shortName: "Orbital\nDef. Grid",
    imageURL: "/hover_images/techs/ds/orbital_defense_grid.jpg",
  },
  {
    alias: "dsmortr",
    name: "Fractal Plating",
    types: ["WARFARE"],
    requirements: "RR",
    faction: "mortheus",
    source: "ds",
    text: "After a unit you control in a system is destroyed, you may exhaust this card to place 1 unit of that type from your reinforcements in the space area of a system adjacent to that system that contains 1 or more of your ships.",
    imageURL: "/hover_images/techs/ds/fractal_plating.jpg",
  },
  {
    alias: "dsmorty",
    name: "Fabrication Grid",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "mortheus",
    source: "ds",
    text: "Each system that contains 1 or more of your ships and no planets gains PRODUCTION 2 as if it were a unit you control.",
    shortName: "Fabrica\n-tion Grid",
    imageURL: "/hover_images/techs/ds/fabrication_grid.jpg",
  },
  {
    alias: "dsmykog",
    name: "Psychoactive Armaments",
    types: ["BIOTIC"],
    requirements: "GG",
    faction: "mykomentori",
    source: "ds",
    text: "After your opponent makes a combat roll, you may exhaust this card. If you do, for each of their units that did not produce a hit, you may reroll that unit's combat roll; any hits that roll produces are produced against your opponent's units instead.",
    shrinkName: false,
    shortName: "Psychoact.\nArmaments",
    imageURL: "/hover_images/techs/ds/psychoactive_armaments.jpg",
  },
  {
    alias: "dsmykosd",
    name: "Mycelium Ring II",
    types: ["UNITUPGRADE"],
    requirements: "YY",
    faction: "mykomentori",
    baseUpgrade: "sd2",
    source: "ds",
    text: "PLANETARY SHIELD, PRODUCTION X\nThis unit's PRODUCTION value is equal to 5 more than the resource value of this planet.\nDEPLOY: When you gain control of a planet, you may replace 3 infantry on that planet with 1 space dock.",
    imageURL: "/hover_images/techs/ds/mycelium_ring_2.jpg",
  },
  {
    alias: "dsnivymf",
    name: "Voidflare Warden II",
    types: ["UNITUPGRADE"],
    requirements: "BRY",
    faction: "nivyn",
    source: "ds",
    text: "Cost 2, Combat 4\nSUSTAIN DAMAGE\nAfter a system is activated, you may have this unit become damaged to place or move the Wound token into this system. [Note: this system refers to the system the mech is in. It does not refer to the active system]",
    imageURL: "/hover_images/techs/ds/voidflare_warden_2.jpg",
  },
  {
    alias: "dsnivyy",
    name: "Voidwake Missiles",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "nivyn",
    source: "ds",
    text: "After 1 or more of your units with SUSTAIN DAMAGE makes a combat roll, you may choose 1 of those units to become damaged to reroll its combat roll.",
    imageURL: "/hover_images/techs/ds/voidwake_missiles.jpg",
  },
  {
    alias: "dsnokadd",
    name: "Sabre II",
    types: ["UNITUPGRADE"],
    requirements: "RR",
    faction: "nokar",
    baseUpgrade: "dd2",
    source: "ds",
    text: "Cost 1, Combat 7, Move 2\nANTI-FIGHTER BARRAGE 6(x3)\nAfter this unit is destroyed during combat, roll a die, on a result equal to or greater than 7, produce up to 1 hit against your opponent's ships.",
    imageURL: "/hover_images/techs/ds/sabre_2.png",
  },
  {
    alias: "dsnokay",
    name: "Local Contracts",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "nokar",
    source: "ds",
    text: "During the action phase: You may exhaust this card to use the PRODUCTION ability of 1 of your units. Then, place 1 command token from your reinforcements in that unit's system.",
    imageURL: "/hover_images/techs/ds/local_contracts.png",
  },
  {
    alias: "dsolrab",
    name: "Geosympathic Impeller",
    types: ["PROPULSION"],
    requirements: "B",
    faction: "olradin",
    source: "ds",
    text: "After you activate a system, you may exhaust a planet you control of the same type as a planet in that system to apply +1 to the move value of each of your ships during this tactical action.",
    shrinkName: true,
    shortName: "Geosympth.\nImpeller",
    imageURL: "/hover_images/techs/ds/geosympathic_impeller.jpg",
  },
  {
    alias: "dsolrar",
    name: "False Flag Operations",
    types: ["WARFARE"],
    requirements: "R",
    faction: "olradin",
    source: "ds",
    text: "At the start of your turn, you may exhaust this card and 1 planet you control to ready 1 non-home planet other than Mecatol Rex.",
    shrinkName: true,
    imageURL: "/hover_images/techs/ds/false_flag_operations.jpg",
  },
  {
    alias: "dspharb",
    name: "Reality-Field Impactor",
    types: ["PROPULSION"],
    requirements: "B",
    faction: "pharadn",
    source: "ds",
    text: "After you activate a system, you may choose 1 anomaly on the game board; that anomaly has no effect until the end of this tactical action.",
  },
  {
    alias: "dspharinf",
    name: "Immortal II",
    types: ["UNITUPGRADE"],
    requirements: "GG",
    faction: "pharadn",
    baseUpgrade: "inf2",
    source: "ds",
    text: "Cost 1(x2), Combat 7\n After this unit is destroyed or removed, capture this unit. DEPLOY: At the start of your turn, you may place 1 infantry on a planet you control. ",
  },
  {
    alias: "dsqhetcr",
    name: "Battlecruiser II",
    types: ["UNITUPGRADE"],
    requirements: "GRY",
    faction: "qhet",
    baseUpgrade: "cr2",
    source: "ds",
    text: "Cost 2, Combat 6, Move 3, Capacity 1, AFB 5 (x2)\nIf the active system contains another player's units, this unit may move through systems that contain ships controlled by different other players.",
  },
  {
    alias: "dsqhetinf",
    name: "Commando II",
    types: ["UNITUPGRADE"],
    requirements: "GG",
    faction: "qhet",
    baseUpgrade: "inf2",
    source: "ds",
    text: "Units cannot use SPACE CANNON against this unit. After this unit is destroyed, place the unit on this card. At the start of your turn, place each unit that is on this card on planets that contain your space docks. 1(x2) 7;",
  },
  {
    alias: "dsrhodb",
    name: "Pilgrimage Beacons",
    types: ["PROPULSION"],
    requirements: "BB",
    faction: "zealots",
    source: "ds",
    text: "Once during your turn, when you exhaust 1 or more planets you control that have a technology specialty, you may produce 1 ship in 1 system that contains 1 of those planets.",
    imageURL: "/hover_images/techs/ds/pilgrimage_beacons.jpg",
  },
  {
    alias: "dsrhody",
    name: "Sanctification Field",
    types: ["CYBERNETIC"],
    requirements: "YYY",
    faction: "zealots",
    source: "ds",
    text: "When you activate a system, or a system that contains 1 or more of your units is activated, you may choose 1 technology or non-fighter, non-structure unit upgrade technology another player controls; until the end of this tactical action, that card is treated as though it had no ability text.",
    shrinkName: true,
    shortName: "Sanctifica\n-tion Field",
    imageURL: "/hover_images/techs/ds/sanctification_field.jpg",
  },
  {
    alias: "dsrohdws",
    name: "Terrafactory II",
    types: ["UNITUPGRADE"],
    requirements: "RRRY",
    faction: "rohdhna",
    baseUpgrade: "ws",
    source: "ds",
    text: "Cost 12, Combat 3(x3), Move 2, Capacity 6\nSUSTAIN DAMAGE, PRODUCTION 5, BOMBARDMENT 3(x3)\nOther player's units in this system lose PLANETARY SHIELD.\nThis unit produces only 1 fighter or infantry for their cost instead of 2.",
    imageURL: "/hover_images/techs/ds/terrafactory_2.jpg",
  },
  {
    alias: "dsrohdy",
    name: "Contractual Obligations",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "rohdhna",
    source: "ds",
    text: "At the start of the agenda phase, for each unit upgrade technology you control, you may choose 1 player. Each of those players must produce 1 ship in a system that contains 1 or more of their space docks or war suns.",
    shrinkName: true,
    shortName: "Contract'l\nObligations",
    imageURL: "/hover_images/techs/ds/contractual_maintenance.jpg",
  },
  {
    alias: "dstneldd",
    name: "Blockade Runner II",
    types: ["UNITUPGRADE"],
    requirements: "RR",
    faction: "tnelis",
    baseUpgrade: "dd2",
    source: "ds",
    text: "Cost 1, Combat 8, Move 2\nANTI-FIGHTER BARRAGE 6(x4)\nThis ship can move through systems that contain other players' ships.",
    imageURL: "/hover_images/techs/ds/blockade_runner_2.jpg",
  },
  {
    alias: "dstnely",
    name: "Daedalon Flight System",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "tnelis",
    source: "ds",
    text: "After a round of space combat in a system that contains 1 or more of your mechs, you may have 1 of those units become damaged to produce 1 hit and assign it to a cruiser, dreadnought, or destroyer you do not control in that system.",
    shortName: "Daedalon Flight Sys.",
    imageURL: "/hover_images/techs/ds/daedalon_flight_system.jpg",
  },
  {
    alias: "dstoldb",
    name: "Emergency Modifications",
    types: ["PROPULSION"],
    requirements: "B",
    faction: "toldar",
    source: "ds",
    text: "After a player activates a system, choose 1 unit on the game board; apply +2 to that unit's Capacity value, if it has one, until the end of this tactical action. ",
  },
  {
    alias: "dstoldr",
    name: "V.I.R. Training",
    types: ["WARFARE"],
    requirements: "R",
    faction: "toldar",
    source: "ds",
    text: "During each round of combat, you may cancel 1 hit your units produced to cancel up to 1 hit your opponent’s units produced.",
  },
  {
    alias: "dsuydab",
    name: "Navigation Relays",
    types: ["PROPULSION"],
    requirements: "BBB",
    faction: "uydai",
    source: "ds",
    text: "After you activate a system, you may exhaust this card to allow your ships and units they transport to move out of systems that contain your command tokens during this tactical action.",
  },
  {
    alias: "dsuydag",
    name: "Messiah Protocols",
    types: ["BIOTIC"],
    requirements: "GG",
    faction: "uydai",
    source: "ds",
    text: "ACTION: Exhaust this card and spend an amount of influence equal to the number of planets you control. Then, place 1 infantry from your reinforcements on each planet you control.",
  },
  {
    alias: "dsvadeb",
    name: "Midas Turbine",
    types: ["PROPULSION"],
    requirements: "B",
    faction: "vaden",
    source: "ds",
    text: "After you activate a system, you may exhaust this card and spend any number of trade goods; for each trade good you spent, apply +1 to the move value of 1 non-fighter ship you control.",
    imageURL: "/hover_images/techs/ds/midas_turbine.jpg",
  },
  {
    alias: "dsvadey",
    name: "Krovoz Strike Teams",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "vaden",
    source: "ds",
    text: "After another player's unit uses SUSTAIN DAMAGE to cancel a hit produced by your units or abilities, you may exhaust this card to destroy that unit.\nAfter you produce 1 or more hits during a round of combat, you may spend 1 trade good to produce 1 additional hit.",
    shrinkName: true,
    imageURL: "/hover_images/techs/ds/krovoz_strike_teams.png",
  },
  {
    alias: "dsvaylcr",
    name: "Raider II",
    types: ["UNITUPGRADE"],
    requirements: "GRY",
    faction: "vaylerian",
    baseUpgrade: "cr2",
    source: "ds",
    text: "Cost 2, Combat 6, Move 3, Capacity 1\nDuring a round of space combat, if your opponent cannot declare a retreat, hits produced by this ship cannot be canceled and must be assigned to non-fighter ships, if able.",
    imageURL: "/hover_images/techs/ds/raider_2.jpg",
  },
  {
    alias: "dsvaylr",
    name: "Scavenger Exos",
    types: ["WARFARE"],
    requirements: "R",
    faction: "vaylerian",
    source: "ds",
    text: "Once per action, after you win a ground combat, you may draw 1 action card.",
    imageURL: "/hover_images/techs/ds/scavenger_exos.jpg",
  },
  {
    alias: "dsvelddn",
    name: "Lancer Dreadnought II",
    types: ["UNITUPGRADE"],
    requirements: "BBY",
    faction: "veldyr",
    baseUpgrade: "dn2",
    source: "ds",
    text: "Cost 4, Combat 5, Move 2, Capacity 1\nSUSTAIN DAMAGE, BOMBARDMENT 5, SPACE CANNON 5\nThis unit cannot be destroyed by “Direct Hit” action cards.",
    imageURL: "/hover_images/techs/ds/lancer_dreadnought_2.jpg",
  },
  {
    alias: "dsveldr",
    name: "Seidr Project",
    types: ["WARFARE"],
    requirements: "RR",
    faction: "veldyr",
    source: "ds",
    text: "At the end of each round of space combat, you may use the SPACE CANNON of 1 of your units in the active system against your opponent's ships, without rolling additional dice.",
    imageURL: "/hover_images/techs/ds/seidr_project.jpg",
  },
  {
    alias: "dszeliinf",
    name: "Impactor II",
    types: ["UNITUPGRADE"],
    requirements: "GG",
    faction: "zelian",
    baseUpgrade: "inf2",
    source: "ds",
    text: "Cost 1(x2), Combat 7\nBOMBARDMENT 8\nAfter this unit is destroyed, roll 1 die. If the result is 6 or greater, place the unit on this card. At the start of your next turn, place each unit that is on this card on a planet you control in your home system.",
    imageURL: "/hover_images/techs/ds/impactor_2.jpg",
  },
  {
    alias: "dszelir",
    name: "Shard Volley",
    types: ["WARFARE"],
    requirements: "R",
    faction: "zelian",
    source: "ds",
    text: "When 1 or more of your units produce 1 or more hits during a BOMBARDMENT roll against a planet, produce 1 additional hit for that roll.",
    imageURL: "/hover_images/techs/ds/shard_volley.jpg",
  },
  {
    alias: "dt2",
    name: "Dimensional Tear II",
    types: ["UNITUPGRADE"],
    requirements: "YY",
    faction: "cabal",
    baseUpgrade: "sd2",
    source: "pok",
    text: "PRODUCTION 7\nThis system is a gravity rift; your ships do not roll for this gravity rift. Place a dimensional tear token beneath this unit as a reminder.\nUp to 12 fighters in this system do not count against your ships' capacity.",
    imageURL: "/hover_images/techs/faction/dimensional_tear_2.jpg",
  },
  {
    alias: "dxa",
    name: "Dacxive Animators",
    types: ["BIOTIC"],
    requirements: "G",
    source: "base",
    text: "After you win a ground combat, you may place 1 infantry from your reinforcements on that planet.",
    imageURL: "/hover_images/techs/generic/dacxive_animators.png",
    initials: "DA",
  },
  {
    alias: "eidolonb",
    name: "Daetalin Engine",
    types: ["PROPULSION"],
    requirements: "BBB",
    faction: "eidolon",
    source: "eronous",
    text: "During your tactical actions, you may treat each system that contains no planets as adjacent to each other.",
  },
  {
    alias: "eidolonff",
    name: "Wurm’s Eye II",
    types: ["UNITUPGRADE"],
    requirements: "BG",
    baseUpgrade: "ff2",
    faction: "eidolon",
    source: "eronous",
    text: "This unit may move without being transported. Fighters in excess of your ships' capacity count against your fleet pool. 1(x2), 7, 2, -;",
  },
  {
    alias: "epigenetic_perfection",
    name: "Epigenetic Perfection",
    types: ["BIOTIC"],
    requirements: "GGG",
    source: "cpti",
    text: "After you resolve a technology that instructs you to draw, gain, or place a component, you may draw/gain/place an additional component of that type from it's deck or supply following that technology's restrictions.",
  },
  {
    alias: "ers",
    name: "E-Res Siphons",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "jolnar",
    source: "base",
    text: "After another player activates a system that contains 1 or more of your ships, gain 4 trade goods.",
    imageURL: "/hover_images/techs/faction/eres_siphons.jpg",
  },
  {
    alias: "executiveorder",
    name: "Executive Order",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "keleres",
    text: "ACTION: Exhaust this card and draw the top or bottom card of the Agenda deck. Players immediately vote on this agenda as if you were the speaker; you can spend trade goods and resources on this agenda as if they were votes.",
    source: "thunders_edge",
  },
  {
    alias: "exile2",
    name: "Exile II",
    types: ["UNITUPGRADE"],
    requirements: "RR",
    faction: "crimson",
    baseUpgrade: "dd2",
    text: "Cost 1, Combat 7, Move 2, ANTI-FIGHTER BARRAGE 6 (x3). At the end of any players' combat in this unit's system or up to 2 systems away, you may place 1 active or inactive breach in that system.",
    source: "thunders_edge",
  },
  {
    alias: "exo2",
    name: "Exotrireme II",
    types: ["UNITUPGRADE"],
    requirements: "BBY",
    faction: "sardakk",
    baseUpgrade: "dn2",
    source: "base",
    text: 'Cost 4, Combat 5, Move 2, Capacity 1\nSUSTAIN DAMAGE, BOMBARDMENT 4(x2)\nThis unit cannot be destroyed by "Direct Hit" action cards.\nAfter a round of space combat, you may destroy this unit to destroy up to 2 ships in this system.',
    imageURL: "/hover_images/techs/faction/exotrireme_2.jpg",
  },
  {
    alias: "fabrilerealignment",
    name: "Fibrile Realignment",
    types: ["BIOTIC"],
    requirements: "GG",
    source: "ignis_aurora",
    text: "ACTION: Exhaust this card to discard either one secret objective or one action card. Then, draw a new card of the same type.",
  },
  {
    alias: "ff2",
    name: "Fighter II",
    types: ["UNITUPGRADE"],
    requirements: "GB",
    source: "base",
    text: "Cost 1(x2), Combat 8, Move 2\nThis unit may move without being transported.\nFighters in excess of your ships' capacity count against your fleet pool.",
    imageURL: "/hover_images/techs/generic/fighter_2.jpg",
  },
  {
    alias: "ffac2",
    name: "Floating Factory II",
    types: ["UNITUPGRADE"],
    requirements: "YY",
    faction: "saar",
    baseUpgrade: "sd2",
    source: "base",
    text: "Move 2, Capacity 5\nPRODUCTION 7.\nThis unit is placed in the space area instead of on a planet.\nThis unit can move and retreat as if it were a ship.\nIf this unit is blockaded, it is destroyed.",
    imageURL: "/hover_images/techs/faction/floating_factory_2.jpg",
  },
  {
    alias: "fl",
    name: "Fleet Logistics",
    types: ["PROPULSION"],
    requirements: "BB",
    source: "base",
    text: "During each of your turns of the action phase, you may perform 2 actions instead of 1.",
    imageURL: "/hover_images/techs/generic/fleet_logistics.png",
    initials: "FL",
  },
  {
    alias: "gd",
    name: "Gravity Drive",
    types: ["PROPULSION"],
    requirements: "B",
    source: "base",
    text: "After you activate a system, apply +1 to the move value of 1 of your ships during this tactical action.",
    imageURL: "/hover_images/techs/generic/gravity_drive.png",
    initials: "GD",
  },
  {
    alias: "gjenspecterii",
    name: "Gjen Specter II",
    types: ["UNITUPGRADE"],
    requirements: "G",
    faction: "qulane",
    baseUpgrade: "inf2",
    source: "ignis_aurora",
    text: "Cost 1(x2), Combat 7\nThis unit does not count against capacity.\nUp to 4 of this unit can be transported on each non-fighter ship.\nAt the start of a space combat, you may remove up to 3 infantry from the space area of the active system. For each 1 that you remove, your opponent must assign 1 hit to their ships.",
  },
  {
    alias: "gls",
    name: "Graviton Laser System",
    types: ["CYBERNETIC"],
    requirements: "Y",
    source: "base",
    text: "You may exhaust this card before 1 or more of your units uses SPACE CANNON; hits produced by those units must be assigned to non-fighter ships if able.",
    imageURL: "/hover_images/techs/generic/graviton_laser_system.png",
    initials: "GLS",
    shortName: "Graviton Laser Sys.",
  },
  {
    alias: "gr",
    name: "Genetic Recombination",
    types: ["BIOTIC"],
    requirements: "G",
    faction: "mahact",
    source: "pok",
    text: "You may exhaust this card before a player casts votes; that player must cast at least 1 vote for an outcome of your choice or remove 1 token from their fleet pool and return it to their reinforcements.",
    shrinkName: true,
    shortName: "Genetic Re-\ncombination",
    imageURL: "/hover_images/techs/faction/genetic_recombination.jpg",
  },
  {
    alias: "guildfund",
    name: "Guild Fund",
    types: ["BIOTIC"],
    requirements: "GGG",
    faction: "drahn",
    source: "keleresplus",
    text: "After you resolve the primary ability of a strategy card, you may resolve the secondary ability of that strategy card.\nWhen you would spend a command token, you may exhaust this card to spend 1 token from your reinforcements instead.",
  },
  {
    alias: "hcf2",
    name: "Hybrid Crystal Fighter II",
    types: ["UNITUPGRADE"],
    requirements: "GB",
    faction: "naalu",
    baseUpgrade: "ff2",
    source: "base",
    text: "Cost 1(x2), Combat 7, Move 2\nThis unit may move without being transported.\nFighters in excess of your ships' capacity count as 1/2 of a ship against your fleet pool.",
    imageURL: "/hover_images/techs/faction/hybrid_crystal_fighter_2.jpg",
  },
  {
    alias: "heavy_rifle_hard_points",
    name: "Heavy Rifle Hard Points",
    types: ["WARFARE"],
    requirements: "RR",
    source: "cpti",
    text: "While in space, each of your ground forces gain ANTI-FIGHTER BARRAGE 8.\n\nAfter you use one or more ships' SUSTAIN DAMAGE abilities during a round combat, you may use your units ANTI-FIGHTER BARRAGE abilities in that system.",
  },
  {
    alias: "helios2",
    name: '4X41C "Helios" V2',
    types: ["UNITUPGRADE"],
    requirements: "YY",
    faction: "bastion",
    baseUpgrade: "sd2",
    text: "This unit's PRODUCTION value is equal to 4 more than the resource value of this planet.\nThe resource value of this planet is increased by 2. \nUp to 3 fighters in this system do not count against your ships' capacity.",
    source: "thunders_edge",
  },
  {
    alias: "hm",
    name: "Hyper Metabolism",
    types: ["BIOTIC"],
    requirements: "GG",
    source: "base",
    text: "During the status phase, gain 3 command tokens instead of 2.",
    imageURL: "/hover_images/techs/generic/hyper_metabolism.png",
    shrinkName: true,
    shortName: "Hyper\nMetabolism",
    initials: "HM",
  },
  {
    alias: "ht2",
    name: "Hel-Titan II",
    types: ["UNITUPGRADE"],
    requirements: "YR",
    faction: "titans",
    baseUpgrade: "pds2",
    source: "pok",
    text: "Combat 6\nPLANETARY SHIELD, SPACE CANNON 5, SUSTAIN DAMAGE, PRODUCTION 1\nThis unit is treated as both a structure and a ground force. It cannot be transported.\nYou may use this unit's SPACE CANNON against ships that are adjacent to this unit's systems.",
    imageURL: "/hover_images/techs/faction/heltitan_2.jpg",
  },
  {
    alias: "htp",
    name: "Hegemonic Trade Policy",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "winnu",
    source: "base",
    text: "Exhaust this card when 1 or more of your units use PRODUCTION; swap the resource and influence values of 1 planet you control during that use of Production.",
    shrinkName: true,
    shortName: "Hegemonic Trade Plcy",
    imageURL: "/hover_images/techs/faction/hegemonic_trade_policy.jpg",
  },
  {
    alias: "hydrothermal",
    name: "Hydrothermal Mining",
    types: ["BIOTIC"],
    requirements: "G",
    faction: "deepwrought",
    text: "At the start of the status phase, gain 1 trade good for each ocean card in play.",
    source: "thunders_edge",
  },
  {
    alias: "ic",
    name: "Impulse Core",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "yin",
    source: "base",
    text: "At the start of a space combat, you may destroy 1 of your cruisers or destroyers in the active system to produce 1 hit against your opponent's ships; that hit must be assigned by your opponent to 1 of their non-fighters ships if able.",
    imageURL: "/hover_images/techs/faction/impulse_core.jpg",
  },
  {
    alias: "ie",
    name: "Integrated Economy",
    types: ["CYBERNETIC"],
    requirements: "YYY",
    source: "base",
    text: "After you gain control of a planet, you may produce any number of units on that planet that have a combined cost equal to or less than that planet's resource value.",
    imageURL: "/hover_images/techs/generic/integrated_economy.png",
    initials: "IE",
    shortName: "Integr't'd Economy",
  },
  {
    alias: "iihq",
    name: "I.I.H.Q. Modernization",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "keleres",
    source: "pok",
    text: "You are neighbors with all players that have units or control planets in or adjacent to the Mecatol Rex system.\nGain the Custodia Vigilia planet card and its legendary planet ability card. You cannot lose these cards, and this card cannot have an X or Y assimilator token placed on it.\n\n[Custodia Vigilia is a 2/3 legendary planet with the ability:\nWhile you control Mecatol Rex, it gains SPACE CANNON 5 and PRODUCTION 3.\nGain 2 command tokens when another player scores Custodians VP using Imperial Primary.]",
    shortName: "I.I.H.Q. Mod-\nernization",
    shrinkName: true,
    imageURL: "/hover_images/techs/faction/iihq_modernization.jpg",
  },
  {
    alias: "inf2",
    name: "Infantry II",
    types: ["UNITUPGRADE"],
    requirements: "GG",
    source: "base",
    text: "Cost 1(x2), Combat 7\nAfter this unit is destroyed, roll 1 die. If the result is 6 or greater, place the unit on this card. At the start of your next turn, place each unit that is on this card on a planet you control in your home system.",
    imageURL: "/hover_images/techs/generic/infantry_2.jpg",
  },
  {
    alias: "is",
    name: "Inheritance Systems",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "l1z1x",
    source: "base",
    text: "You may exhaust this card and spend 2 resources when you research a technology; ignore all of that technology's prerequisites.",
    shrinkName: true,
    imageURL: "/hover_images/techs/faction/inheritance_systems.jpg",
  },
  {
    alias: "it",
    name: "Instinct Training",
    types: ["BIOTIC"],
    requirements: "G",
    faction: "xxcha",
    source: "base",
    text: "You may exhaust this card and spend 1 token from your strategy pool when another player plays an action card; cancel that action card.",
    imageURL: "/hover_images/techs/faction/instinct_training.jpg",
  },
  {
    alias: "l4",
    name: "L4 Disruptors",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "letnev",
    source: "base",
    text: "During an invasion, units cannot use SPACE CANNON against your units.",
    shortName: "L4 Dis-\nruptors",
    imageURL: "/hover_images/techs/faction/l4_disruptors.jpg",
  },
  {
    alias: "leviathaneruptor",
    name: "Leviathan Eruptor",
    types: ["WARFARE"],
    requirements: "RR",
    faction: "cabal",
    source: "ignis_aurora",
    text: "At the end of a combat round, you may exhaust this card to return 1 captured unit and place a unit of the same type from your reinforcements in the active system.",
  },
  {
    alias: "lgf",
    name: "Lazax Gate Folding",
    types: ["PROPULSION"],
    requirements: "BB",
    faction: "winnu",
    source: "base",
    text: "During your tactical actions, if you do not control Mecatol Rex, treat its system as if it has both an α and β wormhole.\nACTION: If you control Mecatol Rex, exhaust this card to place 1 infantry from your reinforcements on Mecatol Rex.",
    imageURL: "/hover_images/techs/faction/lazax_gate_folding.jpg",
  },
  {
    alias: "linkship2",
    name: "Linkship II",
    types: ["UNITUPGRADE"],
    requirements: "RR",
    faction: "ralnel",
    baseUpgrade: "dd2",
    text: "Cost 1, Combat 8, Move 4, ANTI-FIGHTER BARRAGE 6 (x3). This unit can use the SPACE CANNON ability of one of your structures in its space area; each linkship can trigger the same structure.",
    source: "thunders_edge",
  },
  {
    alias: "lw2",
    name: "Letani Warrior II",
    types: ["UNITUPGRADE"],
    requirements: "GG",
    faction: "arborec",
    baseUpgrade: "inf2",
    source: "base",
    text: "Cost 1(x2), Combat 7\nPRODUCTION 2\nAfter this unit is destroyed, roll 1 die. If the result is 6 or greater, place the unit on this card. At the start of your next turn, place each unit that is on this card on a planet you control in your home system.",
    imageURL: "/hover_images/techs/faction/letani_warrior_2.jpg",
  },
  {
    alias: "lwd",
    name: "Light/Wave Deflector",
    types: ["PROPULSION"],
    requirements: "BBB",
    source: "base",
    text: "Your ships can move through systems that contain other players' ships.",
    imageURL: "/hover_images/techs/generic/lightwave_deflector.png",
    initials: "L/W",
  },
  {
    alias: "m2",
    name: "Memoria II",
    types: ["UNITUPGRADE"],
    requirements: "GBY",
    faction: "nomad",
    source: "pok",
    text: "Cost 8, Combat 5(x2), Move 2, Capacity 6\nSUSTAIN DAMAGE, ANTI-FIGHTER BARRAGE 5(x3)\nYou may treat this unit as if it were adjacent to systems that contain one or more of your mechs.",
    imageURL: "/hover_images/techs/faction/memoria_2.jpg",
  },
  {
    alias: "mc",
    name: "Mirror Computing",
    types: ["CYBERNETIC"],
    requirements: "YYY",
    faction: "mentak",
    source: "base",
    text: "When you spend trade goods, each trade good is worth 2 resources or influence instead of 1.",
    imageURL: "/hover_images/techs/faction/mirror_computing.jpg",
  },
  {
    alias: "md",
    name: "Magen Defense Grid ΩΩ",
    types: ["WARFARE"],
    requirements: "R",
    source: "codex4",
    text: "When any player activates a system that contains 1 or more of your structures, place 1 infantry from your reinforcements with each of those structures.\nAt the start of ground combat on a planet that contains 1 or more of your structures, produce 1 hit and assign it to 1 of your opponent's ground forces.",
    imageURL: "/hover_images/techs/generic/magen_defense_grid_omega_omega.png",
    initials: "MDG",
    shortName: "Magen Def. Grid",
  },
  {
    alias: "md_base",
    name: "Magen Defense Grid",
    types: ["WARFARE"],
    requirements: "R",
    source: "base",
    text: "You may exhaust this card at the start of a round of ground combat on a planet that contains 1 or more of your units that have PLANETARY SHIELD; your opponent cannot make combat rolls this combat round.",
    imageURL: "/hover_images/techs/generic/magen_defense_grid.png",
    initials: "MDG",
    shortName: "Magen Def. Grid",
  },
  {
    alias: "md_c1",
    name: "Magen Defense Grid Ω",
    types: ["WARFARE"],
    requirements: "R",
    source: "codex1",
    text: "At the start of ground combat on a planet that contains 1 or more of your structures, produce 1 hit and assign it to 1 of your opponent's ground forces.",
    imageURL: "/hover_images/techs/generic/magen_defense_grid_omega.png",
    initials: "MDG",
    shortName: "Magen Def. Grid",
  },
  {
    alias: "mechig",
    name: "Overdrive",
    types: ["BIOTIC"],
    requirements: "GG",
    faction: "mechi",
    source: "eronous",
    text: "After another player activates a system that contains one of your units with PRODUCTION. You may produce one unit in that system.",
  },
  {
    alias: "mechiy",
    name: "Ikanium Shielding",
    types: ["CYBERNETIC"],
    requirements: "YYY",
    faction: "mechi",
    source: "eronous",
    text: "Each of your units with PRODUCTION gains PLANETARY SHIELD. Planets that contain one of your Space Docks cannot be bombarded.",
  },
  {
    alias: "mi",
    name: "Mageon Implants",
    types: ["BIOTIC"],
    requirements: "GGG",
    faction: "yssaril",
    source: "base",
    text: "ACTION: Exhaust this card to look at another player's hand of action cards.  Choose 1 of those cards and add it to your hand.",
    imageURL: "/hover_images/techs/faction/mageon_implants.jpg",
  },
  {
    alias: "miltymod_ac2",
    name: "Advanced Carrier II",
    types: ["UNITUPGRADE"],
    requirements: "GB",
    faction: "sol",
    baseUpgrade: "miltymod_cv2",
    source: "miltymod",
    text: "Cost 3, Combat 9, Move 2, Capacity 8\nSUSTAIN DAMAGE",
    homebrewReplacesID: "ac2",
  },
  {
    alias: "miltymod_bio",
    name: "Bioplasmosis",
    types: ["BIOTIC"],
    requirements: "G",
    faction: "arborec",
    source: "miltymod",
    text: "ACTION: You may exhaust this card to remove any number of ground forces from planets you control and place them on 1 or more planets you control in the same or adjacent systems.",
    homebrewReplacesID: "bio",
  },
  {
    alias: "miltymod_cm",
    name: "Chaos Mapping",
    types: ["PROPULSION"],
    requirements: "BBB",
    faction: "saar",
    source: "miltymod",
    text: "Other players cannot move ships into asteroid fields that contain 1 or more of your ships.\nAt the start of your turn during the action phase, you may produce 1 unit in a system that contains at least 1 of your units that has PRODUCTION.",
    homebrewReplacesID: "cm",
  },
  {
    alias: "miltymod_cr2",
    name: "Cruiser II",
    types: ["UNITUPGRADE"],
    requirements: "GYR",
    source: "miltymod",
    text: "Cost 2, Combat 6, Move 3, Capacity 1\nWhen making a combat roll for this ship, each result of 9 or 10, before applying modifiers, produces 1 additional hit.",
    homebrewReplacesID: "cr2",
  },
  {
    alias: "miltymod_cv2",
    name: "Carrier II",
    types: ["UNITUPGRADE"],
    requirements: "GB",
    source: "miltymod",
    text: "Cost 3, Combat 9, Move 2, Capacity 6",
    homebrewReplacesID: "cv2",
  },
  {
    alias: "miltymod_da",
    name: "Duranium Armor",
    types: ["WARFARE"],
    requirements: "RR",
    source: "miltymod",
    text: "During each combat round, after you assign hits to your units, repair 1 of your participating damaged units that did not use SUSTAIN DAMAGE during this combat round.",
    homebrewReplacesID: "da",
  },
  {
    alias: "miltymod_dn2",
    name: "Dreadnought II",
    types: ["UNITUPGRADE"],
    requirements: "RBY",
    source: "miltymod",
    text: "Cost 4, Combat 4, Move 2, Capacity 1\nSUSTAIN DAMAGE, BOMBARDMENT 4",
    homebrewReplacesID: "dn2",
  },
  {
    alias: "miltymod_dxa",
    name: "Dacxive Animators",
    types: ["BIOTIC"],
    requirements: "G",
    source: "miltymod",
    text: "After 1 of your ground forces is destroyed during ground combat, you may place that unit on this card.\nAt the end of your turn, you may exhaust this card to place any number of units from it onto a planet you control.",
    homebrewReplacesID: "dxa",
  },
  {
    alias: "miltymod_ers",
    name: "E-Res Siphons",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "jolnar",
    source: "miltymod",
    text: "After another player activates a system that contains 1 or more of your ships, gain 2 trade goods.",
    homebrewReplacesID: "ers",
  },
  {
    alias: "miltymod_exo2",
    name: "Exotrireme II",
    types: ["UNITUPGRADE"],
    requirements: "RBY",
    faction: "sardakk",
    baseUpgrade: "miltymod_dn2",
    source: "miltymod",
    text: "Cost 4, Combat 4, Move 2, Capacity 1\nSUSTAIN DAMAGE, BOMBARDMENT 3(x2)\nAfter a round of space combat, you may destroy this unit to destroy up to 2 ships in this system.",
    homebrewReplacesID: "exo2",
  },
  {
    alias: "miltymod_ff2",
    name: "Fighter II",
    types: ["UNITUPGRADE"],
    requirements: "BY",
    source: "miltymod",
    text: "Cost: 1(x2), Combat 8, Move 2\nThis unit may move without being transported. Fighters in excess of your ships' capacity count against your fleet pool.",
    homebrewReplacesID: "ff2",
  },
  {
    alias: "miltymod_ffac2",
    name: "Floating Factory II",
    types: ["UNITUPGRADE"],
    requirements: "YY",
    faction: "saar",
    baseUpgrade: "miltymod_sd2",
    source: "miltymod",
    text: "Move 2, Capacity 6\nPRODUCTION 7\nThis unit is placed in the space area instead of on a planet.\nThis unit can move and retreat as if it were a ship.\nIf this unit is blockaded, it is destroyed.",
    homebrewReplacesID: "ff2",
  },
  {
    alias: "miltymod_fl",
    name: "Fleet Logistics",
    types: ["PROPULSION"],
    requirements: "BB",
    source: "miltymod",
    text: "During each of your turns of the action phase, after you perform an action, you may perform an additional tactical action.",
    homebrewReplacesID: "fl",
  },
  {
    alias: "miltymod_gls",
    name: "Graviton Laser System",
    types: ["CYBERNETIC"],
    requirements: "Y",
    source: "miltymod",
    text: "You may exhaust this card before 1 or more of your units use SPACE CANNON; hits produced by those units must be assigned to non-fighter ships if able.\nYou may exhaust this card at the start of a space combat round; hits produced by your units this round must be assigned to non-fighter ships if able.",
    homebrewReplacesID: "gls",
  },
  {
    alias: "miltymod_hcf2",
    name: "Hybrid Crystal Fighter II",
    types: ["UNITUPGRADE"],
    requirements: "BY",
    faction: "naalu",
    baseUpgrade: "miltymod_ff2",
    source: "miltymod",
    text: "Cost: 1(x2), Combat 7, Move 2\nThis unit may move without being transported.\nFighters in excess of your ships' capacity count as 1/2 of a ship against your fleet pool.",
    homebrewReplacesID: "hcf2",
  },
  {
    alias: "miltymod_hm",
    name: "Hyper Metabolism",
    types: ["BIOTIC"],
    requirements: "GG",
    source: "miltymod",
    text: "At the end of your turn, you may exhaust this card to gain 1 command token.",
    homebrewReplacesID: "hm",
  },
  {
    alias: "miltymod_htp",
    name: "Hegemonic Trade Policy",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "winnu",
    source: "miltymod",
    text: "Exhaust this card when 1 or more of your units use PRODUCTION; swap the resource and influence values of 1 planet you control during that use of PRODUCTION.",
    homebrewReplacesID: "htp",
  },
  {
    alias: "miltymod_ic",
    name: "Impulse Core",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "yin",
    source: "miltymod",
    text: "At the start of a space combat, you may destroy 1 of your cruisers or destroyers in the active system to produce 1 hit and assign it to 1 of your opponent's ships.",
    homebrewReplacesID: "ic",
  },
  {
    alias: "miltymod_ie",
    name: "Integrated Economy",
    types: ["CYBERNETIC"],
    requirements: "YYY",
    source: "miltymod",
    text: "After you gain control of a planet, you may produce any number of units on that planet that have a combined cost equal or less than the sum of that planet's resource and influence values.",
    homebrewReplacesID: "ie",
  },
  {
    alias: "miltymod_inf2",
    name: "Infantry II",
    types: ["UNITUPGRADE"],
    requirements: "GG",
    source: "miltymod",
    text: "Cost: 1(x2), Combat 7\nWhen you gain control of a planet, if you have more infantry on that planet than that planet's resource value, gain that planet readied.",
    homebrewReplacesID: "inf2",
  },
  {
    alias: "miltymod_is",
    name: "Inheritance Systems",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "l1z1x",
    source: "miltymod",
    text: "You may exhaust this card when you research a technology; ignore all of that technology's prerequisites.",
    homebrewReplacesID: "is",
  },
  {
    alias: "miltymod_it",
    name: "Instinct Training",
    types: ["BIOTIC"],
    requirements: "GG",
    faction: "xxcha",
    source: "miltymod",
    text: "You may exhaust this card when another player plays an action card; cancel that action card.",
    homebrewReplacesID: "it",
  },
  {
    alias: "miltymod_l4",
    name: "L4 Disruptors",
    types: ["CYBERNETIC"],
    requirements: "",
    faction: "letnev",
    source: "miltymod",
    text: "During an invasion, units cannot use SPACE CANNON against your units.",
    homebrewReplacesID: "l4",
  },
  {
    alias: "miltymod_lgf",
    name: "Lazax Gate Folding",
    types: ["PROPULSION"],
    requirements: "B",
    faction: "winnu",
    source: "miltymod",
    text: "During your tactical actions, treat the Mecatol Rex system as if it contains each non-delta wormhole.\nACTION: If you control Mecatol Rex, exhaust this card to place 1 infantry from your reinforcements on Mecatol Rex.",
    homebrewReplacesID: "lgf",
  },
  {
    alias: "miltymod_lw2",
    name: "Letani Warrior II",
    types: ["UNITUPGRADE"],
    requirements: "GG",
    faction: "arborec",
    baseUpgrade: "miltymod_inf2",
    source: "miltymod",
    text: "Cost: 1(x2), Combat 7\nPRODUCTION 2\nWhen you gain control of a planet, if you have more infantry on that planet than that planet's resource value, gain that planet readied.",
    homebrewReplacesID: "lw2",
  },
  {
    alias: "miltymod_md",
    name: "Magen Defense Grid",
    types: ["WARFARE"],
    requirements: "R",
    source: "miltymod",
    text: "You may exhaust this card at the start of a round of ground combat on a planet you control; your opponent cannot make combat rolls during this combat round.",
    homebrewReplacesID: "md",
  },
  {
    alias: "miltymod_mr",
    name: "Magmus Reactor",
    types: ["WARFARE"],
    requirements: "RR",
    faction: "muaat",
    source: "miltymod",
    text: "Your ships can move into supernovas.\nAfter you place 1 or more units in a system that either contains a war sun or is adjacent to a supernova, gain 1 trade good.",
    homebrewReplacesID: "mr",
  },
  {
    alias: "miltymod_nm",
    name: "Neural Motivator",
    types: ["BIOTIC"],
    source: "miltymod",
    text: "At the end of your turn, you may exhaust this card to draw 1 action card.",
    homebrewReplacesID: "nm",
  },
  {
    alias: "miltymod_pm",
    name: "Production Biomes",
    types: ["BIOTIC"],
    requirements: "GG",
    faction: "hacan",
    source: "miltymod",
    text: "Exhaust this card to gain 3 trade goods and choose 1 other player; that player gains 2 trade goods.",
    homebrewReplacesID: "pm",
  },
  {
    alias: "miltymod_ps",
    name: "Plasma Scoring",
    types: ["WARFARE"],
    source: "miltymod",
    text: "When 1 or more of your units make a roll for a unit ability, you may choose 1 of those units to roll 1 additional die.",
    homebrewReplacesID: "ps",
  },
  {
    alias: "miltymod_pws2",
    name: "Prototype Warsun II",
    types: ["UNITUPGRADE"],
    requirements: "GYRR",
    faction: "muaat",
    source: "miltymod",
    text: "Cost 10, Combat 3(x3), Move 3, Capacity 6\nSUSTAIN DAMAGE, BOMBARDMENT 3(x3)\nOther player's units in this system lose PLANETARY SHIELD.",
    homebrewReplacesID: "pws2",
  },
  {
    alias: "miltymod_sd2",
    name: "Space Dock II",
    types: ["UNITUPGRADE"],
    requirements: "YY",
    source: "miltymod",
    text: "PRODUCTION X\nThis unit's PRODUCTION value is equal to 5 more than the resource value of this planet.\nUp to 5 fighters in this system do not count against your ships' capacity.\nWhen you use this unit's PRODUCTION ability, you may place the produced units on any planet in this system.",
    homebrewReplacesID: "sd2",
  },
  {
    alias: "miltymod_sdn2",
    name: "Super-Dreadnought II",
    types: ["UNITUPGRADE"],
    requirements: "RBY",
    faction: "l1z1x",
    baseUpgrade: "miltymod_dn2",
    source: "miltymod",
    text: "Cost 4, Combat 3, Move 2, Capacity 2\nSUSTAIN DAMAGE, BOMBARDMENT 3",
    homebrewReplacesID: "sdn2",
  },
  {
    alias: "miltymod_so2",
    name: "Spec Ops II",
    types: ["UNITUPGRADE"],
    requirements: "GG",
    faction: "sol",
    baseUpgrade: "miltymod_inf2",
    source: "miltymod",
    text: "Cost: 1(x2), Combat 6\nWhen you gain control of a planet, if you have more infantry on that planet than that planet's resource or influence value, gain that planet readied.",
    homebrewReplacesID: "so2",
  },
  {
    alias: "miltymod_st",
    name: "Sarween Tools",
    types: ["CYBERNETIC"],
    source: "miltymod",
    text: "When you produce 1 or more units, reduce the combined cost of the produced units by 1.",
    homebrewReplacesID: "st",
  },
  {
    alias: "miltymod_vpw",
    name: "Valkyrie Particle Weave",
    types: ["WARFARE"],
    requirements: "R",
    faction: "sardakk",
    source: "miltymod",
    text: "After making combat rolls during a round of ground combat, if your opponent produced 1 or more hits, you produce 1 additional hit.",
    homebrewReplacesID: "vpw",
  },
  {
    alias: "miltymod_wg",
    name: "Wormhole Generator",
    types: ["PROPULSION"],
    requirements: "B",
    faction: "ghost",
    source: "miltymod",
    text: "ACTION: Exhaust this card to place or move a Creuss wormhole token into either a system that contains a planet you control or a non-home system that does not contain another player's ships.",
    homebrewReplacesID: "wg",
  },
  {
    alias: "miltymod_ws",
    name: "War Sun",
    types: ["UNITUPGRADE"],
    requirements: "GYRR",
    source: "miltymod",
    text: "Cost 12, Combat 3(x3), Move 2, Capacity 6\nSUSTAIN DAMAGE, BOMBARDMENT 3(x3)\nOther player's units in this system lose PLANETARY SHIELD.",
    homebrewReplacesID: "ws",
  },
  {
    alias: "miltymod_x89",
    name: "X-89 Bacterial Weapon",
    types: ["BIOTIC"],
    requirements: "GGG",
    source: "miltymod",
    text: "After 1 or more of your units use BOMBARDMENT against a planet, if at least 1 hit was produced, you may exhaust this card; if you do, destroy all other player's ground forces on that planet.",
    homebrewReplacesID: "x89",
  },
  {
    alias: "miltymod_yso",
    name: "Yin Spinner",
    types: ["BIOTIC"],
    requirements: "G",
    faction: "yin",
    source: "miltymod",
    text: "After you produce units, place up to 2 infantry from your reinforcements on any planet you control or in any space area that contains 1 or more of your ships.",
    homebrewReplacesID: "yso",
  },
  {
    alias: "mr",
    name: "Magmus Reactor",
    types: ["WARFARE"],
    requirements: "RR",
    faction: "muaat",
    source: "base",
    text: "Your ships can move into supernovas.\nEach supernova that contains 1 or more of your units gains the PRODUCTION 5 ability as if it were 1 of your units.",
    imageURL: "/hover_images/techs/faction/magmus_reactor_omega.jpg",
  },
  {
    alias: "murmuration",
    name: "Murmuration",
    types: ["PROPULSION"],
    requirements: "BB",
    faction: "raven",
    source: "ignis_aurora",
    text: "At the start of your turn, you may purge this card. If you do, you may remove any number of your ships from the gameboard and then place them in any systems that do not contain another player's ships.",
  },
  {
    alias: "nanomachines",
    name: "Nanomachines",
    types: ["WARFARE"],
    requirements: "R",
    faction: "ralnel",
    text: "ACTION: Exhaust this card to place 1 PDS on a planet you control.\nACTION: Exhaust this card to repair all of your damaged units.\nACTION: Exhaust this card and discard 1 action card to draw 1 action card.",
    source: "thunders_edge",
  },
  {
    alias: "ne_inf2",
    name: "Sentinel II",
    types: ["UNITUPGRADE"],
    requirements: "GG",
    faction: "netharii",
    source: "memephilosopher",
    text: "Cost 1, Combat 5\nThis unit can only use SPACE CANNON if on a planet. You may use this unit's SPACE CANNON against ships that are adjacent to this unit's system.",
  },
  {
    alias: "ne_qed",
    name: "Quantum Entanglement Drive",
    types: ["PROPULSION"],
    requirements: "B",
    faction: "netharii",
    source: "memephilosopher",
    text: "Exhaust this card. Systems with sentinels are considered adjacent during this tactical action.",
  },
  {
    alias: "negr",
    name: "Nexus Grid",
    types: ["PROPULSION"],
    requirements: "BB",
    faction: "raven",
    source: "pbd2000",
    text: "After you activate a system, you may exhaust this card to treat the active system as if it contains every type of wormhole.",
  },
  {
    alias: "nekroc4r",
    name: "???\\_ERROR\\_ERROR\\_???",
    types: ["WARFARE"],
    requirements: "R",
    faction: "nekro",
    source: "pok",
    text: "ACTION: Exhaust this card to place 1 PDS on a planet you control. \nACTION: Exhaust this card to repair all of your damaged units. \nACTION: Exhaust this card and discard 1 action card to draw 1 action card.",
    shortName: "Error\nError",
    shrinkName: false,
    imageURL: "/hover_images/techs/faction/error_error.jpg",
  },
  {
    alias: "nekroc4y",
    name: "???\\_NULL\\_REFERENCE\\_???",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "nekro",
    source: "pok",
    text: "When one of your ships is destroyed, you may produce a ship of the same type at a space dock in your home system.",
    shortName: "Null_\nRef'rence",
    shrinkName: false,
    imageURL: "/hover_images/techs/faction/null_ref.jpg",
  },
  {
    alias: "nes",
    name: "Non-Euclidean Shielding",
    types: ["WARFARE"],
    requirements: "RR",
    faction: "letnev",
    source: "base",
    text: "When 1 of your units uses SUSTAIN DAMAGE, cancel 2 hits instead of 1.",
    shortName: "Non-Euclid.\nShielding",
    shrinkName: false,
    imageURL: "/hover_images/techs/faction/noneuclidean_shielding.jpg",
  },
  {
    alias: "nexusgrid",
    name: "Nexus Grid",
    types: ["PROPULSION"],
    requirements: "BB",
    faction: "raven",
    source: "ignis_aurora",
    text: "After you activate a system, you may exhaust this card to treat the active system as if it contains every type of wormhole.",
  },
  {
    alias: "nf",
    name: "Nullification Field",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "xxcha",
    source: "base",
    text: "After another player activates a system that contains 1 or more of your ships, you may exhaust this card and spend 1 token from your strategy pool; immediately end that player's turn.",
    shrinkName: true,
    shortName: "Nullificat'n\nField",
    imageURL: "/hover_images/techs/faction/nullification_field.jpg",
  },
  {
    alias: "nf_al",
    name: "Assault Laser",
    types: ["WARFARE"],
    requirements: "RRY",
    source: "nomadfalcon",
    text: "At the start of a combat you are participating in you may exhaust this card to produce 2 hits against your opponent.\nAfter 1 or more of your non-fighter non-infantry unit is destroyed roll 1 die for 1 of the destroyed unit on a result of 6 or higher produce 1 hit against your opponent.",
  },
  {
    alias: "nf_brs",
    name: "Bio-Redesign Systems",
    types: ["BIOTIC"],
    requirements: "GGR",
    source: "nomadfalcon",
    text: "After the move ships step of the tactical action if you have ships in the active system you may exhaust this card to remove 1 of your opponents ships in the active system and place a matching unit from your reinforcements in the active system.",
  },
  {
    alias: "nf_bws",
    name: "Bacterial Weapons Systems",
    types: ["WARFARE", "BIOTIC"],
    requirements: "RG",
    source: "nomadfalcon",
    text: "After 1 of your opponents ships are destroyed you may exhaust this card to destroy 2 infantry in the space area of the active system.\nThis card counts as either a red or green prerequisite.",
  },
  {
    alias: "nf_clp",
    name: "Carrier Life Pods",
    types: ["CYBERNETIC"],
    requirements: "YYG",
    source: "nomadfalcon",
    text: "Each of your non-fighter ships without the capacity stat gain the capacity stat.\nApply +1 to the capacity value to each of your units with capacity.",
  },
  {
    alias: "nf_gdn",
    name: "Gene Data Network",
    types: ["BIOTIC", "PROPULSION"],
    requirements: "GB",
    source: "nomadfalcon",
    text: "At the start of the status phase you may place 1 infantry on each planet with 1 or more structures.\nThis card counts as either a green or blue prerequisite.",
  },
  {
    alias: "nf_hg",
    name: "Hypernet Gates",
    types: ["PROPULSION"],
    requirements: "BBY",
    source: "nomadfalcon",
    text: "After you activate a system that contains 1 or more of your structures you may exhaust this card to treat all systems with your structures as adjacent till the end of your turn.",
  },
  {
    alias: "nf_ni",
    name: "Neural Implants",
    types: ["CYBERNETIC", "BIOTIC"],
    requirements: "YG",
    source: "nomadfalcon",
    text: "At the start of your turn you may remove 1 of your infantry from the game board to gain 1 trade good.\nThis card counts as either a green or yellow prerequisite.",
  },
  {
    alias: "nf_pdn",
    name: "Planetary Defense Network",
    types: ["PROPULSION", "WARFARE "],
    requirements: "BR",
    source: "nomadfalcon",
    text: "At the start of the status phase you may remove 1 PDS from 1 planet you control and place it on another planet you control in or adjacent to that system.\nThis card counts as either a red or blue prerequisite.",
  },
  {
    alias: "nf_prs",
    name: "Plasma Radial Shielding",
    types: ["BIOTIC", "WARFARE"],
    requirements: "RG",
    source: "nomadfalcon",
    text: "If you win a combat in a system with 3 or more non-fighter ships you may move 1 ship with a cost of 4 or fewer out of the active system into an adjacent system with any of your ships.\nThis card counts as either a red or green prerequisite.",
  },
  {
    alias: "nf_tfp",
    name: "Transit Fighter Pods",
    types: ["PROPULSION", "CYBERNETIC"],
    requirements: "BY",
    source: "nomadfalcon",
    text: "At the start of your turn during the action phase you may exhaust this card to remove up to 4 of your fighters from the game board and place them in up to 4 systems with any of your ships.",
  },
  {
    alias: "ng",
    name: "Neuroglaive",
    types: ["BIOTIC"],
    requirements: "GGG",
    faction: "naalu",
    source: "base",
    text: "After another player activates a system that contains 1 or more of your ships, that player removes 1 token from their fleet pool and returns it to their reinforcements.",
    shortName: "Neuro-\nglaive",
    imageURL: "/hover_images/techs/faction/neuroglaive.jpg",
  },
  {
    alias: "nm",
    name: "Neural Motivator",
    types: ["BIOTIC"],
    source: "base",
    text: "During the status phase, draw 2 action cards instead of 1.",
    imageURL: "/hover_images/techs/generic/neural_motivator.png",
    initials: "NM",
  },
  {
    alias: "pa",
    name: "Psychoarchaeology",
    types: ["BIOTIC"],
    source: "pok",
    text: "You can use technology specialties on planets you control without exhausting them, even if those planets are exhausted.\nDuring the action phase, you can exhaust planets you control that have technology specialties to gain 1 trade good.",
    imageURL: "/hover_images/techs/generic/psychoarchaeology.png",
    initials: "PSY",
    shortName: "Psychoarch\n-aeology",
    shrinkName: true,
  },
  {
    alias: "parasite-firm",
    name: "Neural Parasite (Firmament)",
    types: ["BIOTIC"],
    requirements: "GG",
    faction: "firmament",
    text: "At the start of the status phase, you may place 1 infantry from your reinforcements on a planet you control in your home system.\nFlip this card if the Obsidian faction is in play.",
    source: "thunders_edge",
  },
  {
    alias: "parasite-obs",
    name: "Neural Parasite (Obsidian)",
    types: ["BIOTIC"],
    requirements: "null",
    faction: "obsidian",
    text: "At the start of your turn, destroy 1 of another player's infantry in or adjacent to a system that contains your infantry.\nThis technology cannot be researched.",
    source: "thunders_edge",
  },
  {
    alias: "pds2",
    name: "PDS II",
    types: ["UNITUPGRADE"],
    requirements: "RY",
    source: "base",
    text: "PLANETARY SHIELD, SPACE CANNON 5\nYou may use this unit's SPACE CANNON against ships that are adjacent to this unit's system.",
    imageURL: "/hover_images/techs/generic/pds_2.jpg",
  },
  {
    alias: "pfa",
    name: "Pre-Fab Arcologies",
    types: ["BIOTIC"],
    requirements: "GGG",
    faction: "naaz",
    source: "pok",
    text: "After you explore a planet, ready that planet.",
    imageURL: "/hover_images/techs/faction/prefab_arcologies.jpg",
  },
  {
    alias: "pi",
    name: "Predictive Intelligence",
    types: ["CYBERNETIC"],
    requirements: "Y",
    source: "pok",
    text: "At the end of your turn, you may exhaust this card to redistribute your command tokens.\nWhen you cast votes during the agenda phase, you may cast 3 additional votes; if you do, and the outcome you voted for is not resolved, exhaust this card.",
    imageURL: "/hover_images/techs/generic/predictive_intelligence.png",
    initials: "PI",
    shortName: "Predictive Intell'ce",
  },
  {
    alias: "pi_ac2",
    name: "Advacned Carrier II",
    types: ["UNITUPGRADE"],
    requirements: "GB",
    baseUpgrade: "cv2",
    faction: "sol",
    source: "project_pi",
    homebrewReplacesID: "ac2",
    text: "Cost 3, Combat 9, Move 2, Capacity 8\nSUSTAIN DAMANGE.",
  },
  {
    alias: "pi_bp",
    name: "Bioplasmosis",
    types: ["BIOTIC"],
    requirements: "G",
    faction: "arborec",
    source: "project_pi",
    homebrewReplacesID: "bio",
    text: "ACTION: You may exhaust this card to remove any number of ground forces from the board and place them on one or more planets you control in the same or adjacent system.",
  },
  {
    alias: "pi_bs",
    name: "Bio-Stems",
    types: ["BIOTIC"],
    requirements: "G",
    source: "project_pi",
    homebrewReplacesID: "bs",
    text: "You may exhaust this card at the end of your turn to ready 1 of your other technologies.",
  },
  {
    alias: "pi_cl2",
    name: "Crimson Legionnaire II",
    types: ["UNITUPGRADE"],
    requirements: "GG",
    baseUpgrade: "inf2",
    faction: "mahact",
    source: "project_pi",
    homebrewReplacesID: "cl2",
    text: "Cost 1(x2), Combat 7\nAfter this unit is destroyed, gain 1 commodity or convert 1 of your commodities to a trade good.\nWhen you gain control of a planet, if the number of infantry you have on the planet is greater than the planet's resource value, gain that planet readied.",
  },
  {
    alias: "pi_cm",
    name: "Chaos Mapping",
    types: ["BIOTIC"],
    requirements: "BB",
    faction: "saar",
    source: "project_pi",
    homebrewReplacesID: "cm",
    text: "Other players cannon move into asteroid fields that contain 1 or more of your units.\nAt the start of your turn during the action phase, you may produce 1 unit in an asteroid field that contains at least 1 of your units that has PRODUCTION.",
  },
  {
    alias: "pi_cr2",
    name: "Cruiser 2",
    types: ["UNITUPGRADE"],
    requirements: "GYB",
    source: "project_pi",
    homebrewReplacesID: "cr2",
    text: "Cost 2, Combat 6, Move 3, Capacity 1\nWhile your cruisers participate in combat, opponent's ships apply -1 to their combat rolls.",
  },
  {
    alias: "pi_cv2",
    name: "Carrier II",
    types: ["UNITUPGRADE"],
    requirements: "GB",
    source: "project_pi",
    homebrewReplacesID: "cv2",
    text: "Cost 3, Combat 9, Move 2, Capacity 6.",
  },
  {
    alias: "pi_da",
    name: "Dacxive Animators",
    types: ["BIOTIC"],
    requirements: "G",
    source: "project_pi",
    homebrewReplacesID: "da",
    text: "After 1 of your ground forces is destroyed during combat, you may place that unit on this card.\nAt the end of your turn, you may exhaust this card to place any number of units from it onto a planet you control.",
  },
  {
    alias: "pi_dn2",
    name: "Dreadnought II",
    types: ["UNITUPGRADE"],
    requirements: "RBY",
    source: "project_pi",
    homebrewReplacesID: "dn2",
    text: "Cost 4, Combat 5, Move 2, Capacity 1\nSUSTAIN DAMAGE, BOMBARDMENT 5\nWhen making a combat roll for this ship, each result of 9 or 10, before applying modifiers, produces 1 additional hits.",
  },
  {
    alias: "pi_dur",
    name: "Duranium Armor",
    types: ["WARFARE"],
    requirements: "RR",
    source: "project_pi",
    homebrewReplacesID: "da",
    text: 'During each round of combat, after you assign hits to your units, repair one participating damaged units that did not use SUSTAIN DAMAGE during this combat round.\nYou may exhaust this tech before assigning hits; Your units cannot be targeted by "Direct Hit" during this combat round.',
  },
  {
    alias: "pi_ers",
    name: "E-Res Siphons",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "jolnar",
    source: "project_pi",
    homebrewReplacesID: "ers",
    text: "After another player activates a system that contain 1 or more of your ships, gain 3 trade goods.",
  },
  {
    alias: "pi_exo2",
    name: "Exotrieme II",
    types: ["UNITUPGRADE"],
    requirements: "RBY",
    baseUpgrade: "dn2",
    faction: "sardakk",
    source: "project_pi",
    homebrewReplacesID: "exo2",
    text: "When making a combat roll for this ship, each result of 9 or 10, before applying modifiers, produces 1 additional hit.\nAfter a round of space combat, you may destroy this unit to destroy up to 2 ships in this system.",
  },
  {
    alias: "pi_ff2",
    name: "Fighter II",
    types: ["UNITUPGRADE"],
    requirements: "BY",
    source: "project_pi",
    homebrewReplacesID: "ff2",
    text: "Cost 1(x2), Combat 8, Move 2\nThis unit may move whiteout being transported.\nFighters in excess of your ship's capacity count against your fleet pool.",
  },
  {
    alias: "pi_ffac2",
    name: "Floating Factory II",
    types: ["UNITUPGRADE"],
    requirements: "YY",
    faction: "saar",
    baseUpgrade: "sd2",
    source: "project_pi",
    homebrewReplacesID: "ffac2",
    text: "Move 2, Capacity 6\nPRODUCTION 7\nThis unit is placed in a space area instead of on a planet.\nThis unit can move and retreat as if it were a ship.\nIf this unit is blockaded it is destroyed.",
  },
  {
    alias: "pi_fl",
    name: "Fleet Logistics",
    types: ["CYBERNETIC"],
    requirements: "YY",
    source: "project_pi",
    homebrewReplacesID: "fl",
    text: "Once during your turn in the action phase, after you perform an action, you may perform an additional action.",
  },
  {
    alias: "pi_gls",
    name: "Graviton Laser Systems",
    types: ["WARFARE"],
    requirements: "R",
    source: "project_pi",
    homebrewReplacesID: "gls",
    text: "You may exhaust this card before 1 or more of your units use SPACE CANNON; hits produced by those units must be assigned to non-fighter ships if able.\nYou may exhaust this card at the start of a space combat round; hits produced by your units this round must be assigned to non-fighter ships if able.",
  },
  {
    alias: "pi_gr",
    name: "Genetic Recombination",
    types: ["BIOTIC"],
    requirements: "G",
    faction: "mahact",
    source: "project_pi",
    homebrewReplacesID: "gr",
    text: "You may exhaust this card before another player casts votes; exhaust 3 of that players' planets to cast votes for the outcome of your choice. That player cannot exhaust any other planets on that agenda. If this card is not exhausted, you may cast 2 additional votes for each player that has abstained.",
  },
  {
    alias: "pi_hcf2",
    name: "Hybrid Crystal Fighter II",
    types: ["UNITUPGRADE"],
    requirements: "YB",
    baseUpgrade: "ff2",
    faction: "naalu",
    source: "project_pi",
    homebrewReplacesID: "hcf2",
    text: "Cost 1(x2), Combat 7, Move 2\nThis unit may move without being transported.\nEach fighter in excess of your ship's capacity counts as 1/2 of a ship against your fleet pool.",
  },
  {
    alias: "pi_hm",
    name: "Hyper Motabolism",
    types: ["BIOTIC"],
    requirements: "GG",
    source: "project_pi",
    homebrewReplacesID: "hm",
    text: "At the end of your turn, you may exhaust this card to gain 1 command token.",
  },
  {
    alias: "pi_htp",
    name: "Hegemonic Trade Policy",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "winnu",
    source: "project_pi",
    homebrewReplacesID: "htp",
    text: "Exhaust this card when 1 or more of your units use PRODUCTION; swap the reousrce and influence values of 1 planet your control during the use of PRODUCTION.",
  },
  {
    alias: "pi_ic",
    name: "Impluse Core",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "yin",
    source: "project_pi",
    homebrewReplacesID: "ic",
    text: "At the start of a space combat, you may destroy 1 of your cruisers or destroyers in the active system to produce 1 hit and assign it to 1 of your opponent's ships.",
  },
  {
    alias: "pi_ie",
    name: "Integrated Economy",
    types: ["CYBERNETIC"],
    requirements: "YYY",
    source: "project_pi",
    homebrewReplacesID: "ie",
    text: "When your perform a tactical action: Treat each planet in the active system as if they are a unit with PRODUCTION X, where X is equal to the sum of the planet's resource and influence values. Planets granted PRODUCTION by this tech may only produce fighters or infantry,",
  },
  {
    alias: "pi_inf2",
    name: "Infantry II",
    types: ["UNITUPGRADE"],
    requirements: "GG",
    source: "project_pi",
    homebrewReplacesID: "inf2",
    text: "Cost 1(x2), Combat 7\nWhen you gain control of a planet, if the number of infantry you have on the planet is greater than the planet's resource value, gain that planet readied.",
  },
  {
    alias: "pi_is",
    name: "Inheritance Sysytems",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "l1z1x",
    source: "project_pi",
    homebrewReplacesID: "is",
    text: "You may exhaust this card when you research a technology; ignore all of that technology's prerequisites.",
  },
  {
    alias: "pi_it",
    name: "Instinct Training",
    types: ["BIOTIC"],
    requirements: "G",
    faction: "xxcha",
    source: "project_pi",
    homebrewReplacesID: "it",
    text: "You may exhaust this card when another player plays an action card; cancel that action card.",
  },
  {
    alias: "pi_l4",
    name: "L4 Disruptors",
    types: ["CYBERNETIC"],
    faction: "letnev",
    source: "project_pi",
    homebrewReplacesID: "l4",
    text: "During an invasion, your opponents cannot use SPACE CANNON against your units.",
  },
  {
    alias: "pi_lgf",
    name: "Lazax Gate Folding",
    types: ["PROPULSION"],
    requirements: "B",
    faction: "winnu",
    source: "project_pi",
    homebrewReplacesID: "lgf",
    text: "During your tactical actions, treat Mecatol Rex's system as if it contains each non-delta wormhole.",
  },
  {
    alias: "pi_lw2",
    name: "Letani Warrior II",
    types: ["UNITUPGRADE"],
    requirements: "GG",
    baseUpgrade: "inf2",
    faction: "arborec",
    source: "project_pi",
    homebrewReplacesID: "lw2",
    text: "Cost 1, Combat 7\nPRODUCTION 2\nWhen you gain control of a planet, if the number of infantry you have on the planet is greater than the planet's resource value, gain that planet readied.",
  },
  {
    alias: "pi_m2",
    name: "Memoria II",
    types: ["UNITUPGRADE"],
    requirements: "RGB",
    faction: "nomad",
    source: "project_pi",
    homebrewReplacesID: "m2",
    text: "Cost 8, Combat 5(x2), Move 2, Capacity 6\nANTI-FIGHTER BARRAGE 5(x3), BOMBARDMENT 5\nYou may treat this unit as if it were adjacent to systems that contain 1 or more of your mechs.",
  },
  {
    alias: "pi_mdg",
    name: "Magen Defence Grid",
    types: ["WARFARE"],
    requirements: "R",
    source: "project_pi",
    homebrewReplacesID: "md",
    text: "After a player activates a system that contains a planet you control, each planet you control in that system gains the SPACE CANNON 6 ability as if it were a unit until the end of that player's turn.",
  },
  {
    alias: "pi_nf",
    name: "Nullification Field",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "xxcha",
    source: "project_pi",
    homebrewReplacesID: "nf",
    text: "After another player activated a system that contains 1 or more of your ships, you may exhaust this card and spend 1 token from your strategy pool; immediately end that player's turn.",
  },
  {
    alias: "pi_nm",
    name: "Neural Motivator",
    types: ["BIOTIC"],
    source: "project_pi",
    homebrewReplacesID: "nm",
    text: "At the end of your turn, you may exhaust this card to draw 1 action card.",
  },
  {
    alias: "pi_pb",
    name: "Production Biomes",
    types: ["BIOTIC"],
    requirements: "G",
    faction: "hacan",
    source: "project_pi",
    homebrewReplacesID: "pm",
    text: "ACTION: Exhaust this card to gain 3 trade goods and choose 1 other player; that player gains 1 trade good.",
  },
  {
    alias: "pi_pi",
    name: "Predictive Intelligence",
    types: ["CYBERNETIC"],
    requirements: "YY",
    source: "project_pi",
    homebrewReplacesID: "pi",
    text: "At the end of your turn, you may exhaust this card to redistribute your command tokens.\nWhen you redistribute your command tokens, you may convert all your commodities into trade goods.",
  },
  {
    alias: "pi_ps",
    name: "Plasma Scoring",
    types: ["WARFARE"],
    source: "project_pi",
    homebrewReplacesID: "ps",
    text: "When 1 or more of your units make a roll for a unit ability, you may choose 1 of those units to roll 1 additional die.",
  },
  {
    alias: "pi_psy",
    name: "Psychoarchaeology",
    types: ["BIOTIC"],
    source: "project_pi",
    homebrewReplacesID: "pa",
    text: "You may exhaust this card to use technology specialties on planets you control without exhausting them, even if those planets are exhausted.\nYou may exhaust this card at the end of your turn to ready 1 of your planets that has a technology specialty.",
  },
  {
    alias: "pi_pws2",
    name: "Protoype War Sun II",
    types: ["UNITUPGRADE"],
    requirements: "RRRY",
    faction: "muaat",
    baseUpgrade: "ws",
    source: "project_pi",
    homebrewReplacesID: "pws2",
    text: "Cost 10, Combat 3(x3), Move 3, Capacity 8\nBOMBARDMENT 3(x3), SUSTAIN DAMAGE\nOther players units in this system lose the PLANETARY SHIELD ability.",
  },
  {
    alias: "pi_sar",
    name: "Self Assembly Routines",
    types: ["CYBERNETIC"],
    requirements: "Y",
    source: "project_pi",
    homebrewReplacesID: "sar",
    text: "After 1 or more of your units use PRODUCTION, you may exhaust this card to place 1 mech from your reinforcements on a planet you control in that system.\nAfter 1 of your mechs is destroyed, gain 1 trade good.",
  },
  {
    alias: "pi_sd2",
    name: "Space Dock II",
    types: ["UNITUPGRADE"],
    requirements: "YY",
    source: "project_pi",
    homebrewReplacesID: "sd2",
    text: "PRODUCTION X\nThis unit's PRODUCTION value is equal to 5 more than the resource value of this planet.\nUp to 6 fighters in this system do not count against your ship's capacity.\nWhen you use this unit's PRODUCTION ability, you may place the produced units on any planet in this system.",
  },
  {
    alias: "pi_sdn2",
    name: "Super Dreadnaught II",
    types: ["UNITUPGRADE"],
    requirements: "RBY",
    baseUpgrade: "dn2",
    faction: "l1z1x",
    source: "project_pi",
    homebrewReplacesID: "sdn2",
    text: "Cost 4, Combat 4, Move 2, Capacity 2\nBOMBARDMENT 4, SUSTAIN DAMAGE\nWhen making a combat roll for this ship, each result of 9 or 10, before applying modifiers, produces 1 additional hit.\nOther players' units in this system lose PLANETARY SHIELD.",
  },
  {
    alias: "pi_se2",
    name: "Saturn Engine II",
    types: ["UNITUPGRADE"],
    requirements: "GYB",
    baseUpgrade: "cr2",
    faction: "titans",
    source: "project_pi",
    homebrewReplacesID: "se2",
    text: "Cost 2, Combat 6, Move 3, Capacity 2\nSUSTAIN DAMAGE.",
  },
  {
    alias: "pi_so2",
    name: "Spec Ops II",
    types: ["UNITUPGRADE"],
    requirements: "GG",
    baseUpgrade: "inf2",
    faction: "sol",
    source: "project_pi",
    homebrewReplacesID: "so2",
    text: "Cost 1(x2), Combat 6\nWhen you gain control of a planet, if the number of infantry you have on the planet is greater than the planet's resource value, gain that planet readied.",
  },
  {
    alias: "pi_st",
    name: "Sarween Tools",
    types: ["CYBERNETIC"],
    source: "project_pi",
    homebrewReplacesID: "st",
    text: "When you produce 1 or more units, reduce the combined cost of the produced units by 1.",
  },
  {
    alias: "pi_td",
    name: "Tranist Diodes",
    types: ["PROPULSION"],
    requirements: "BB",
    source: "project_pi",
    homebrewReplacesID: "td",
    text: "You may exhaust this card at the start of your turn during the action phase; remove up to 4 of your ground forces from the game board and place them on 1 or more planets you control.",
  },
  {
    alias: "pi_tp",
    name: "Transparasteel Plating",
    types: ["BIOTIC"],
    requirements: "G",
    faction: "yssaril",
    source: "project_pi",
    homebrewReplacesID: "tp",
    text: "During your turn of the action phase, players that have passed cannot play action cards, exhaust technology cards, or use the SPACE CANNON ability of their units against your units.",
  },
  {
    alias: "pi_vpw",
    name: "Valkyrie Particle Weave",
    types: ["WARFARE"],
    requirements: "R",
    faction: "sardakk",
    source: "project_pi",
    homebrewReplacesID: "vpw",
    text: "After making combat rolls during a round of ground combat, if your opponent produced 1 or more hits, you produce 1 additional hit.",
  },
  {
    alias: "pi_wg",
    name: "Wormhole Generator",
    types: ["PROPULSION"],
    requirements: "G",
    faction: "ghost",
    source: "project_pi",
    homebrewReplacesID: "wg",
    text: "ACTION: Exhaust this card to place or move a Creuss wormhole token into either a system that contains a planet you control or a non-home system that does not contain another player's ships.",
  },
  {
    alias: "pi_ws",
    name: "War Sun",
    types: ["UNITUPGRADE"],
    requirements: "YRRR",
    source: "project_pi",
    homebrewReplacesID: "ws",
    text: "Cost 12, Combat 3(x3), Move 2, Capacity 8\nSUSTAIN DAMAGE, BOMBARDMENT 3(x3)\nOther players' units in this system lose PLANETARY SHIELD.",
  },
  {
    alias: "pi_x89",
    name: "X-89 Bacterial Weapon",
    types: ["BIOTIC"],
    requirements: "GGG",
    source: "project_pi",
    homebrewReplacesID: "x89",
    text: "At the start of your turn, exhaust this card and remove one of your structures from the board to choose a planet controlled by another player that is in or adjacent to a system where you have ships. Destroy all ground forces on that planet.",
  },
  {
    alias: "planesplitter-firm",
    name: "Planesplitter (Firmament)",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "firmament",
    text: "When you gain this card, put The Fracture into play.\nFlip this card if the Obsidian faction is in play.",
    source: "thunders_edge",
  },
  {
    alias: "planesplitter-obs",
    name: "Planesplitter (Obsidian)",
    types: ["CYBERNETIC"],
    requirements: "null",
    faction: "obsidian",
    text: "When you perform a strategic action, you may move an ingress token into a system that contains or is adjacent to your units.\nThis technology cannot be researched.",
    source: "thunders_edge",
  },
  {
    alias: "pm",
    name: "Production Biomes",
    types: ["BIOTIC"],
    requirements: "GG",
    faction: "hacan",
    source: "base",
    text: "ACTION: Exhaust this card and spend 1 token from your strategy pool to gain 4 trade goods and choose 1 other player; that player gains 2 trade goods.",
    shrinkName: true,
    imageURL: "/hover_images/techs/faction/production_biomes.jpg",
  },
  {
    alias: "precisiontargeting",
    name: "Precision Targeting",
    types: ["WARFARE"],
    requirements: "R",
    source: "ignis_aurora",
    text: "At the start of a round of combat you may exhaust this card to cause your opponents' units participating in that combat that have SUSTAIN DAMAGE to become damaged.",
  },
  {
    alias: "proxima",
    name: "Proxima Targeting V1",
    types: ["WARFARE"],
    requirements: "R",
    faction: "bastion",
    text: "Cancel 1 hit produced by BOMBARDMENT rolls made against your ground forces for each of your galvanized units present. \nAt the start of a round of ground combat, you may resolve BOMBARDMENT 8 (x3) against your opponent's ground forces; if you do, make an identical roll against your ground forces.",
    source: "thunders_edge",
  },
  {
    alias: "ps",
    name: "Plasma Scoring",
    types: ["WARFARE"],
    source: "base",
    text: "When 1 or more of your units use BOMBARDMENT or SPACE CANNON, 1 of those units may roll 1 additional die.",
    imageURL: "/hover_images/techs/generic/plasma_scoring.png",
    initials: "PS",
  },
  {
    alias: "pws2",
    name: "Prototype War Sun II",
    types: ["UNITUPGRADE"],
    requirements: "RRRY",
    faction: "muaat",
    baseUpgrade: "ws",
    source: "base",
    text: "Cost 10, Combat 3(x3), Move 3, Capacity 6\nSUSTAIN DAMAGE, BOMBARDMENT 3(x3)\nOther players' units in this system lose PLANETARY SHIELD.",
    imageURL: "/hover_images/techs/faction/prototype_war_sun_2.jpg",
  },
  {
    alias: "pyrrhic_generation",
    name: "Pyrrhic Generation",
    types: ["BIOTIC"],
    requirements: "GG",
    source: "cpti",
    text: "ACTION: Exhaust this card to purge 1 units from your reinforcements. Then, place 1 unit of the same type from your reinforcements into a system that contains your units and no other player's ships.\n\nACTION: Exhaust this card to purge 1 of your leaders. Then, draw 1 relic.",
  },
  {
    alias: "qdn",
    name: "Quantum Datahub Node",
    types: ["CYBERNETIC"],
    requirements: "YYY",
    faction: "hacan",
    source: "base",
    text: "At the end of the strategy phase, you may spend 1 token from your strategy pool and give another player 3 of your trade goods. If you do, give 1 of your strategy cards to that player and take 1 of their strategy cards.",
    shrinkName: true,
    shortName: "Quantum\nDatahub Nd",
    imageURL: "/hover_images/techs/faction/quantum_datahub_node.jpg",
  },
  {
    alias: "quantum_transport",
    name: "Quantum Transport",
    types: ["PROPULSION"],
    requirements: "BB",
    source: "cpti",
    text: "Apply +2 to each of your units capacity values that have one.\n\nAfter you transport infrantry or fighters, you may exhaust this card to replace any of those units with an infantry or fighter.",
  },
  {
    alias: "radical",
    name: "Radical Advancement",
    types: ["BIOTIC"],
    requirements: "G",
    faction: "deepwrought",
    text: "At the start of the status phase, you may replace one of your non-unit upgrade technologies with a technology of the same color that has exactly 1 more prerequisite.",
    source: "thunders_edge",
  },
  {
    alias: "saeracr",
    name: "Avenger II ",
    types: ["UNITUPGRADE"],
    requirements: "GRY",
    baseUpgrade: "cr2",
    faction: "saera",
    source: "eronous",
    text: "After this unit is destroyed, roll 1 die. If the result is 6 or greater, place the unit in your home system if able. 2, 6, 3, 1; Abilities: ANTI-FIGHTER BARRAGE 6",
  },
  {
    alias: "saeray",
    name: "Hielas Array",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "saera",
    source: "eronous",
    text: "After a player activates a system that contains 1 or more of another player’s units, you may exhaust this card and spend 1 token from your strategy pool; immediately end that player's turn.",
  },
  {
    alias: "sap",
    name: "Sappers",
    types: ["CYBERNETIC"],
    requirements: "YYY",
    faction: "enclave",
    source: "pbd2000",
    text: "Secret",
  },
  {
    alias: "sar",
    name: "Self-Assembly Routines",
    types: ["WARFARE"],
    requirements: "R",
    source: "pok",
    text: "After 1 or more of your units use PRODUCTION, you may exhaust this card to place 1 mech from your reinforcements on a planet you control in that system.\nAfter 1 of your mechs is destroyed, gain 1 trade good.",
    imageURL: "/hover_images/techs/generic/self_assembly_routines.png",
    initials: "SAR",
    shortName: "Self-Ass. Routines",
  },
  {
    alias: "sc",
    name: "Supercharge",
    types: ["WARFARE"],
    requirements: "R",
    faction: "naaz",
    source: "pok",
    text: "At the start of a combat round, you may exhaust this card to apply +1 to the result of each of your unit's combat rolls during this combat round.",
    shortName: "Super-\ncharge",
    imageURL: "/hover_images/techs/faction/supercharge.jpg",
  },
  {
    alias: "scc",
    name: "Spatial Conduit Cylinders",
    types: ["PROPULSION"],
    requirements: "BB",
    faction: "jolnar",
    source: "base",
    text: "You may exhaust this card after you activate a system that contains 1 or more of your units; that system is adjacent to all other systems that contain 1 or more of your units during this activation.",
    shrinkName: false,
    shortName: "Spatial\nCond't Cyl.",
    imageURL: "/hover_images/techs/faction/spacial_conduit_cylinder.jpg",
  },
  {
    alias: "sd2",
    name: "Space Dock II",
    types: ["UNITUPGRADE"],
    requirements: "YY",
    source: "base",
    text: "PRODUCTION X\nThis unit's PRODUCTION value is equal to 4 more than the resource value of this planet.\nUp to 3 fighters in this system do not count against your ships' capacity.",
    imageURL: "/hover_images/techs/generic/spacedock_2.jpg",
  },
  {
    alias: "sdn",
    name: "Scanlink Drone Network",
    types: ["CYBERNETIC"],
    source: "pok",
    text: "When you activate a system, you may explore 1 planet in that system which contains 1 or more of your units.",
    imageURL: "/hover_images/techs/generic/scanlink_drone_network.png",
    initials: "SDN",
    shortName: "Scanlink Drone Net.",
  },
  {
    alias: "sdn2",
    name: "Super Dreadnought II",
    types: ["UNITUPGRADE"],
    requirements: "BBY",
    faction: "l1z1x",
    baseUpgrade: "dn2",
    source: "base",
    text: 'Cost 4, Combat 4, Move 2, Capacity 2\nSUSTAIN DAMAGE, BOMBARDMENT 4\nThis unit cannot be destroyed by "Direct Hit" action cards.',
    imageURL: "/hover_images/techs/faction/superdreadnought_2.jpg",
  },
  {
    alias: "se2",
    name: "Saturn Engine II",
    types: ["UNITUPGRADE"],
    requirements: "GYR",
    faction: "titans",
    baseUpgrade: "cr2",
    source: "pok",
    text: "Cost 2, Combat 6, Move 3, Capacity 2\nSUSTAIN DAMAGE",
    imageURL: "/hover_images/techs/faction/saturn_engine_2.jpg",
  },
  {
    alias: "shadowssd",
    name: "Umbrae Ring II",
    types: ["UNITUPGRADE"],
    requirements: "YY",
    baseUpgrade: "sd2",
    faction: "shadows",
    source: "eronous",
    text: "This unit may move without being transported. Fighters in excess of your ships' capacity count against your fleet pool. 1(x2), 7, 2, -;",
  },
  {
    alias: "shadowsy",
    name: "Byophysma",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "shadows",
    source: "eronous",
    text: "When you produce units in a system, you may exhaust this card to place up to 1 non-fighter ship you produce in an adjacent system that does not contain another player's ships.",
  },
  {
    alias: "sigma_cow",
    name: "The Changer of Ways",
    types: ["WARFARE"],
    requirements: "RR",
    faction: "nomad",
    source: "flagshipping",
    text: "At the start of any space combat, you may choose 1 participating unit with the SUSTAIN DAMAGE ability; repair or damage that unit.",
    shortName: "Changer of Ways",
    imageURL: "/hover_images/techs/misc_homebrew/changer_of_ways.png",
  },
  {
    alias: "so",
    name: "Salvage Operations",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "mentak",
    source: "base",
    text: "After you win or lose a space combat, gain 1 trade good; if you won the combat, you may also produce 1 ship in that system of any ship type that was destroyed during the combat.",
    shortName: "Salvage Operat'ns",
    imageURL: "/hover_images/techs/faction/salvage_operations.jpg",
  },
  {
    alias: "so2",
    name: "Spec Ops II",
    types: ["UNITUPGRADE"],
    requirements: "GG",
    faction: "sol",
    baseUpgrade: "inf2",
    source: "base",
    text: "Cost 1(x2), Combat 6\nAfter this unit is destroyed, roll 1 die. If the result is 5 or greater, place the unit on this card. At the start of your next turn, place each unit that is on this card on a planet you control in your home system.",
    imageURL: "/hover_images/techs/faction/spec_ops_2.jpg",
  },
  {
    alias: "specialclearances",
    name: "Special Clearances",
    types: ["CYBERNETIC"],
    requirements: "YYY",
    faction: "keleresplus",
    source: "keleresplus",
    text: "At the start of a player's turn, ou may spend 1 trade good or 1 commodity to allow that player to choose any number of laws in play. Those laws are treated as blank until the end of that player's turn.",
  },
  {
    alias: "specialforces",
    name: "Special Forces",
    types: ["CYBERNETIC"],
    requirements: "YY",
    faction: "drahn",
    source: "keleresplus",
    text: "At the start of a space combat in which you are the defender: You may exhaust this card and spend 1 command token from your strategy pool to force your opponent to declare a retreat.",
  },
  {
    alias: "sr",
    name: "Sling Relay",
    types: ["PROPULSION"],
    requirements: "B",
    source: "pok",
    text: "ACTION: Exhaust this card to produce 1 ship in any system that contains one of your space docks.",
    imageURL: "/hover_images/techs/generic/sling_relay.png",
    initials: "SR",
  },
  {
    alias: "st",
    name: "Sarween Tools",
    types: ["CYBERNETIC"],
    source: "base",
    text: "When 1 or more of your units use PRODUCTION, reduce the combined cost of the produced units by 1.",
    imageURL: "/hover_images/techs/generic/sarween_tools.png",
    initials: "ST",
  },
  {
    alias: "stco",
    name: "Stellar Corridors",
    types: ["PROPULSION"],
    requirements: "BB",
    faction: "raven",
    source: "pbd2000",
    text: "At the start of your turn you may exhaust this card to remove 1 of your command tokens from a system that does not contain a planet owned by another player or another player's units.",
  },
  {
    alias: "stellarcorridors",
    name: "Stellar Corridors",
    types: ["PROPULSION"],
    requirements: "BB",
    source: "ignis_aurora",
    text: "At the end of your turn you may remove 1 of your command tokens from a system that does not contain a planet owned by another player. If you do, return this card to your technology deck.",
  },
  {
    alias: "subatomic",
    name: "Subatomic Splicer",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "crimson",
    text: "When one of your ships is destroyed, you may produce a ship of the same type at a space dock in your home system.",
    source: "thunders_edge",
  },
  {
    alias: "swa2",
    name: "Strike Wing Alpha II",
    types: ["UNITUPGRADE"],
    requirements: "RR",
    faction: "argent",
    baseUpgrade: "dd2",
    source: "pok",
    text: "Cost 1, Combat 7, Move 2, Capacity 1\nANTI-FIGHTER BARRAGE 6(x3)\nWhen this unit uses ANTI-FIGHTER BARRAGE, each result of 9 or 10 also destroys 1 of your opponents infantry in the space area of the active system.",
    imageURL: "/hover_images/techs/faction/strike_wing_alpha_2.jpg",
  },
  {
    alias: "syntheticreclamation",
    name: "Synthetic Reclamation",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "arborec",
    source: "ignis_aurora",
    text: "When 1 or more of your units produces units, you may remove infantry from that system. For each infantry that you remove, reduce the combined cost of the produced units by 1.",
  },
  {
    alias: "tac",
    name: "Tactical Retreat",
    types: ["PROPULSION"],
    requirements: "B",
    faction: "syndicate",
    source: "pbd2000",
    text: "Secret",
  },
  {
    alias: "tcs",
    name: "Temporal Command Suite",
    types: ["CYBERNETIC"],
    requirements: "Y",
    faction: "nomad",
    source: "pok",
    text: "After any player's agent becomes exhausted, you may exhaust this card to ready that agent; if you ready another player's agent, you may perform a transaction with that player.",
    shrinkName: true,
    shortName: "Temporal\nC'mm'nd St.",
    imageURL: "/hover_images/techs/faction/temporal_command_suite.jpg",
  },
  {
    alias: "td",
    name: "Transit Diodes",
    types: ["CYBERNETIC"],
    requirements: "YY",
    source: "base",
    text: "You may exhaust this card at the start of your turn during the action phase; remove up to 4 of your ground forces from the game board and place them on 1 or more planets you control.",
    imageURL: "/hover_images/techs/generic/transit_diodes.png",
    initials: "TD",
  },
  {
    alias: "terradesic_dividends",
    name: "Terradesic Dividends",
    types: ["CYBERNETIC"],
    requirements: "YYY",
    source: "cpti",
    text: "After an objective is revealed, you may exhaust any number of planets to gain trade goods equal to 1 less than their combined resource or combined influence values.",
  },
  {
    alias: "tf-abundance",
    name: "Abundance",
    types: ["CYBERNETIC"],
    text: "When 1 or more of your units use PRODUCTION, you may gain 1 trade good.",
    source: "twilights_fall",
    faction: "titans",
  },
  {
    alias: "tf-ahl",
    name: "Aerie Hololattice",
    types: ["CYBERNETIC"],
    text: "Other players cannot move ships through systems that contain your structures.\nEach planet that contains 1 or more of your structures gains the PRODUCTION 1 ability as if it were a unit.",
    source: "twilights_fall",
    faction: "argent",
  },
  {
    alias: "tf-amalgamation",
    name: "Amalgamation",
    types: ["WARFARE"],
    text: "Capture your opponent's non-structure units that are destroyed during combat.\nWhen you produce a unit, you may return 1 captured unit of that type to produce that unit without spending resources.",
    source: "twilights_fall",
    faction: "cabal",
  },
  {
    alias: "tf-ambush",
    name: "Ambush",
    types: ["WARFARE"],
    text: "At the start of a space combat: You may roll 1 die for each of up to 2 of your cruisers or destroyers in the system. For each result equal to or greater than that ship's combat value, produce 1 hit; your opponent must assign it to 1 of their ships.",
    source: "twilights_fall",
    faction: "mentak",
  },
  {
    alias: "tf-armada",
    name: "Armada",
    types: ["WARFARE"],
    text: "The maximum number of non-fighter ships you can have in each system is equal to 2 more than the number of tokens in your fleet pool.",
    source: "twilights_fall",
    faction: "letnev",
  },
  {
    alias: "tf-as",
    name: "Aetherstream",
    types: ["PROPULSION"],
    text: "After you or one of your neighbors activates a system that is adjacent to an anomaly: You may apply +1 to the move value of all of that player's ships during this tactical action",
    source: "twilights_fall",
    faction: "empyrean",
  },
  {
    alias: "tf-asn",
    name: "Agency Supply Network",
    types: ["CYBERNETIC"],
    text: "Once per action, when you resolve a unit's PRODUCTION ability: You may resolve another of your unit's PRODUCTION abilities in any system.",
    source: "twilights_fall",
    faction: "keleres",
  },
  {
    alias: "tf-assimilate",
    name: "Assimilate",
    types: ["CYBERNETIC"],
    text: "When you gain control of a planet: Replace each structure on that planet with a matching unit from your reinforcements.",
    source: "twilights_fall",
    faction: "l1z1x",
  },
  {
    alias: "tf-awaken",
    name: "Awaken",
    types: ["PROPULSION"],
    text: "After you activate a system that contains 1 or more of your infantry, you may replace 1 of those infantry with a PDS from your reinforcements.",
    source: "twilights_fall",
    faction: "titans",
  },
  {
    alias: "tf-bio",
    name: "Bioplasmosis",
    types: ["BIOTIC"],
    text: "At the end of the status phase: You may remove any number of ground forces from planets you control and place them on 1 or more planets you control in the same or adjacent systems.",
    source: "twilights_fall",
    faction: "arborec",
  },
  {
    alias: "tf-biosyntheticsynergy",
    name: "Bio-Synthetic Synergy",
    types: ["BIOTIC"],
    text: 'Your action cards cannot be canceled by "Shatter" action cards.\nYour other abilities cannot be taken, purged, or discarded by other players and cannot have singularity tokens placed on them.',
    source: "twilights_fall",
    faction: "bastion",
  },
  {
    alias: "tf-chaos",
    name: "Chaos Mapping",
    types: ["CYBERNETIC"],
    text: "At the start of your turn in the action phase: You may produce 1 unit in a system that contains at least 1 of your units that has PRODUCTION.",
    source: "twilights_fall",
    faction: "saar",
  },
  {
    alias: "tf-couriertransport",
    name: "Courier Transport",
    types: ["PROPULSION"],
    text: "After you activate a system: You may move your structures from adjacent systems that do not contain your command tokens onto planets you control in the active system.",
    source: "twilights_fall",
    faction: "ralnel",
  },
  {
    alias: "tf-crafty",
    name: "Crafty",
    types: ["BIOTIC"],
    text: "You can have any number of action cards in your hand.  Game effects cannot prevent you from using this ability.\nDuring your turn of the action phase, players that have passed cannot play action cards.",
    source: "twilights_fall",
    faction: "yssaril",
  },
  {
    alias: "tf-crucible",
    name: "Crucible",
    types: ["PROPULSION"],
    text: "Your ships do not roll for gravity rifts; apply an additional +1 to the move values of your ships that would move out of or through a gravity rift instead.",
    source: "twilights_fall",
    faction: "cabal",
  },
  {
    alias: "tf-devotion",
    name: "Devotion",
    types: ["PROPULSION"],
    text: "After each space combat round: You may destroy 1 of your cruisers or destroyers in the active system to produce 1 hit and assign it to 1 of your opponent's ships in that system.",
    source: "twilights_fall",
    faction: "yin",
  },
  {
    alias: "tf-dimensionaltear",
    name: "Dimensional Tear",
    types: ["PROPULSION"],
    text: "Systems that contain your space docks are also gravity rifts; your ships do not roll for these gravity rifts.\nPlace a dimensional tear token beneath your space docks as a reminder.",
    source: "twilights_fall",
    faction: "cabal",
  },
  {
    alias: "tf-distantsuns",
    name: "Distant Suns",
    types: ["PROPULSION"],
    text: "When you explore a planet that contains 1 of your mechs: You may draw 1 additional card; choose 1 to resolve and discard the rest.",
    source: "twilights_fall",
    faction: "naaz",
  },
  {
    alias: "tf-ds",
    name: "Dimensional Splicer",
    types: ["WARFARE"],
    text: "At the start of a space combat in a system that contains a wormhole and 1 or more of your ships: You may produce 1 hit and assign it to 1 of your opponent's ships.",
    source: "twilights_fall",
    faction: "ghost",
  },
  {
    alias: "tf-entropicharvest",
    name: "Entropic Harvest",
    types: ["CYBERNETIC"],
    text: "At the end of any player's combat: Gain 1 commodity or convert 1 of your commodities to a trade good.",
    source: "twilights_fall",
    faction: "crimson",
  },
  {
    alias: "tf-ers",
    name: "E-Res Siphons",
    types: ["CYBERNETIC"],
    text: "After another player activates a system that contains 1 or more of your ships: Gain 4 trade goods.",
    source: "twilights_fall",
    faction: "jolnar",
  },
  {
    alias: "tf-fabrication",
    name: "Fabrication",
    types: ["CYBERNETIC"],
    text: "ACTION: Either purge 2 of your relic fragments of the same type to gain 1 relic or purge 1 of your relic fragments to gain 1 command token.",
    source: "twilights_fall",
    faction: "naaz",
  },
  {
    alias: "tf-fl",
    name: "Fleet Logistics",
    types: ["PROPULSION"],
    text: "During each of your turns of the action phase, you may perform 2 actions instead of 1.",
    source: "twilights_fall",
    faction: "keleres",
  },
  {
    alias: "tf-foresight",
    name: "Foresight",
    types: ["PROPULSION"],
    text: "After another player moves ships into a system that contains 1 or more of your ships: You may place 1 token from your reinforcements in an adjacent system that does not contain another player's ships; move your ships from the active system into that system.",
    source: "twilights_fall",
    faction: "naalu",
  },
  {
    alias: "tf-futurepath",
    name: "Future Path",
    types: ["PROPULSION"],
    text: 'When you gain trade goods from choosing a previously unchosen strategy card or from the "Amicus" strategy card: Triple those trade goods.',
    source: "twilights_fall",
    faction: "nomad",
  },
  {
    alias: "tf-geneticresearch",
    name: "Genetic Research",
    types: ["BIOTIC"],
    text: "At the start of the status phase: Choose a technology color and gain trade goods equal to the number of abilities you own that have that technology color.",
    source: "twilights_fall",
    faction: "deepwrought",
  },
  {
    alias: "tf-guildships",
    name: "Guild Ships",
    types: ["PROPULSION"],
    text: "You can negotiate transactions with players who are not your neighbor.\nYou can exchange action cards with other players as part of transactions.",
    source: "twilights_fall",
    faction: "hacan",
  },
  {
    alias: "tf-harrow",
    name: "Harrow",
    types: ["WARFARE"],
    text: "At the end of each round of ground combat: Your ships in the active system may use their bombardment abilities against your opponent's ground forces on the planet.",
    source: "twilights_fall",
    faction: "l1z1x",
  },
  {
    alias: "tf-htp",
    name: "Hegemonic Trade Policy",
    types: ["CYBERNETIC"],
    text: "When 1 or more of your units use PRODUCTION: You may swap the resource and influence values of 1 planet you control during this use of PRODUCTION.",
    source: "twilights_fall",
    faction: "winnu",
  },
  {
    alias: "tf-indoctrination",
    name: "Indoctrination",
    types: ["BIOTIC"],
    text: "At the start of a ground combat: You may spend 2 influence to replace 1 of your opponent's participating infantry with 1 infantry from your reinforcements.",
    source: "twilights_fall",
    faction: "yin",
  },
  {
    alias: "tf-inheritancesystems",
    name: "Inheritance Systems",
    types: ["CYBERNETIC"],
    text: "During the status phase: Draw 1 additional action card and gain 1 additional command token.",
    source: "twilights_fall",
    faction: "l1z1x",
  },
  {
    alias: "tf-it",
    name: "Instinct Training",
    types: ["BIOTIC"],
    text: "When another player plays an action card: You may spend 1 token from your strategy pool to cancel that action card.",
    source: "twilights_fall",
    faction: "xxcha",
  },
  {
    alias: "tf-lazaxgatefolding",
    name: "Lazax Gate Folding",
    types: ["PROPULSION"],
    text: "During your tactical actions: Treat systems that contain legendary planets you do not control as if they contain both an alpha and beta wormhole.",
    source: "twilights_fall",
    faction: "winnu",
  },
  {
    alias: "tf-liberate",
    name: "Liberate",
    types: ["WARFARE"],
    text: "When you gain control of a planet: Ready that planet if it contains a number of your infantry equal to or greater than that planet's resource value; otherwise, place 1 infantry on that planet.",
    source: "twilights_fall",
    faction: "bastion",
  },
  {
    alias: "tf-mc",
    name: "Mirror Computing",
    types: ["CYBERNETIC"],
    text: "When you spend trade goods: Each trade good is worth 2 resources or influence instead of 1.",
    source: "twilights_fall",
    faction: "mentak",
  },
  {
    alias: "tf-mitosis",
    name: "Mitosis",
    types: ["BIOTIC"],
    text: "At the start of the status phase: Place 1 infantry from your reinforcements on any planet you control.",
    source: "twilights_fall",
    faction: "arborec",
  },
  {
    alias: "tf-mr",
    name: "Magmus Reactor",
    types: ["PROPULSION"],
    text: "Your units can move into and through supernovas.\nEach supernova that does not contain other players' units and each system that contains 1 or more of your war suns gains PRODUCTION 5 as if it were one of your units.",
    source: "twilights_fall",
    faction: "muaat",
  },
  {
    alias: "tf-munitions",
    name: "Munitions Reserves",
    types: ["WARFARE"],
    text: "At the start of each round of space combat: You may spend 2 trade goods; you may re-roll any number of your dice during that combat round.",
    source: "twilights_fall",
    faction: "letnev",
  },
  {
    alias: "tf-nanomachines",
    name: "Nanomachines",
    types: ["WARFARE"],
    text: "ACTION: Exhaust this card to place 1 PDS on a planet you control.\nACTION: Exhaust this card to repair all of your damaged units.\nACTION: Exhaust this card and discard an action card to draw 1 action card.",
    source: "twilights_fall",
    faction: "ralnel",
  },
  {
    alias: "tf-nes",
    name: "Non-Euclidean Shielding",
    types: ["WARFARE"],
    text: "When 1 of your units uses SUSTAIN DAMAGE: Cancel 2 hits instead of 1.",
    source: "twilights_fall",
    faction: "letnev",
  },
  {
    alias: "tf-neuralparasite",
    name: "Neural Parasite",
    types: ["BIOTIC"],
    text: "At the start of your turn: Destroy 1 of another player's infantry in or adjacent to a system that contains your infantry.",
    source: "twilights_fall",
    faction: "obsidian",
  },
  {
    alias: "tf-nf",
    name: "Nullification Field",
    types: ["CYBERNETIC"],
    text: "When another player activates a system that contains 1 or more of your ships: You may exhaust this card and spend 1 token from your strategy pool to immediately end that player's turn.",
    source: "twilights_fall",
    faction: "xxcha",
  },
  {
    alias: "tf-ng",
    name: "Neuroglaive",
    types: ["WARFARE"],
    text: "After another player activates a system that contains 1 or more of your ships: That player removes 1 token from their fleet pool and returns it to their reinforcements.",
    source: "twilights_fall",
    faction: "naalu",
  },
  {
    alias: "tf-nomadic",
    name: "Nomadic",
    types: ["PROPULSION"],
    text: "You can score objectives even if you do not control the planets in your home system.\nOther players cannot activate asteroid fields that contain 1 or more of your ships.",
    source: "twilights_fall",
    faction: "saar",
  },
  {
    alias: "tf-orbitaldrop",
    name: "Orbital Drop",
    types: ["BIOTIC"],
    text: "ACTION: Spend 1 token from your strategy pool to place 2 infantry from your reinforcements on 1 planet you control.",
    source: "twilights_fall",
    faction: "sol",
  },
  {
    alias: "tf-overwatch",
    name: "Overwatch",
    types: ["WARFARE"],
    text: "After another player moves ships into a system that contains 1 of your command tokens: You may return that token to your reinforcements.",
    source: "twilights_fall",
    faction: "empyrean",
  },
  {
    alias: "tf-pacifist",
    name: "Pacifist",
    types: ["BIOTIC"],
    text: "When ground forces are committed: If your units on that planet are not already coexisting, you may choose for your units to coexist.",
    source: "twilights_fall",
    faction: "deepwrought",
  },
  {
    alias: "tf-peaceaccords",
    name: "Peace Accords",
    types: ["CYBERNETIC"],
    text: "After you spend a token from your strategy pool: You may gain control of 1 planet that does not contain the custodians token or any units and is adjacent to a planet you control.",
    source: "twilights_fall",
    faction: "xxcha",
  },
  {
    alias: "tf-pillage",
    name: "Pillage",
    types: ["CYBERNETIC"],
    text: "After 1 of your neighbors gains trade goods or resolves a transaction: If they have 3 or more trade goods, you may take 1 of their trade goods or commodities.",
    source: "twilights_fall",
    faction: "mentak",
  },
  {
    alias: "tf-planesplitter",
    name: "Planesplitter",
    types: ["WARFARE"],
    text: "When you gain this card, put the Fracture into play.\nApply +1 to the Move value to each of your ships that starts its movement in The Fracture.\nApply +2 to the result of each of your units' combat rolls in The Fracture.",
    source: "twilights_fall",
    faction: "obsidian",
  },
  {
    alias: "tf-proxima",
    name: "Proxima Targeting VI",
    types: ["WARFARE"],
    text: "Cancel 1 hit produced by each BOMBARDMENT roll against your units.\nAt the start of a round of ground combat, you may resolve BOMBARDMENT 7(x3) against your opponet's ground forces; if you do, resolve BOMBARDMENT 7 (x3) agianst your own ground forces.",
    source: "twilights_fall",
    faction: "bastion",
  },
  {
    alias: "tf-puppetcouncil",
    name: "Puppet Council",
    types: ["BIOTIC"],
    text: "At the start of the strategy phase: Replenish your commodities, then gain 1 trade good.",
    source: "twilights_fall",
    faction: "keleres",
  },
  {
    alias: "tf-qdn",
    name: "Quantum Datahub Node",
    types: ["CYBERNETIC"],
    text: "At the end of the strategy phase: You may spend 1 token from your strategy pool and give another player 3 of your trade goods. If you do, give 1 of your strategy cards to that player and take 1 of their strategy cards.",
    source: "twilights_fall",
    faction: "hacan",
  },
  {
    alias: "tf-quantumdrive",
    name: "Quantum Drive",
    types: ["PROPULSION"],
    text: "You can produce your flagship without spending resources.",
    source: "twilights_fall",
    faction: "nomad",
  },
  {
    alias: "tf-quantumentanglement",
    name: "Quantum Entanglement",
    types: ["PROPULSION"],
    text: "You treat all systems that contain an alpha or beta wormhole as adjacent to each other. Game effects cannot prevent you from using this ability.",
    source: "twilights_fall",
    faction: "ghost",
  },
  {
    alias: "tf-radicaladvancement",
    name: "Radical Advancement",
    types: ["BIOTIC"],
    text: "At the start of the status phase: You may choose to replace 1 of your abilities; if you do, reveal abilities from the deck until you reveal another ability with the same technology color and gain it; shuffle the replaced technology and the other revealed cards back into your deck.",
    source: "twilights_fall",
    faction: "deepwrought",
  },
  {
    alias: "tf-raidformation",
    name: "Raid Formation",
    types: ["WARFARE"],
    text: "When 1 or more of your units uses ANTI-FIGHTER BARRAGE: For each hit produced in excess of your opponent's fighters, choose 1 of your opponent's ships that has SUSTAIN DAMAGE to become damaged.",
    source: "twilights_fall",
    faction: "argent",
  },
  {
    alias: "tf-reclamation",
    name: "Reclamation",
    types: ["CYBERNETIC"],
    text: "After you resolve an action during which you gained control of a legendary planet: You may place 1 PDS and 1 space dock from your reinforcements on that planet.",
    source: "twilights_fall",
    faction: "winnu",
  },
  {
    alias: "tf-scavenge",
    name: "Scavenge",
    types: ["CYBERNETIC"],
    text: "After you gain control of a planet: Gain 1 trade good.",
    source: "twilights_fall",
    faction: "saar",
  },
  {
    alias: "tf-scc",
    name: "Spatial Conduit Cylinder",
    types: ["PROPULSION"],
    text: "After you activate a system that contains 1 or more of your units: That system is adjacent to all other systems that contain 1 or more of your units during this action.",
    source: "twilights_fall",
    faction: "jolnar",
  },
  {
    alias: "tf-scheming",
    name: "Scheming",
    types: ["BIOTIC"],
    text: "When you draw 1 or more action cards: Draw 1 additional action card.  Then, choose and discard 1 action card from your hand.",
    source: "twilights_fall",
    faction: "yssaril",
  },
  {
    alias: "tf-singularityx",
    name: "Singularity X",
    types: ["BIOTIC"],
    text: "Once per combat, after 1 of your opponent's units is destroyed: You may place the \"Singularity X\" token on one of your opponent's abilities. While that token is on a card, this card gains that card's text. You cannot place a singularity token on a card that already has a singularity token.",
    source: "twilights_fall",
    faction: "nekro",
  },
  {
    alias: "tf-singularityy",
    name: "Singularity Y",
    types: ["BIOTIC"],
    text: "Once per combat, after 1 of your opponent's units is destroyed: You may place the \"Singularity Y\" token on one of your opponent's abilities. While that token is on a card, this card gains that card's text. You cannot place a singularity token on a card that already has a singularity token.",
    source: "twilights_fall",
    faction: "nekro",
  },
  {
    alias: "tf-singularityz",
    name: "Singularity Z",
    types: ["BIOTIC"],
    text: "Once per combat, after 1 of your opponent's units is destroyed: You may place the \"Singularity Z\" token on one of your opponent's abilities. While that token is on a card, this card gains that card's text. You cannot place a singularity token on a card that already has a singularity token.",
    source: "twilights_fall",
    faction: "nekro",
  },
  {
    alias: "tf-sledfactories",
    name: "Sled Factories",
    types: ["CYBERNETIC"],
    text: "When 1 or more of your units use PRODUCTION: Reduce the combined cost of the produced units by 2.",
    source: "twilights_fall",
    faction: "hacan",
  },
  {
    alias: "tf-slipstream",
    name: "Slipstream",
    types: ["PROPULSION"],
    text: "During your tactical actions: Apply +1 to the move value of each of your ships that starts its movement in a system that contains a wormhole.",
    source: "twilights_fall",
    faction: "ghost",
  },
  {
    alias: "tf-smotheringpresence",
    name: "Smothering Presence",
    types: ["BIOTIC"],
    text: "Other players units in or adjacent to systems that contain your structures lose all of their unit abilities.",
    source: "twilights_fall",
    faction: "crimson",
  },
  {
    alias: "tf-specops",
    name: "Spec Ops Training",
    types: ["BIOTIC"],
    text: "After 1 of your ground forces are destroyed: Roll 1 die. If the result is 5 or greater, place the unit on this card. At the start of your next turn, place each unit that is on this card on a planet you control in your home system.",
    source: "twilights_fall",
    faction: "sol",
  },
  {
    alias: "tf-stalltactics",
    name: "Stall Tactics",
    types: ["PROPULSION"],
    text: "ACTION: Discard 1 action card from your hand.",
    source: "twilights_fall",
    faction: "yssaril",
  },
  {
    alias: "tf-starforge",
    name: "Star Forge",
    types: ["CYBERNETIC"],
    text: "Action: Spend 1 token from your strategy pool to place either 2 fighters or 1 destroyer from your reinforcements in a system that contains 1 or more of your ships.",
    source: "twilights_fall",
    faction: "muaat",
  },
  {
    alias: "tf-stellargenesis",
    name: "Stellar Genesis",
    types: ["CYBERNETIC"],
    text: "After you spend a token from your strategy pool: You may gain 1 trade good.",
    source: "twilights_fall",
    faction: "muaat",
  },
  {
    alias: "tf-stymie",
    name: "Stymie",
    types: ["BIOTIC"],
    text: "After another player moves ships into a system that contains 1 or more of your units: You may place 1 command token from that player's reinforcements in any non-home system.",
    source: "twilights_fall",
    faction: "arborec",
  },
  {
    alias: "tf-subatomic",
    name: "Subatomic Splicer",
    types: ["CYBERNETIC"],
    text: "When one of your ships is destroyed: You may produce a ship of the same type at a space dock in your home system.",
    source: "twilights_fall",
    faction: "crimson",
  },
  {
    alias: "tf-supercharge",
    name: "Supercharge",
    types: ["WARFARE"],
    text: "Before making a combat roll: Choose 1 of your units and apply +2 to the result of its combat roll.",
    source: "twilights_fall",
    faction: "naaz",
  },
  {
    alias: "tf-survivalinstinct",
    name: "Survival Instinct",
    types: ["WARFARE"],
    text: "After a player activates a system that contains your ships: You may move up to 2 of your ships into the active system from adjacent systems that do not contain your command tokens.",
    source: "twilights_fall",
    faction: "ralnel",
  },
  {
    alias: "tf-tacticalbrilliance",
    name: "Tactical Brilliance",
    types: ["WARFARE"],
    text: "After you roll dice for a unit ability: You may reroll any of those dice.",
    source: "twilights_fall",
    faction: "jolnar",
  },
  {
    alias: "tf-tcs",
    name: "Temporal Command Suite",
    types: ["CYBERNETIC"],
    text: "After any player's genome becomes exhausted: You may spend 1 command token from any pool to ready that genome; if you ready another player's genome you may perform a transaction with that player. [Note: Cost changed from 3 influence to 1 CC, to combat infinite loops and with Dane's assent.)",
    source: "twilights_fall",
    faction: "nomad",
  },
  {
    alias: "tf-telepathic",
    name: "Telepathic",
    types: ["BIOTIC"],
    text: 'At the end of the strategy phase: Place the Naalu "0" token on your strategy card; you are first in initiative order.\nThis card cannot have a singularity token placed on it.',
    source: "twilights_fall",
    faction: "naalu",
  },
  {
    alias: "tf-terraform",
    name: "Terraform",
    types: ["PROPULSION"],
    text: "Planets that contain your structures are treated as having all 3 planet traits (cultural, hazardous, and industrial).",
    source: "twilights_fall",
    faction: "titans",
  },
  {
    alias: "tf-theburningeye",
    name: "The Burning Eye",
    types: ["WARFARE"],
    text: "You can treat planets in systems that contain your ships as if you controlled them for the purpose of scoring secret objectives.",
    source: "twilights_fall",
    faction: "obsidian",
  },
  {
    alias: "tf-unrelenting",
    name: "Unrelenting",
    types: ["WARFARE"],
    text: "Apply +1 to the result of each of your unit's combat rolls.",
    source: "twilights_fall",
    faction: "sardakk",
  },
  {
    alias: "tf-valkyrie",
    name: "Valkyrie Vanguard",
    types: ["PROPULSION"],
    text: 'During the "Commit Ground Forces" step: You can commit up to 1 ground force from each planet in the active system and each planet in adjacent systems that do not contain 1 of your command tokens.',
    source: "twilights_fall",
    faction: "sardakk",
  },
  {
    alias: "tf-versatile",
    name: "Versatile",
    types: ["BIOTIC"],
    text: "When you gain command tokens during the status phase: Gain 1 additional command token.",
    source: "twilights_fall",
    faction: "sol",
  },
  {
    alias: "tf-voidborn",
    name: "Voidborn",
    types: ["PROPULSION"],
    text: "Nebulae do not affect your ships' movement.\nYou can allow players to move their ships through systems that contain your ships.",
    source: "twilights_fall",
    faction: "empyrean",
  },
  {
    alias: "tf-vpw",
    name: "Valkyrie Particle Weave",
    types: ["WARFARE"],
    text: "After making combat rolls during a round of ground combat: If your opponent produced 1 or more hits, you produce 1 additional hit.",
    source: "twilights_fall",
    faction: "sardakk",
  },
  {
    alias: "tf-yinascendant",
    name: "Yin Ascendant",
    types: ["BIOTIC"],
    text: "When you score an objective: Draw 1 genome, ability, or unit upgrade.",
    source: "twilights_fall",
    faction: "yin",
  },
  {
    alias: "tf-zealous",
    name: "Zealous",
    types: ["WARFARE"],
    text: "When 1 or more of your units roll dice for a unit ability: You may choose 1 of those units to roll 1 additional die.",
    source: "twilights_fall",
    faction: "argent",
  },
  {
    alias: "tp",
    name: "Transparasteel Plating",
    types: ["BIOTIC"],
    requirements: "G",
    faction: "yssaril",
    source: "base",
    text: "During your turn of the action phase, players that have passed cannot play action cards.",
    shrinkName: true,
    shortName: "Transpara.\nPlating",
    imageURL: "/hover_images/techs/faction/transparasteel_plating.jpg",
  },
  {
    alias: "ultrafine_satellite_array",
    name: "Ultrafine Satellite Array",
    types: ["CYBERNETIC"],
    requirements: "YY",
    source: "cpti",
    text: "When you explore a planet, you may draw your card from the top the that deck's discard pile instead.\n\nACTION: Exhaust this card to explore each planet you control that contains one of your units with PLANETARY SHIELD.",
  },
  {
    alias: "vacuum_transposition",
    name: "Vacuum Transposition",
    types: ["PROPULSION"],
    requirements: "BBB",
    source: "cpti",
    text: "After you produce units, you may exhaust this card to move those units into a system containing your units and no planets.\n\nAfter you perform your Hero's ability, place 1 frontier token in each system that contains your ships and no planets.",
  },
  {
    alias: "vax",
    name: "Valefar Assimilator X",
    types: ["NONE"],
    faction: "nekro",
    source: "base",
    text: "When you would gain another player's technology using 1 of your faction abilities, you may place the \"X\" assimilator token on a faction technology owned by that player instead.\nWhile that token is on a technology, this card gains that technology's text.\nYou cannot place an assimilator token on technology that already has an assimilator token.",
    imageURL: "/hover_images/techs/faction/valefar_assimilator_x.jpg",
  },
  {
    alias: "vay",
    name: "Valefar Assimilator Y",
    types: ["NONE"],
    faction: "nekro",
    source: "base",
    text: 'When you would gain another players technology using 1 of your faction abilities, you may place the "Y" assimilator token on a faction technology owned by that player instead.\nWhile that token is on a technology, this card gains that technology\'s text.\nYou cannot place an assimilator token on technology that already has an assimilator token.',
    imageURL: "/hover_images/techs/faction/valefar_assimilator_y.jpg",
  },
  {
    alias: "vpw",
    name: "Valkyrie Particle Weave",
    types: ["WARFARE"],
    requirements: "RR",
    faction: "sardakk",
    source: "base",
    text: "After making combat rolls during a round of ground combat, if your opponent produced 1 or more hits, you produce 1 additional hit.",
    shortName: "Valkyrie\nPtcl. Weave",
    shrinkName: true,
    imageURL: "/hover_images/techs/faction/valkyrie_particle_weave.jpg",
  },
  {
    alias: "vtx",
    name: "Vortex",
    types: ["WARFARE"],
    requirements: "R",
    faction: "cabal",
    source: "pok",
    text: "ACTION: Exhaust this card to choose another player's non-structure unit in a system that is adjacent to 1 or more of your space docks. Capture 1 unit of that type from that player's reinforcements.",
    imageURL: "/hover_images/techs/faction/vortex.jpg",
  },
  {
    alias: "vw",
    name: "Voidwatch",
    types: ["BIOTIC"],
    requirements: "G",
    faction: "empyrean",
    source: "pok",
    text: "After a player moves ships into a system that contains 1 or more of your units, they must give you 1 promissory note from their hand, if able.",
    imageURL: "/hover_images/techs/faction/voidwatch.jpg",
  },
  {
    alias: "wavelength",
    name: "Wavelength",
    types: ["NONE"],
    source: "twilights_fall",
    text: "Your ships can move into and through asteroid fields. When you activate a system, you may explore 1 planet in that system that contains 1 or more of your units.",
  },
  {
    alias: "wg",
    name: "Wormhole Generator",
    types: ["PROPULSION"],
    requirements: "BB",
    faction: "ghost",
    source: "base",
    text: "ACTION: Exhaust this card to place or move a Creuss wormhole token into either a system that contains a planet you control or a non-home system that does not contain another player's ships.",
    imageURL: "/hover_images/techs/faction/wormhole_generator_omega.jpg",
  },
  {
    alias: "ws",
    name: "War Sun",
    types: ["UNITUPGRADE"],
    requirements: "RRRY",
    source: "base",
    text: "Cost 12, Combat 3(x3), Move 2, Capacity 6\nSUSTAIN DAMAGE, BOMBARDMENT 3(x3)\n Other players' units in this system lose PLANETARY SHIELD.",
    imageURL: "/hover_images/techs/generic/warsun.jpg",
  },
  {
    alias: "x89",
    name: "X-89 Bacterial Weapon Ω",
    types: ["BIOTIC"],
    requirements: "GGG",
    source: "codex1",
    text: "After 1 or more of your units use BOMBARDMENT against a planet, if at least 1 of your opponent's infantry was destroyed, you may destroy all of your opponent's infantry on that planet.",
    imageURL: "/hover_images/techs/generic/x89_bacterial_weapon_omega.png",
    initials: "X89",
    shortName: "X-89 Bact. Weapon",
  },
  {
    alias: "x89_base",
    name: "X-89 Bacterial Weapon",
    types: ["BIOTIC"],
    requirements: "GGG",
    source: "base",
    text: "ACTION: Exhaust this card and choose 1 planet in a system that contains 1 or more of your ships that have BOMBARDMENT; destroy all infantry on that planet",
    imageURL: "/hover_images/techs/generic/x89_bacterial_weapon.png",
    initials: "X89",
    shortName: "X-89 Bact. Weapon",
  },
  {
    alias: "x89c4",
    name: "X-89 Bacterial Weapon ΩΩ",
    types: ["BIOTIC"],
    requirements: "GGG",
    source: "codex4",
    text: "Double the hits produced by your units' BOMBARDMENT and ground combat rolls.\nExhaust each planet you use BOMBARDMENT against.",
    imageURL:
      "/hover_images/techs/generic/x89_bacterial_weapon_omega_omega.png",
    initials: "X89",
    shortName: "X-89 Bact. Weapon",
  },
  {
    alias: "yso",
    name: "Yin Spinner Omega",
    types: ["BIOTIC"],
    requirements: "GG",
    faction: "yin",
    source: "base",
    text: "After you produce units, place up to 2 infantry from your reinforcements on any planet you control or in any space area that contains 1 or more of your ships.",
    shortName: "Yin Spinner",
    imageURL: "/hover_images/techs/faction/yin_spinner_omega.jpg",
  },
];
